...
README images (`docs/screenshots/readme-*.jpg`) and dedicated desktop captures (`docs/screenshots/site/`), because the website
...
from its original capture, resized to the 1280x800 the frames show and encoded as
...
Run `node scripts/capture-site-gallery.mjs` after building the application to
refresh the additional Teaching, Study and Databases views. It uses an isolated
temporary profile, English interface and example content, light mode, and the
current Stellar graph. It preserves all existing white README captures.
The generated example photos and their prompts are in `scripts/fixtures/site-gallery/`.
