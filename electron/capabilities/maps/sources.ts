...
export const mapSourceTransport: MapSourceTransport = {
  async read(url, signal, limit) {
    ...
    const response = await fetch(url, { signal, redirect:'error', credentials:'omit', headers:{Accept:'application/json, application/geo+json, text/plain', 'User-Agent':'NodusResearch/5.4.3 (https://nodusresearch.com)'} });
