...
export interface RuntimeLockEntry { name: string; url: string; bytes: number; sha256: string }
...
export interface EnsureRuntimeContext {
  ...
  minVersion: string;
  /** The lock shipped inside the installed package, already read from disk. */
  lock: RuntimeLock;
  signal: AbortSignal;
}
...
export async function ensurePythonRuntime(runtime: TrustedWorkerRuntime, runtimeId: string, context: EnsureRuntimeContext): Promise<{ ready: boolean; detail?: string }> {
  ...
  const lockDigest = createHash('sha256').update(JSON.stringify(context.lock)).digest('hex');

  // The marker records which lock produced the environment: a package that changes its
  // dependencies gets a rebuild instead of an environment that no longer matches it.
  try {
  ...
  try {
    ...
    for (const entry of context.lock.packages) {
    ...
    fs.writeFileSync(requirements, context.lock.packages.map(entry => `${entry.name} --hash=sha256:${entry.sha256}`).join('\n'), { mode: 0o600 });
