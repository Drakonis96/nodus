...
import {
  ...
  hasDemoPortraitKey,
} from "../ai/genealogyDemoPortraits";
...
import {
  ...
  saveSearch,
  deleteSavedSearch,
} from "../db/savedSearchesRepo";
import { getCorpusHealth } from "../db/corpusHealthRepo";
import {
  analyzeChapterRelations,
  getChapterRelations,
  onChapterRelationsProgress,
} from "../ai/chapterIdeas";
import {
  applyManuscriptCitation,
  verifyManuscriptCitations,
} from "../ai/manuscriptVerifier";
...
import {
  ...
  synthesizeMatrixCell,
} from "../ai/synthesisMatrix";
import {
  getCachedWorkIdeaSynthesis,
  synthesizeWorkIdeas,
} from "../ai/workIdeaSynthesis";
...
import {
  buildImmersionScope,
  evaluateImmersionAnswer,
  generateImmersionSession,
} from "../ai/immersion";
...
import {
  annotatedEpubBytes,
  annotatedPdfBytes,
} from "../export/studyMaterialAnnotations";
import { generateStudyQuestions } from "../ai/studyQuestions";
import {
  onStudyMaterialIndexChanged,
  queueStudyMaterialIndex,
  reindexStudyMaterial,
} from "../ai/studyMaterialIndex";
import {
...
import {
  buildWritingWorkshopSnapshot,
  generateWritingWorkshopDraft,
} from "../ai/writingWorkshop";
...
import {
  discoverSemanticBridges,
  onSemanticBridgeProgress,
} from "../ai/semanticBridges";
...
import {
  ...
  exportWritingWorkshopDraft,
} from "../export/writingWorkshopExport";
...
async function importStudyMaterialPaths(
  paths: string[],
  input: StudyMaterialImportInput = {},
) {
  const results: Awaited<
    ReturnType<typeof studyMaterials.importStudyMaterialFile>
  >[] = [];
  const visit = async (filePath: string): Promise<void> => {
    ...
    if (path.extname(filePath).toLowerCase() === ".zip") {
      const temp = fs.mkdtempSync(
        path.join(os.tmpdir(), "nodus-study-import-"),
      );
      try {
        ...
        for (const entry of entries) {
          ...
          const extracted = path.join(
            temp,
            `${String(index++).padStart(4, "0")}-${name}`,
          );
          fs.writeFileSync(extracted, entry.getData());
          results.push(
            await studyMaterials.importStudyMaterialFile(extracted, input),
          );
        }
      } finally {
    ...
    if (studyMaterials.supportsStudyMaterial(filePath))
      results.push(
        await studyMaterials.importStudyMaterialFile(filePath, input),
      );
  };
...
function announceWritingDrafts(): void {
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed() && !win.webContents.isDestroyed())
      win.webContents.send("writing:saved:changed", null);
  }
}
...
function announceDictionary(entryId: string | null): void {
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed() && !win.webContents.isDestroyed())
      win.webContents.send("dictionary:changed", entryId);
  }
}
...
export function registerAcademicIpc({
  h,
  getWindow,
  chatAborters,
}: IpcContext): void {
  ...
  h("dictionary:list", async (_e, request: DictionaryListRequest) =>
    dictionaryRepo.listDictionaryEntries(request),
  );
  h("dictionary:facets", async () => dictionaryRepo.listDictionaryFacets());
  h("dictionary:get", async (_e, id: string) =>
    dictionaryRepo.getDictionaryEntryDetail(id),
  );
  ...
  h(
    ...
    async (
      ...
      const entry = dictionaryRepo.updateDictionaryEntry(
        id,
        patch,
        expectedUpdatedAt,
      );
  ...
  h("dictionary:duplicates", async (_e, name: string, aliases: string[]) =>
    detectDictionaryDuplicatesSemantic(name, aliases),
  );
  ...
  h(
    ...
    async (_e, request: DictionaryEvidenceRequest) =>
      dictionaryRepo.listDictionaryEvidence(request),
  );
  h(
    "dictionary:evidence:decision",
    async (
      _e,
      entryId: string,
      refs: DictionaryEvidenceRef[],
  ...
  h("dictionary:generate", async (_e, request: DictionaryGenerationRequest) => {
    try {
      ...
      console.error(
        `[dictionary] generation failed for ${request.entryId}`,
        error,
      );
  ...
  h(
    "dictionary:generate:start",
    async (_e, request: DictionaryGenerationRequest) => {
      const running = dictionaryGenerationJobs.get(request.entryId);
      if (running && !["done", "failed"].includes(running.phase))
        return running;
      ...
      setImmediate(() => {
        void (async () => {
          try {
            const retrieving: DictionaryProgress = {
              entryId: request.entryId,
              phase: "retrieving",
              message: "Analizando corpus",
            };
            ...
            const generating: DictionaryProgress = {
              entryId: request.entryId,
              phase: "generating",
            ...
            const failed: DictionaryProgress = {
              entryId: request.entryId,
              phase: "failed",
  ...
  h("dictionary:versions:list", async (_e, entryId: string) =>
    dictionaryRepo.listDictionaryVersions(entryId),
  );
  h(
    "dictionary:versions:accept",
    async (
      _e,
      entryId: string,
      versionId: string,
      ...
      const detail = dictionaryRepo.acceptDictionaryVersion(
        entryId,
        versionId,
  ...
  h(
    ...
    async (
      ...
      const detail = dictionaryRepo.restoreDictionaryVersion(
        entryId,
        versionId,
        expectedCurrentVersionId,
      );
  ...
  h(
    ...
    async (
      ...
      const relation = dictionaryRepo.addDictionaryRelation(
        fromEntryId,
        toEntryId,
        type,
      );
  ...
  const queueImportedStudyKnowledge = async (
    ...
    const decision = await decideStudyMaterialAiProcessing(
      results,
      subjectId,
      getWindow(),
    );
    ...
    queueStudyKnowledgeSources(
      "material",
      results.map((result) => result.material.id),
      false,
      {
        approved: true,
  ...
  h("works:listPage", async (_e, filter, request) =>
    works.listWorksPage(filter, request),
  );
  ...
  h(
    "works:setManualDeep",
    async (_e, nodusId: string, value: boolean, model?: ModelRef | null) => {
  ...
  h(
    "works:setManualDeepBulk",
    async (_e, nodusIds: string[], value: boolean, model?: ModelRef | null) => {
  ...
  h(
    "works:analyzeBoth",
    async (_e, nodusId: string, model?: ModelRef | null) => {
      const w = works.getWork(nodusId);
      if (!w) return;
  ...
  h(
    "works:analyzeBothBulk",
    async (_e, nodusIds: string[], model?: ModelRef | null) => {
      for (const id of nodusIds) {
        const w = works.getWork(id);
        if (!w) continue;
        works.setLightPending(id);
  ...
  h("works:reassignThemes", async (_e, model?: ModelRef | null) => {
    ...
    const all = getDb()
      .prepare("SELECT nodus_id, title FROM works WHERE archived = 0")
      .all() as {
      nodus_id: string;
      title: string;
    }[];
  ...
  h("works:meta", async (_e, nodusId: string) => {
    ...
    const meta = await zotero
      .getItemMeta(zoteroUserId, w.zotero_key)
      .catch(() => null);
    if (!meta) return null;
    return meta;
  });
  h(
    "works:rescan",
    async (_e, nodusId: string, kind: QueueKind, model?: ModelRef | null) => {
      const w = works.getWork(nodusId);
      if (!w) return;
      if (kind === "deep") {
  ...
  h("works:getSummary", async (_e, nodusId: string) =>
    workSummaries.getWorkSummary(nodusId),
  );
  ...
  h(
    "works:openAtPage",
    async (_e, nodusId: string, location: string | null) => {
      const work = works.getWork(nodusId);
      if (!work?.zotero_key) return { ok: false, mode: "none" as const };
      const page = parsePageNumber(location);
      if (page !== null) {
        const attachmentKey = await zotero.resolvePdfAttachmentKey(
          getSettings().zoteroUserId,
          work.zotero_key,
        );
        if (attachmentKey) {
  ...
  h("libraryReader:get", async (_e, nodusId: string) =>
    libraryReader.getLibraryReaderDocument(nodusId),
  );
  h(
    "libraryReader:attachmentContent",
    async (_e, nodusId: string, attachmentId: string) =>
  ...
  h(
    "libraryReader:annotations:create",
    async (_e, nodusId: string, input: WritingDraftAnnotationInput) => {
      const annotation = await createLibraryReaderAnnotationInWorker(nodusId, {
        ...input,
        draftId: nodusId,
      });
  ...
  h(
    "libraryReader:annotations:updateComment",
    async (_e, nodusId: string, id: string, comment: string) => {
      const annotation = await updateLibraryReaderCommentInWorker(
        nodusId,
        id,
        comment,
      );
  ...
  h(
    "libraryReader:annotations:delete",
    async (_e, nodusId: string, id: string) => {
  ...
  h(
    "libraryReader:chat:stream",
    async (event, requestId: string, request: LibraryReaderChatRequest) => {
      ...
      try {
        const response = await streamLibraryReaderChat(
          ...
          (delta, kind) =>
            event.sender.send(
              kind === "reasoning"
                ? "libraryReader:chat:reasoning"
                : "libraryReader:chat:delta",
              requestId,
              delta,
            ),
          controller.signal,
        ...
        libraryReader.saveLibraryReaderChatMessages(
          request.documentId,
          persisted,
        );
        return response;
  ...
  h(
    "study:knowledge:processing:resolve",
    async (event, requestId: string, decision) => {
      resolveStudyMaterialAiProcessingRequest(
        event.sender.id,
        requestId,
        decision,
  ...
  h("works:uploadText", async (_e, nodusId: string, filePath: string) => {
    const w = getDb()
      .prepare("SELECT * FROM works WHERE nodus_id = ?")
      .get(nodusId) as any;
    if (!w) return;
    const s = getSettings();
  ...
  h("queue:enqueueBridge", async (_e, model?: ModelRef | null) =>
    scanQueue.enqueueBridge(model),
  );
  ...
  h("graph:theme", async (_e, theme: string, cap?: number) =>
    buildIdeaThemeGraph(theme, cap),
  );
  h("ideas:listPage", async (_e, request) => ideas.listIdeasPage(request));
  h("ideas:picker", async () => ideas.listPickerIdeas());
  h("ideas:connections", async (_e, globalId: string) =>
    ideas.listIdeaConnections(globalId),
  );
  h("graph:ideaDetail", async (_e, globalId: string) =>
    ideas.getIdeaDetail(globalId),
  );
  h("ideas:delete", async (_e, globalId: string) => ideas.deleteIdea(globalId));
  h("graph:edgeDetail", async (_e, edgeId: string) =>
    ideas.getEdgeDetail(edgeId),
  );
  h("graph:ideaEdges", async (_e, globalId: string) =>
    ideas.getIdeaEdges(globalId),
  );
  h(
    "graph:edgeFeedback:set",
    async (
      _e,
      fromId: string,
      toId: string,
      type: string,
      verdict: "rejected" | "confirmed" | null,
      note?: string,
  ...
  h(
    "works:ideasByWork",
    async (_e, nodusId: string, limit: number, offset: number) =>
      ideas.getIdeasByWork(nodusId, limit, offset),
  );
  h("works:getIdeaSynthesis", async (_e, nodusId: string) =>
    getCachedWorkIdeaSynthesis(nodusId),
  );
  h(
    "works:synthesizeIdeas",
    async (_e, nodusId: string, model?: ModelRef | null) =>
  ...
  h("authors:setSaved", async (_e, authorId: string, saved: boolean) =>
    setAuthorSaved(authorId, saved),
  );
  h("authors:dossier", async (_e, authorId: string) =>
    buildAuthorDossier(authorId),
  );
  h(
    "authors:synthesize",
    async (_e, authorId: string, model?: ModelRef | null) =>
      synthesizeAuthorDossier(authorId, model),
  );
  h("authors:matrix", async () => buildSynthesisMatrix());
  h(
    "authors:matrixCell",
    async (_e, authorId: string, themeId: string, model?: ModelRef | null) =>
      synthesizeMatrixCell(authorId, themeId, model),
  );
  h(
    "authors:exportSyntheses",
    async (_e, request: AuthorSynthesisExportRequest) =>
      exportAuthorSyntheses(request),
  );

  // study guide
  h("study:workspace", async (_e, options?: StudyWorkspaceOptions) =>
    studyOrg.getStudyWorkspace(options),
  );
  h("study:schedule:get", async (_e, academicYearId?: string | null) =>
    studySchedule.getStudySchedule(academicYearId ?? null),
  );
  h("study:schedule:save", async (_e, schedule: StudySchedule) =>
    studySchedule.saveStudySchedule(schedule),
  );
  h(
    "study:schedule:copy",
    async (
  ...
  h(
    "study:academicYear:create",
    async (_e, input: CreateStudyAcademicYearInput) =>
      studyOrg.createStudyAcademicYear(input),
  );
  h(
    "study:academicYear:update",
    async (_e, id: string, patch: UpdateStudyAcademicYearInput) =>
      studyOrg.updateStudyAcademicYear(id, patch),
  );
  h("study:academicYear:delete", async (_e, id: string) => {
    studyOrg.deleteStudyAcademicYear(id);
  });
  h("study:course:create", async (_e, input: CreateStudyCourseInput) =>
    studyOrg.createStudyCourse(input),
  );
  h("study:subject:create", async (_e, input: CreateStudySubjectInput) =>
    studyOrg.createStudySubject(input),
  );
  h("study:topic:create", async (_e, input: CreateStudyTopicInput) =>
    studyOrg.createStudyTopic(input),
  );
  h("study:folder:create", async (_e, input: CreateStudyFolderInput) =>
    studyOrg.createStudyFolder(input),
  );
  h("study:document:create", async (_e, input: CreateStudyDocumentInput) => {
    const result = studyOrg.createStudyDocument(input);
    queueStudyKnowledgeSources("document", [result.id]);
  ...
  h(
    ...
    async (
      _e,
      kind: "subject" | "folder" | "topic",
      id: string,
      input: StudyEntityMoveInput,
    ) => {
      const result = studyOrg.moveStudyEntity(kind, id, input);
      const materialIds = (
        getDb()
          .prepare("SELECT id FROM study_materials WHERE deleted_at IS NULL")
          .all() as Array<{ id: string }>
      ).map((row) => row.id);
      const documentIds = (
        getDb()
          .prepare("SELECT id FROM study_docs WHERE deleted_at IS NULL")
          .all() as Array<{ id: string }>
      ).map((row) => row.id);
      for (const sourceId of materialIds)
        studyKnowledgeRepo.syncStudyKnowledgeSourceScopes("material", sourceId);
      for (const sourceId of documentIds)
        studyKnowledgeRepo.syncStudyKnowledgeSourceScopes("document", sourceId);
      queueStudyKnowledgeSources("material", materialIds);
      queueStudyKnowledgeSources("document", documentIds);
      studySearch.queueStudySearchIndexRefresh();
      return result;
    },
  );
  h(
    "study:placement:add",
    async (_e, documentId: string, input: StudyPlacementInput) => {
      const result = studyOrg.addStudyPlacement(documentId, input);
  ...
  h(
    "study:placement:setPrimary",
    async (_e, documentId: string, input: StudyPlacementInput) => {
      const result = studyOrg.setPrimaryStudyPlacement(documentId, input);
      studyKnowledgeRepo.syncStudyKnowledgeSourceScopes("document", documentId);
      queueStudyKnowledgeSources("document", [documentId]);
      studySearch.queueStudySearchIndexRefresh();
      return result;
  ...
  h("study:placement:remove", async (_e, id: string) => {
    const row = getDb()
      .prepare("SELECT document_id FROM study_placements WHERE id=?")
      .get(id) as { document_id: string } | undefined;
    const result = studyOrg.removeStudyPlacement(id);
    if (row) {
      studyKnowledgeRepo.syncStudyKnowledgeSourceScopes(
        "document",
        row.document_id,
      );
      queueStudyKnowledgeSources("document", [row.document_id]);
      studySearch.queueStudySearchIndexRefresh();
    }
    return result;
  });
  h(
    ...
    async (
      ...
      kind: StudyEntityKind,
      id: string,
      action: StudyLifecycleAction,
      options?: { purgeLinkedKnowledge?: boolean },
    ) => {
      const destructive = action === "trash";
      const purgeLinkedKnowledge =
        destructive && options?.purgeLinkedKnowledge !== false;
      if (kind === "document" && purgeLinkedKnowledge)
        studyKnowledgeRepo.purgeStudyKnowledgeSource("document", id);
      const result = studyOrg.setStudyLifecycle(kind, id, action);
      if (kind === "document") {
        if (!destructive || purgeLinkedKnowledge)
          studyKnowledgeRepo.syncStudyKnowledgeSourceScopes("document", id);
        if (action === "restore" || action === "recover")
          queueStudyKnowledgeSources("document", [id]);
        studySearch.queueStudySearchIndexRefresh();
      } else {
        const materialIds = (
          getDb()
  ...
  h("study:tree:duplicate", async (_e, kind: StudyEntityKind, id: string) =>
    studyOrg.duplicateStudyTree(kind, id),
  );
  h("study:tag:create", async (_e, input: CreateStudyTagInput) =>
    studyOrg.createStudyTag(input),
  );
  h(
    "study:tag:update",
    async (
      _e,
      id: string,
      patch: Partial<CreateStudyTagInput> & {
        favorite?: boolean;
        position?: number;
      },
  ...
  h(
    "study:document:setTags",
    async (_e, documentId: string, tagIds: string[]) =>
      studyOrg.setStudyDocumentTags(documentId, tagIds),
  );
  h("study:template:create", async (_e, input: CreateStudyTemplateInput) =>
    studyOrg.createStudyTemplate(input),
  );
  h(
    "study:template:update",
    async (
      _e,
      id: string,
      patch: Partial<CreateStudyTemplateInput> & {
        favorite?: boolean;
        position?: number;
      },
    ) => studyOrg.updateStudyTemplate(id, patch),
  );
  h("study:template:delete", async (_e, id: string) =>
    studyOrg.deleteStudyTemplate(id),
  );
  h("study:template:apply", async (_e, id: string, name?: string) =>
    studyOrg.applyStudyTemplate(id, name),
  );
  h("study:editor:data", async (_e, documentId: string) =>
    studyEditor.getStudyDocEditorData(documentId),
  );
  h(
    "study:editor:update",
    async (_e, documentId: string, input: StudyDocUpdateInput) => {
  ...
  h(
    "study:editor:restore",
    async (_e, documentId: string, versionId: string) => {
  ...
  h(
    "study:annotation:create",
    async (_e, documentId: string, input: StudyAnnotationInput) =>
      studyEditor.createStudyAnnotation(documentId, input),
  );
  h(
  ...
  h("study:annotation:delete", async (_e, id: string) =>
    studyEditor.deleteStudyAnnotation(id),
  );
  h("study:stt:transcribe", async (event, request: StudySttRequest) => {
    ...
    if (provider === "whisper_cpp") {
      result = await transcribeWhisperCpp(request, {
        onProgress: (fraction) =>
          event.sender.send("study:stt:progress", request.requestId, fraction),
        onPartial: (text) =>
          event.sender.send("study:stt:partial", request.requestId, text),
      });
    } else if (provider === "openai") {
      result = await transcribeOpenAiStudyAudio(request);
    } else {
      throw new Error(
        "Transformers.js se ejecuta en el worker local del renderer.",
      );
    }
    event.sender.send("study:stt:complete", request.requestId);
    return result;
  });
  h("study:stt:cancel", async (_event, requestId: string) =>
    cancelWhisperCpp(requestId),
  );
  ...
  h(
    "study:stt:whisperCpp:download",
    async (event, requestId: string, model: string) =>
      downloadWhisperCppModel(model, (fraction) =>
        event.sender.send("study:stt:modelProgress", requestId, fraction),
      ),
  );
  h("study:stt:whisperCpp:delete", async (_event, model: string) =>
    deleteWhisperCppModel(model),
  );
  h(
    "study:styles:list",
    async (_e, options?: { includeArchived?: boolean; search?: string }) =>
      studyStyles.listStudyStyles(options),
  );
  h("study:styles:create", async (_e, input: StudyStyleInput) =>
    studyStyles.createStudyStyle(input),
  );
  h(
    "study:styles:update",
    async (_e, id: string, patch: Partial<StudyStyleInput>) =>
      studyStyles.updateStudyStyle(id, patch),
  );
  h("study:styles:duplicate", async (_e, id: string) =>
    studyStyles.duplicateStudyStyle(id),
  );
  h("study:styles:archive", async (_e, id: string, archived: boolean) =>
    studyStyles.archiveStudyStyle(id, archived),
  );
  h("study:styles:delete", async (_e, id: string) =>
    studyStyles.deleteStudyStyle(id),
  );
  h("study:styles:versions", async (_e, styleId: string) =>
    studyStyles.listStudyStyleVersions(styleId),
  );
  h("study:styles:restore", async (_e, styleId: string, versionId: string) =>
    studyStyles.restoreStudyStyleVersion(styleId, versionId),
  );
  h("study:styles:associations", async () =>
    studyStyles.listStudyStyleAssociations(),
  );
  h(
    "study:styles:associate",
    async (
      _e,
      styleId: string,
  ...
  h(
    "study:styles:default",
    async (_e, subjectId?: string | null, documentKind?: string | null) =>
  ...
  h("study:styles:export", async (_e, styleIds?: string[]) => {
    ...
    const picked = await dialog.showSaveDialog(getWindow() ?? undefined!, {
      title: "Exportar estilos de estudio",
      defaultPath: "nodus-study-styles.json",
      filters: [{ name: "Nodus Study Styles", extensions: ["json"] }],
    });
  ...
  h("study:styles:import", async () => {
    const picked = await showImportOpenDialog(getWindow() ?? undefined!, {
      title: "Importar estilos de estudio",
      properties: ["openFile"],
      filters: [{ name: "Nodus Study Styles", extensions: ["json"] }],
    });
    if (picked.canceled || !picked.filePaths[0]) return [];
    const payload = JSON.parse(
      fs.readFileSync(picked.filePaths[0], "utf8"),
    ) as StudyStyleExport;
    return studyStyles.importStudyStyles(payload);
  });
  h(
    "study:improve",
    async (e, requestId: string, request: StudyImproveRequest) => {
      const controller = new AbortController();
      studyImproveAborters.set(requestId, controller);
      try {
        return await improveStudyText(
          request,
          (delta) => {
            if (!e.sender.isDestroyed())
              e.sender.send("study:improve:delta", requestId, delta);
          },
          controller.signal,
        );
      } finally {
  ...
  h("study:synonyms", async (_e, request: StudySynonymRequest) =>
    suggestStudySynonyms(request),
  );
  h("study:improve:cancel", async (_e, requestId: string) =>
    studyImproveAborters.get(requestId)?.abort(),
  );
  h("study:improve:log", async (_e, documentId: string) =>
    studyStyles.listStudyImprovementLog(documentId),
  );
  h(
    "study:improve:action",
    async (_e, id: string, action: StudyImprovementLog["action"]) =>
      studyStyles.updateStudyImprovementAction(id, action),
  );
  h("study:materials:list", async (_e, options?: StudyMaterialListOptions) =>
    studyMaterials.listStudyMaterials(options),
  );
  h("study:materials:get", async (_e, id: string) =>
    studyMaterials.getStudyMaterial(id),
  );
  h("study:materials:content", async (_e, id: string) =>
    studyMaterials.getStudyMaterialContent(id),
  );
  h("study:materials:download", async (_e, id: string) => {
    const material = studyMaterials.getStudyMaterial(id);
    if (material.origin === "zotero_link")
      throw new Error(
        "Este material es un enlace de Zotero y no contiene un fichero local que descargar.",
      );
    const content = studyMaterials.getStudyMaterialContent(id);
    const safeName =
      path.basename(material.fileName).replace(/[\\/:*?"<>|]+/g, "-") ||
      `material.${material.extension || "bin"}`;
    const picked = await dialog.showSaveDialog(getWindow() ?? undefined!, {
      title: "Descargar material",
      defaultPath: safeName,
      filters: material.extension
        ? [
            {
              name: material.extension.toUpperCase(),
  ...
  h("study:materials:import", async (_e, input?: StudyMaterialImportInput) => {
    const picked = await showImportOpenDialog(getWindow() ?? undefined!, {
      title: "Añadir materiales de estudio",
  ...
  h(
    "study:materials:importFolder",
    async (_e, input?: StudyMaterialImportInput) => {
      const picked = await showImportOpenDialog(getWindow() ?? undefined!, {
        title: "Añadir carpeta de materiales",
  ...
  h("study:materials:choosePaths", async (_e, folder?: boolean) => {
    const picked = await showImportOpenDialog(
      getWindow() ?? undefined!,
      folder
        ? {
            title: "Seleccionar carpeta de materiales",
            properties: ["openDirectory"],
          }
        : {
            title: "Seleccionar materiales de estudio",
            properties: ["openFile", "multiSelections"],
            filters: [
              {
                name: "Materiales de estudio",
  ...
  h(
    "study:materials:importPaths",
    async (_e, paths: string[], input?: StudyMaterialImportInput) => {
      const safePaths = [
        ...new Set(
          paths.filter(
  ...
  h(
    "study:materials:importZotero",
    async (_e, input: ZoteroStudyMaterialImportInput) => {
      const { zoteroUserId } = getSettings();
      const canonicalItemKey =
        input.library.type === "group"
          ? `groups:${input.library.id}:${input.itemKey}`
          : input.itemKey;
      const item = await zotero.getItem(
        zoteroUserId,
        canonicalItemKey,
        input.library,
      );
      if (!item)
        throw new Error("El elemento ya no está disponible en Zotero.");
      const attachments = await zotero.itemAttachments(
        zoteroUserId,
        canonicalItemKey,
        input.library,
      );
      const attachment = input.attachmentKey
        ? (attachments.find(
            (candidate) =>
              candidate.itemKey === input.attachmentKey ||
              candidate.key === input.attachmentKey,
          ) ?? null)
        : (attachments[0] ?? null);
      ...
      if (input.mode === "link") {
        return studyMaterials.linkStudyMaterialFromZotero(
          input.library,
          item,
          attachment,
          placement,
      ...
      if (!attachment)
        throw new Error("Elige un adjunto para importarlo a Nodus.");
      const filePath = await zotero.attachmentFilePath(
        zoteroUserId,
        attachment.key,
      );
      if (!filePath || !fs.existsSync(filePath))
        throw new Error(
          "El adjunto no está descargado en este equipo. Ábrelo o descárgalo primero desde Zotero.",
        );
      if (!studyMaterials.supportsStudyMaterial(filePath))
        throw new Error(
          `Formato no compatible: .${path.extname(filePath).replace(/^\./, "") || "?"}`,
        );
      const result = await studyMaterials.importStudyMaterialFromZotero(
        filePath,
        input.library,
        item,
        attachment,
  ...
  h("study:materials:openZotero", async (_e, id: string) => {
    const material = studyMaterials.getStudyMaterial(id);
    if (
      !material.zoteroItemKey ||
      !material.zoteroLibraryType ||
      !material.zoteroLibraryId
    )
      throw new Error("Este material no conserva un enlace con Zotero.");
    const key =
      material.zoteroLibraryType === "group"
  ...
  h("study:materials:replace", async (_e, id: string, ocr?: boolean) => {
    const picked = await showImportOpenDialog(getWindow() ?? undefined!, {
      title: "Sustituir fichero del material",
      properties: ["openFile"],
      filters: [
        {
          name: "Materiales de estudio",
    ...
    if (picked.canceled || !picked.filePaths[0]) return null;
    const updated = await studyMaterials.replaceStudyMaterialFile(
      id,
      picked.filePaths[0],
      Boolean(ocr),
    );
  ...
  h(
    ...
    async (_e, id: string, patch: StudyMaterialUpdateInput) => {
      const updated = studyMaterials.updateStudyMaterial(id, patch);
      if (
        patch.title !== undefined ||
        patch.description !== undefined ||
        patch.metadata !== undefined ||
        patch.bibliography !== undefined
      ) {
        queueStudyMaterialIndex([id]);
        queueStudyKnowledgeSources("material", [id]);
      }
      return updated;
    },
  );
  h("study:materials:reindex", async (_e, id: string) =>
    reindexStudyMaterial(id),
  );
  h(
    "study:materials:version:restore",
    async (_e, id: string, versionId: string) => {
      const restored = studyMaterials.restoreStudyMaterialVersion(
        id,
        versionId,
      );
      queueStudyMaterialIndex([id]);
  ...
  h(
    "study:materials:placement:add",
    async (_e, id: string, input: StudyMaterialImportInput) => {
      const result = studyMaterials.addStudyMaterialPlacement(id, input);
      studyKnowledgeRepo.syncStudyKnowledgeSourceScopes("material", id);
      queueStudyKnowledgeSources("material", [id]);
      studySearch.queueStudySearchIndexRefresh();
      return result;
    },
  ...
  h(
    "study:materials:placement:setPrimary",
    async (_e, id: string, input: StudyMaterialImportInput) => {
      const result = studyMaterials.setPrimaryStudyMaterialPlacement(id, input);
      studyKnowledgeRepo.syncStudyKnowledgeSourceScopes("material", id);
      queueStudyKnowledgeSources("material", [id]);
      studySearch.queueStudySearchIndexRefresh();
      return result;
  ...
  h(
    "study:materials:placement:remove",
    async (_e, id: string, placementId: string) => {
      const result = studyMaterials.removeStudyMaterialPlacement(
        id,
        placementId,
      );
      studyKnowledgeRepo.syncStudyKnowledgeSourceScopes("material", id);
  ...
  h(
    "study:materials:annotation:create",
    async (_e, materialId: string, input: StudyMaterialAnnotationInput) =>
      studyMaterials.createStudyMaterialAnnotation(materialId, input),
  );
  h(
    "study:materials:annotation:update",
    async (_e, id: string, patch: Partial<StudyMaterialAnnotationInput>) =>
      studyMaterials.updateStudyMaterialAnnotation(id, patch),
  );
  h("study:materials:annotation:delete", async (_e, id: string) =>
    studyMaterials.deleteStudyMaterialAnnotation(id),
  );
  h("study:materials:annotation:export", async (_e, id: string) => {
    ...
    if (!isPdf && !isEpub)
      throw new Error(
        "La exportación anotada solo está disponible para PDF y EPUB.",
      );
    const extension = isPdf ? "pdf" : "epub";
    const baseName =
      path
        .basename(material.fileName, path.extname(material.fileName))
        .replace(/[\\/:*?"<>|]+/g, "-") || "material";
    const picked = await dialog.showSaveDialog(getWindow() ?? undefined!, {
      title: "Descargar material anotado",
      defaultPath: `${baseName}-anotado.${extension}`,
      filters: [
        {
          name: isPdf ? "PDF anotado" : "EPUB anotado",
    ...
    if (picked.canceled || !picked.filePath) return null;
    const bytes = isPdf
      ? await annotatedPdfBytes(content, material)
      : annotatedEpubBytes(content, material);
    fs.writeFileSync(picked.filePath, Buffer.from(bytes));
    return { path: picked.filePath };
  });
  h(
    "study:materials:note:create",
    async (
      _e,
      materialId: string,
      ...
      const result = studyMaterials.createStudyNoteFromMaterial(
        materialId,
        annotationId,
        title,
      );
      queueStudyKnowledgeSources("document", [result.documentId]);
      studySearch.queueStudySearchIndexRefresh();
      return result;
    },
  );
  h(
    "study:materials:lifecycle",
    async (
      _e,
      id: string,
      action: "archive" | "restore" | "trash" | "recover" | "delete",
      options?: { purgeLinkedKnowledge?: boolean },
    ) => {
      const destructive = action === "trash" || action === "delete";
      const purgeLinkedKnowledge =
        destructive && options?.purgeLinkedKnowledge !== false;
      if (purgeLinkedKnowledge)
        studyKnowledgeRepo.purgeStudyKnowledgeSource("material", id);
      const result = studyMaterials.setStudyMaterialLifecycle(id, action);
      if (action !== "delete" && (!destructive || purgeLinkedKnowledge))
        studyKnowledgeRepo.syncStudyKnowledgeSourceScopes("material", id);
      if (action === "restore" || action === "recover")
        queueStudyKnowledgeSources("material", [id]);
      studySearch.queueStudySearchIndexRefresh();
  ...
  h("study:recordings:list", async (_e, options?: StudyRecordingListOptions) =>
    studyRecordings.listStudyRecordings(options),
  );
  h("study:recordings:get", async (_e, id: string) =>
    studyRecordings.getStudyRecording(id),
  );
  h("study:recordings:content", async (_e, id: string) =>
    studyRecordings.getStudyRecordingContent(id),
  );

  h("study:recordings:create", async (_e, input: StudyRecordingCreateInput) =>
    studyRecordings.createStudyRecording(input),
  );
  h(
    "study:recordings:import",
    async (
      _e,
      scope?: Omit<
        StudyRecordingCreateInput,
      ...
      const picked = await showImportOpenDialog(getWindow() ?? undefined!, {
        title: "Añadir grabaciones de clase",
        properties: ["openFile", "multiSelections"],
        filters: [
          {
            name: "Grabaciones de audio",
      ...
      return picked.filePaths.map((filePath) =>
        studyRecordings.importStudyRecordingFile(filePath, scope),
      );
    },
  );
  h(
    "study:recordings:update",
    async (_e, id: string, patch: StudyRecordingUpdateInput) =>
      studyRecordings.updateStudyRecording(id, patch),
  );
  h(
    "study:recordings:marker:create",
    async (_e, recordingId: string, input: StudyAudioMarkerInput) =>
      studyRecordings.createStudyAudioMarker(recordingId, input),
  );
  h(
    "study:recordings:marker:update",
    async (_e, id: string, patch: Partial<StudyAudioMarkerInput>) =>
      studyRecordings.updateStudyAudioMarker(id, patch),
  );
  h("study:recordings:marker:delete", async (_e, id: string) =>
    studyRecordings.deleteStudyAudioMarker(id),
  );
  h(
    "study:recordings:transcript:save",
    async (_e, recordingId: string, input: StudyTranscriptInput) =>
      studyRecordings.saveStudyTranscript(recordingId, input),
  );
  h(
    "study:recordings:transcript:update",
    async (
      _e,
      id: string,
      contentMarkdown: string,
      segments?: StudyTranscriptSegmentInput[],
    ) => studyRecordings.updateStudyTranscript(id, contentMarkdown, segments),
  );
  h("study:recordings:diarize", async (_e, request: StudyDiarizationRequest) =>
    diarizeStudyRecording(request),
  );
  h(
    "study:recordings:segment:update",
    async (_e, id: string, patch: Partial<StudyTranscriptSegmentInput>) =>
      studyRecordings.updateStudyTranscriptSegment(id, patch),
  );
  h("study:recordings:transcript:delete", async (_e, id: string) =>
    studyRecordings.deleteStudyTranscript(id),
  );
  h(
    "study:recordings:note:create",
    async (
      _e,
      recordingId: string,
      transcriptId: string,
      placements?: StudyPlacementInput[],
    ) => {
      const result = studyRecordings.createStudyNoteFromTranscript(
        recordingId,
        transcriptId,
        placements,
      );
  ...
  h("study:recordings:audio:delete", async (_e, id: string) =>
    studyRecordings.deleteStudyRecordingAudio(id),
  );
  h(
    "study:recordings:lifecycle",
    async (
      _e,
      id: string,
      action: "archive" | "restore" | "trash" | "recover" | "delete",
    ) => studyRecordings.setStudyRecordingLifecycle(id, action),
  );
  h(
    "study:search:query",
    async (_e, query: string, options?: StudySearchOptions) =>
      studySearch.searchStudyCorpus(query, options),
  );
  h("study:search:status", async () => studySearch.getStudySearchIndexStatus());
  h("study:search:rebuild", async (e) => {
    const off = studySearch.onStudySearchProgress((next) => {
      if (!e.sender.isDestroyed()) e.sender.send("study:search:progress", next);
    });
    try {
      return await studySearch.rebuildStudySearchIndex();
    } finally {
      off();
    }
  });
  ...
  h("study:search:deleteIndex", async () =>
    studySearch.deleteStudySearchIndex(),
  );
  h("study:search:exclude", async (_e, sourceId: string, excluded: boolean) =>
    studySearch.setStudySearchSourceExcluded(sourceId, excluded),
  );
  h("study:search:saved:list", async () =>
    studySearch.listStudySavedSearches(),
  );
  h(
    "study:search:saved:create",
    async (_e, name: string, query: string, options: StudySearchOptions) =>
      studySearch.saveStudySearch(name, query, options),
  );
  h("study:search:saved:delete", async (_e, id: string) =>
    studySearch.deleteStudySavedSearch(id),
  );
  h("study:search:history:list", async () =>
    studySearch.listStudySearchHistory(),
  );
  h("study:search:history:clear", async () =>
    studySearch.clearStudySearchHistory(),
  );
  h("study:knowledge:ideas", async (_e, subjectId: string, query?: string) =>
    studyKnowledgeRepo.listStudyIdeas(subjectId, query),
  );
  h("study:knowledge:idea", async (_e, id: string) =>
    studyKnowledgeRepo.getStudyIdeaDetail(id),
  );
  h("study:knowledge:idea:delete", async (_e, id: string) =>
    studyKnowledgeRepo.deleteStudyIdea(id),
  );
  h("study:knowledge:graph", async (_e, subjectId: string) =>
    studyKnowledgeRepo.getStudyKnowledgeGraph(subjectId),
  );
  h("study:knowledge:jobs", async (_e, subjectId?: string) =>
    studyKnowledgeRepo.listStudyKnowledgeJobs(subjectId),
  );
  h("study:knowledge:progress", async () => getStudyKnowledgeProgress());
  h(
    "study:knowledge:reanalyze",
    async (_e, sourceKind: "material" | "document", sourceId: string) =>
      reanalyzeStudyKnowledgeSource(sourceKind, sourceId),
  );
  h("study:assistant:sources", async () =>
    studyAssistant.getStudyAssistantSources(),
  );
  h("study:assistant:list", async (_e, includeArchived?: boolean) =>
    studyAssistant.listStudyAssistantConversations(Boolean(includeArchived)),
  );
  h("study:assistant:get", async (_e, id: string) =>
    studyAssistant.getStudyAssistantConversation(id),
  );
  h(
    "study:assistant:create",
    async (_e, input?: StudyAssistantConversationInput) =>
      studyAssistant.createStudyAssistantConversation(input),
  );
  h(
    "study:assistant:update",
    async (_e, id: string, patch: StudyAssistantConversationPatch) =>
      studyAssistant.updateStudyAssistantConversation(id, patch),
  );
  h("study:assistant:delete", async (_e, id: string) =>
    studyAssistant.deleteStudyAssistantConversation(id),
  );
  h(
    "study:assistant:stream",
    async (e, requestId: string, request: StudyAssistantRequest) => {
      const controller = new AbortController();
      studyAssistantAborters.set(requestId, controller);
      try {
        return await studyAssistant.streamStudyAssistant(
          request,
          (delta, kind) => {
            if (!e.sender.isDestroyed())
              e.sender.send(
                kind === "reasoning"
                  ? "study:assistant:reasoning"
                  : "study:assistant:delta",
                requestId,
                delta,
              );
          },
        ...
        studyAssistantAborters.delete(requestId);
      }
    },
  );
  h("study:assistant:cancel", async (_e, requestId: string) =>
    studyAssistantAborters.get(requestId)?.abort(),
  );
  h("study:assistant:export", async (_e, id: string) => {
    const conversation = studyAssistant.getStudyAssistantConversation(id);
    if (!conversation) return null;
    const safeTitle =
      conversation.title.replace(/[\\/:*?"<>|]+/g, "-").slice(0, 80) ||
      "chat-estudio";
    const picked = await dialog.showSaveDialog(getWindow() ?? undefined!, {
      title: "Exportar conversación de estudio",
    ...
    fs.writeFileSync(
      picked.filePath,
      studyAssistant.renderStudyAssistantConversation(conversation),
      "utf8",
    );
    return { path: picked.filePath };
  });
  h("study:questions:list", async (_e, filters?: StudyQuestionFilters) =>
    studyQuestions.listStudyQuestions(filters),
  );
  h("study:questions:get", async (_e, id: string) =>
    studyQuestions.getStudyQuestion(id),
  );
  h("study:questions:create", async (_e, input: StudyQuestionInput) =>
    studyQuestions.createStudyQuestion(input),
  );
  h(
    "study:questions:update",
    async (_e, id: string, patch: Partial<StudyQuestionInput>) =>
      studyQuestions.updateStudyQuestion(id, patch),
  );
  h("study:questions:duplicate", async (_e, id: string) =>
    studyQuestions.duplicateStudyQuestion(id),
  );
  h("study:questions:versions", async (_e, id: string) =>
    studyQuestions.listStudyQuestionVersions(id),
  );
  h("study:questions:restore", async (_e, id: string, versionId: string) =>
    studyQuestions.restoreStudyQuestionVersion(id, versionId),
  );
  h(
    "study:questions:lifecycle",
    async (
      _e,
      id: string,
      action: "archive" | "restore" | "trash" | "recover" | "delete",
    ) => studyQuestions.setStudyQuestionLifecycle(id, action),
  );
  h(
    "study:questions:generate",
    async (_e, request: StudyQuestionGenerationRequest) =>
      generateStudyQuestions(request),
  );
  h("study:questions:export", async (_e, ids?: string[]) => {
    const payload = studyQuestions.exportStudyQuestions(ids);
    const picked = await dialog.showSaveDialog(getWindow() ?? undefined!, {
      title: "Exportar banco de preguntas",
      defaultPath: "nodus-preguntas.json",
      filters: [{ name: "Nodus Study Questions", extensions: ["json"] }],
    });
    if (picked.canceled || !picked.filePath) return null;
    fs.writeFileSync(picked.filePath, JSON.stringify(payload, null, 2), "utf8");
    return { path: picked.filePath };
  ...
  h("study:questions:import", async () => {
    ...
    if (picked.canceled || !picked.filePaths[0]) return [];
    return studyQuestions.importStudyQuestions(
      JSON.parse(
        fs.readFileSync(picked.filePaths[0], "utf8"),
      ) as StudyQuestionExport,
    );
  });
  h("study:questions:collections:list", async () =>
    studyQuestions.listStudyQuestionCollections(),
  );
  h(
    "study:questions:collections:create",
    async (_e, name: string, description?: string) =>
      studyQuestions.createStudyQuestionCollection(name, description),
  );
  h(
    "study:questions:collections:setItems",
    async (_e, collectionId: string, questionIds: string[]) =>
      studyQuestions.setStudyQuestionCollectionItems(collectionId, questionIds),
  );
  h("study:questions:collections:delete", async (_e, id: string) =>
    studyQuestions.deleteStudyQuestionCollection(id),
  );
  h("study:questions:analytics", async (_e, id: string) =>
    studyQuestions.getStudyQuestionAnalytics(id),
  );
  h("study:questions:similar", async (_e, id: string, threshold?: number) =>
    studyQuestions.findSimilarStudyQuestions(id, threshold),
  );
  h(
    "study:assessments:list",
    async (_e, kind?: "test" | "exam", includeArchived?: boolean) =>
      studyAssessments.listStudyAssessments(kind, includeArchived),
  );
  h("study:assessments:get", async (_e, id: string) =>
    studyAssessments.getStudyAssessment(id),
  );
  h("study:assessments:create", async (_e, input: StudyAssessmentInput) =>
    studyAssessments.createStudyAssessment(input),
  );
  h("study:assessments:buildTest", async (_e, input: StudyTestBuildRequest) =>
    buildStudyTest(input),
  );
  h(
    "study:assessments:update",
    async (
      _e,
      id: string,
      patch: Partial<Omit<StudyAssessmentInput, "questionIds">> & {
        archived?: boolean;
      },
    ) => studyAssessments.updateStudyAssessment(id, patch),
  );
  h("study:assessments:delete", async (_e, id: string) =>
    studyAssessments.deleteStudyAssessment(id),
  );
  h("study:attempts:list", async (_e, assessmentId?: string) =>
    studyAssessments.listStudyAttempts(assessmentId),
  );
  h("study:attempts:get", async (_e, id: string) =>
    studyAssessments.getStudyAttempt(id),
  );
  h("study:attempts:start", async (_e, input: StudyAttemptStartInput) =>
    studyAssessments.startStudyAttempt(input),
  );
  h(
    "study:attempts:answer",
    async (_e, id: string, input: StudyAttemptAnswerInput) =>
      studyAssessments.saveStudyAttemptAnswer(id, input),
  );
  h("study:attempts:submit", async (_e, id: string, expired?: boolean) =>
    studyAssessments.submitStudyAttempt(id, expired),
  );
  h("study:attempts:abandon", async (_e, id: string) =>
    studyAssessments.abandonStudyAttempt(id),
  );
  h(
    "study:assessments:export",
    async (_e, id: string, includeAnswers?: boolean) => {
      const assessment = studyAssessments.getStudyAssessment(id);
      if (!assessment) return null;
      const picked = await dialog.showSaveDialog(getWindow() ?? undefined!, {
        title: "Exportar test de estudio",
        defaultPath: `${assessment.title.replace(/[\\/:*?"<>|]+/g, "-").slice(0, 80) || "test"}.md`,
        filters: [{ name: "Markdown", extensions: ["md"] }],
      });
      if (picked.canceled || !picked.filePath) return null;
      fs.writeFileSync(
        picked.filePath,
        studyAssessments.renderStudyAssessmentMarkdown(
          assessment,
  ...
  h("study:grading:rubrics:list", async (_e, includeArchived?: boolean) =>
    studyGrading.listStudyRubrics(includeArchived),
  );
  h("study:grading:rubrics:create", async (_e, input: StudyRubricInput) =>
    studyGrading.createStudyRubric(input),
  );
  h(
    "study:grading:rubrics:update",
    async (
      _e,
      id: string,
      patch: Partial<StudyRubricInput> & { archived?: boolean },
    ) => studyGrading.updateStudyRubric(id, patch),
  );
  h("study:grading:rubrics:duplicate", async (_e, id: string) =>
    studyGrading.duplicateStudyRubric(id),
  );
  h("study:grading:rubrics:delete", async (_e, id: string) =>
    studyGrading.deleteStudyRubric(id),
  );
  h("study:grading:runs:list", async (_e, attemptAnswerId?: string) =>
    studyGrading.listStudyGradingRuns(attemptAnswerId),
  );
  h(
    "study:grading:manual",
    async (_e, id: string, score: number, comment?: string) =>
      studyGrading.setStudyGradingManualScore(id, score, comment),
  );
  h("study:flashcards:list", async (_e, options) =>
    studyLearning.listStudyFlashcards(options),
  );
  h("study:flashcards:create", async (_e, input) =>
    studyLearning.createStudyFlashcard(input),
  );
  h("study:flashcards:update", async (_e, id: string, patch) =>
    studyLearning.updateStudyFlashcard(id, patch),
  );
  h("study:flashcards:fromQuestions", async (_e, ids: string[]) =>
    studyLearning.createStudyFlashcardsFromQuestions(ids),
  );
  h("study:flashcards:review", async (_e, input) =>
    studyLearning.reviewStudyFlashcard(input),
  );
  h("study:flashcards:state", async (_e, id: string, action) =>
    studyLearning.setStudyFlashcardState(id, action),
  );
  h("study:learning:progress", async () =>
    studyLearning.getStudyProgressDashboard(),
  );
  h("study:planner:get", async () => studyLearning.getStudyPlanner());
  h("study:planner:create", async (_e, input) =>
    studyLearning.createStudyPlan(input),
  );
  h("study:planner:block:create", async (_e, input) =>
    studyLearning.createStudyPlanBlock(input),
  );
  h("study:planner:event:create", async (_e, input) =>
    studyLearning.createStudyCalendarEvent(input),
  );
  h("study:planner:event:update", async (_e, id: string, input) =>
    studyLearning.updateStudyCalendarEvent(id, input),
  );
  h("study:planner:event:delete", async (_e, id: string) =>
    studyLearning.deleteStudyCalendarEvent(id),
  );
  h(
    "study:planner:event:external",
    async (_e, id: string, target: "google" | "icloud") => {
      const event = studyLearning
        .getStudyPlanner()
      ...
      fs.writeFileSync(
        filePath,
        studyLearning.renderStudyCalendarEventIcs(id),
        "utf8",
      );
  ...
  h("study:planner:goal:create", async (_e, input) =>
    studyLearning.createStudyGoal(input),
  );
  h("study:planner:item:update", async (_e, kind, id: string, patch) =>
    studyLearning.updateStudyPlannerItem(kind, id, patch),
  );
  h("study:planner:session:start", async (_e, input) =>
    studyLearning.startStudySession(input),
  );
  h("study:planner:session:finish", async (_e, id: string, input) =>
    studyLearning.finishStudySession(id, input),
  );
  h("study:planner:exportIcs", async () => {
    const picked = await dialog.showSaveDialog(getWindow() ?? undefined!, {
      title: "Exportar calendario de estudio",
      defaultPath: "nodus-estudio.ics",
      filters: [{ name: "iCalendar", extensions: ["ics"] }],
    });
    if (picked.canceled || !picked.filePath) return null;
    fs.writeFileSync(
      picked.filePath,
      studyLearning.renderStudyPlannerIcs(),
      "utf8",
    );
    return { path: picked.filePath };
  });
  h("study:ai:usage:list", async (_e, limit?: number) =>
    studyAiUsage.listStudyAiUsage(limit),
  );
  h("study:ai:usage:summary", async () =>
    studyAiUsage.getStudyAiUsageSummary(),
  );
  h("study:ai:usage:clear", async () => studyAiUsage.clearStudyAiUsage());

  h("study:plan", async (_e, request?: StudyPlanRequest) =>
    buildStudyPlan(request ?? {}),
  );
  h(
    "study:progress:set",
    async (
  ...
  h("study:session", async (_e, request: StudySessionRequest) =>
    generateStudySession(request),
  );
  // inmersión (guided topic mastery: scope → generate → resume/replay forever)
  h("immersion:scope", async (_e, request: ImmersionScopeRequest) =>
    buildImmersionScope(request),
  );
  h(
    "immersion:generate",
    async (e, requestId: string, request: ImmersionRequest) => {
      ...
      const image = applyDecorativeImageOption(
        "immersion",
        session.id,
        request.decorativeImage,
        (next) => {
          if (!e.sender.isDestroyed())
            e.sender.send("images:changed", localizedForUi(next));
        },
      );
  ...
  h("immersion:get", async (_e, id: string) =>
    immersionRepo.getImmersionSession(id),
  );
  h("immersion:restart", async (_e, id: string) =>
    immersionRepo.restartImmersionSession(id),
  );
  h(
    "immersion:progress:set",
    async (_e, id: string, progress: ImmersionProgress) =>
      immersionRepo.setImmersionProgress(id, progress),
  );
  h("immersion:answer", async (_e, request: ImmersionAnswerRequest) =>
    evaluateImmersionAnswer(request),
  );
  ...
  h(
    "themes:reprocess",
    async (e, options: ReprocessConnectionsOptions, model?: ModelRef | null) =>
  ...
  h("gaps:listPage", async (_e, offset: number, limit: number) =>
    aggregateGapsPage(offset, limit),
  );
  ...
  h("reading:path", async (_e, request?: ReadingPathRequest) =>
    buildReadingPath(request),
  );

  // debates (contradiction face-offs)
  h("debates:list", async () => getDebates());
  h(
    "debates:analyzeStream",
    async (e, requestId: string, request: DebateAnalysisRequest) =>
      streamDebateAnalysis(request, (delta, kind) => {
        const channel =
          kind === "reasoning"
            ? "debates:analyzeStream:reasoning"
            : "debates:analyzeStream:delta";
  ...
  h("research:rq:get", async (_e, id: string) =>
    rqRepo.getResearchQuestionDetail(id),
  );
  h(
    "research:rq:create",
    async (_e, input: { question: string; notes?: string }) =>
      rqRepo.createResearchQuestion(input.question, input.notes),
  );
  h("research:rq:decompose", async (_e, request: RqDecomposeRequest) =>
    decomposeQuestion(request),
  );
  h(
    "research:rq:updateSubs",
    async (_e, request: RqUpdateSubQuestionsRequest) => {
  ...
  h("research:rq:map", async (e, requestId: string, request: RqMapRequest) =>
    mapCoverage(request, (p) =>
      e.sender.send("research:rq:map:progress", requestId, p),
    ),
  );
  h("research:rq:delete", async (_e, id: string) => {
    rqRepo.deleteResearchQuestion(id);
  });
  h("research:rq:export", async (_e, request: RqExportRequest) =>
    exportResearchCoverage(request),
  );

  // hypothesis lab
  h("hypothesis:generate", async (_e, request: HypothesisLabRequest) =>
    generateHypothesisLab(request),
  );

  // research assistant
  h("research:chat", async (_e, request: ResearchChatRequest) =>
    answerResearchChat(request),
  );
  h(
    "research:chatStream",
    async (e, requestId: string, request: ResearchChatRequest) => {
      ...
      try {
        return await streamResearchChat(
          ...
          (delta, kind) => {
            const channel =
              kind === "reasoning"
                ? "research:chatStream:reasoning"
                : "research:chatStream:delta";
            e.sender.send(channel, requestId, delta);
          },
          controller.signal,
        );
  ...
  // writing workshop
  h("writing:snapshot", async (_e, brief: WritingWorkshopBrief) =>
    buildWritingWorkshopSnapshot(brief),
  );
  h("writing:draft", async (_e, request: WritingWorkshopDraftRequest) =>
    generateWritingWorkshopDraft(request),
  );
  h("writing:export", async (_e, request: WritingWorkshopExportRequest) =>
    exportWritingWorkshopDraft(request),
  );
  h(
    "writing:exportZip",
    async (e, requestId: string, request: DeepResearchArchiveRequest) =>
      exportDeepResearchArchive(request, (done, total, title) => {
        if (!e.sender.isDestroyed())
          e.sender.send(
            "writing:exportZip:progress",
            requestId,
            done,
            total,
            title,
  ...
  h("writing:saved:list", async () =>
    writingDrafts.listWritingWorkshopDrafts(),
  );
  h(
    "writing:saved:save",
    async (e, request: WritingWorkshopSaveDraftRequest) => {
  ...
  h("writing:annotations:list", async (_e, draftId: string) =>
    writingAnnotations.listWritingDraftAnnotations(draftId),
  );
  h(
    "writing:annotations:create",
    async (_e, input: WritingDraftAnnotationInput) => {
  ...
  h(
    "writing:annotations:updateComment",
    async (_e, id: string, comment: string) => {
      const annotation = writingAnnotations.updateWritingDraftComment(
        id,
        comment,
      );
  ...
  h(
    "research:deep",
    async (e, requestId: string, request: DeepResearchRequest) => {
      ensureDeepResearchLane();
      return runDeepResearchJob(
        { request, origin: "app", save: false },
        (p) => {
          if (!e.sender.isDestroyed())
            e.sender.send("research:deep:progress", requestId, p);
        },
      );
    },
  );
  ...
  h("tutor:plan", async (_e, request: TutorPlanRequest) =>
    buildTutorPlan(request),
  );
  ...
  h("tutor:routes:rate", async (_e, routeId: string, rating: number | null) =>
    tutorRoutes.rateTutorRoute(routeId, rating),
  );
  h("tutor:routes:played", async (_e, routeId: string) =>
    tutorRoutes.markTutorRoutePlayed(routeId),
  );
  h("tutor:routes:delete", async (_e, routeId: string) =>
    tutorRoutes.deleteTutorRoute(routeId),
  );
  h("tutor:step", async (_e, request: TutorStepRequest) =>
    answerTutorStep(request),
  );
  h(
    "tutor:stepStream",
    async (e, requestId: string, request: TutorStepRequest) =>
      streamTutorStep(request, (delta, kind) => {
        const channel =
          kind === "reasoning"
            ? "tutor:stepStream:reasoning"
            : "tutor:stepStream:delta";
        e.sender.send(channel, requestId, delta);
      }),
  );
  ...
  h("chat:list", async (_e, includeArchived?: boolean) =>
    chat.listConversations(includeArchived ?? false),
  );
  ...
  h("chat:rename", async (_e, id: string, title: string) =>
    chat.renameConversation(id, title),
  );
  h("chat:archive", async (_e, id: string, archived: boolean) =>
    chat.setArchived(id, archived),
  );
  ...
  h("notes:tree", async (_e, includeTrashed?: boolean) =>
    notes.getNotesTree(includeTrashed ?? false),
  );
  h("notes:folders:create", async (_e, input: CreateNoteFolderInput) =>
    notes.createNoteFolder(input),
  );
  h("notes:folders:rename", async (_e, id: string, name: string) =>
    notes.renameNoteFolder(id, name),
  );
  h("notes:folders:move", async (_e, id: string, parentId: string | null) =>
    notes.moveNoteFolder(id, parentId ?? null),
  );
  ...
  h("notes:create", async (_e, input: CreateNoteInput) =>
    notes.createNote(input),
  );
  h("notes:get", async (_e, id: string) => notes.getNote(id));
  h("notes:update", async (_e, input: UpdateNoteInput) =>
    notes.updateNote(input),
  );
  h("notes:move", async (_e, id: string, folderId: string | null) =>
    notes.moveNote(id, folderId ?? null),
  );
  h("notes:tags:patch", async (_e, noteIds: string[], patch: NoteTagPatch) =>
    notes.patchNoteTags(noteIds, patch),
  );
  h("notes:trash", async (_e, noteIds: string[]) => {
    notes.trashNotes(noteIds);
  });
  h("notes:restore", async (_e, noteIds: string[]) => {
    notes.restoreNotes(noteIds);
  });
  ...
  h(
    "manualIdeas:create",
    async (_e, input: { folderId: string | null; title?: string }) =>
  ...
  h(
    "manualIdeas:searchCandidates",
    async (_e, query: string, excludeIds?: string[], limit?: number) =>
      manualIdeas.searchIdeaCandidates(query, excludeIds ?? [], limit ?? 20),
  );

  // notes export + reordering
  h("notes:export", async (_e, options: NotesExportOptions) =>
    exportNotes(options),
  );
  h("notes:reorder", async (_e, noteIds: string[]) => {
    notes.reorderNotes(noteIds);
  });
  h("notes:reorderByAI", async (_e, noteIds: string[]) =>
    reorderNotesByAI(noteIds),
  );
  h("notes:folders:updateSummary", async (_e, id: string, summary: string) =>
    notes.updateNoteFolderSummary(id, summary ?? ""),
  );
  h("notes:folders:suggestIdeas", async (_e, folderId: string) =>
    suggestFolderIdeas(folderId),
  );

  // workspace — el editor completo sobre una nota, y sus enlaces con la biblioteca
  h("workspace:editor:data", async (_e, noteId: string) =>
    workspace.getWorkspaceNoteEditorData(noteId),
  );
  h(
    "workspace:editor:update",
    async (_e, noteId: string, input: StudyDocUpdateInput) =>
  ...
  h(
    ...
    async (_e, noteId: string, input: StudyAnnotationInput) =>
      workspace.createWorkspaceAnnotation(noteId, input),
  );
  h(
    "workspace:annotation:update",
    async (
      _e,
      id: string,
      patch: Partial<StudyAnnotationInput> & { resolved?: boolean },
  ...
  h(
    ...
    async (_e, ownerKind: WorkspaceLinkOwnerKind, ownerId: string) =>
  ...
  h("workspace:library:add", async (_e, input: WorkspaceLibraryLinkInput) =>
    workspace.addWorkspaceLibraryLink(input),
  );
  h(
    ...
    async (
      ...
      workspace.removeWorkspaceLibraryLink(
        ownerKind,
        ownerId,
        libraryItemId,
        scope ?? "global",
  ...
  h("citations:verify", async (_e, refs: CitationRef[]) =>
    verifyCitations(refs ?? []),
  );
  h("citations:preview", async (_e, ref: CitationRef) =>
    ref ? previewCitation(ref) : null,
  );
  h("search:global", async (_e, query: string, limitPerKind?: number) =>
    globalSearch(query ?? "", limitPerKind ?? 8),
  );
  h("search:detail", async (_e, kind: SearchResultKind, id: string) =>
    getSearchResultDetail(kind, id),
  );
  h(
    "search:semantic",
    async (_e, query: string, options?: SemanticSearchOptions) =>
  ...
  h("search:saved:create", async (_e, input: SaveSearchInput) =>
    saveSearch(input),
  );
  ...
  h(
    "gaps:suggestSearch",
    async (_e, statement: string, workTitles?: string[]) =>
  ...
  h("projects:create", async (_e, input: CreateProjectInput) =>
    projects.createProject(input),
  );
  h("projects:update", async (_e, input: UpdateProjectInput) =>
    projects.updateProject(input),
  );
  h("projects:delete", async (_e, id: string) => {
    projects.deleteProject(id);
  });
  h("projects:sections:update", async (_e, input: UpdateProjectSectionInput) =>
    projects.updateSection(input),
  );
  h("projects:links:add", async (_e, input: AddProjectLinkInput) =>
    projects.addLink(input),
  );
  h("projects:links:delete", async (_e, id: string) => {
    projects.deleteLink(id);
  });
  h(
    "projects:chapters:import",
    async (_e, input: ImportProjectChapterInput) => {
          ...
          filters: [
            {
              ...
              extensions: ["docx", "pdf", "epub", "md", "markdown", "txt"],
            },
      ...
      if (!doc.text.trim())
        throw new Error("No se pudo extraer texto útil del capítulo.");
  ...
  h(
    "projects:chapters:update",
    async (_e, chapterId: string, markdown: string) =>
      projects.updateChapterMarkdown(chapterId, markdown, {
        versionLabel: "Antes de guardar edicion manual",
      }),
  );
  h("projects:suggestions:list", async (_e, chapterId: string) =>
    projects.listSuggestions(chapterId),
  );
  h(
    "projects:suggestions:generate",
    async (_e, request: GenerateProjectSuggestionsRequest) =>
      generateProjectSuggestions(request),
  );
  h(
    "projects:suggestions:updateStatus",
    async (_e, id: string, status: ChapterSuggestionStatus) =>
      projects.updateSuggestionStatus(id, status),
  );
  h(
    "projects:suggestions:apply",
    async (_e, request: ApplyProjectSuggestionsRequest) =>
      projects.applySuggestions(request.chapterId, request.suggestionIds),
  );
  h("projects:versions:list", async (_e, chapterId: string) =>
    projects.listChapterVersions(chapterId),
  );
  h("projects:versions:restore", async (_e, versionId: string) =>
    projects.restoreChapterVersion(versionId),
  );
  h("projects:chapterRelations:get", async (_e, chapterId: string) =>
    getChapterRelations(chapterId),
  );
  h(
    "projects:chapterRelations:analyze",
    async (_e, request: AnalyzeChapterRelationsRequest) =>
      analyzeChapterRelations(request),
  );
  h(
    "projects:manuscript:verify",
    async (_e, request: ManuscriptVerificationRequest) =>
      verifyManuscriptCitations(request),
  );
  h(
    "projects:manuscript:applyCitation",
    async (_e, request: ApplyManuscriptCitationRequest) =>
      applyManuscriptCitation(request),
  );
  h("projects:export", async (_e, request: ExportProjectRequest) =>
    exportProject(request),
  );
  h(
    "projects:chapters:export",
    async (_e, request: ExportProjectChapterRequest) =>
      exportProjectChapter(request),
  );

  // embedding pipeline
  h("embeddings:start", async (_e, nodusIds?: string[]) =>
    startEmbedding(nodusIds),
  );
  ...
  h("embeddings:workStatuses", async (_e, nodusIds?: string[]) =>
    getWorkEmbeddingStatuses(nodusIds),
  );

  // Full-text passage index
  h("passages:start", async (_e, nodusIds?: string[]) =>
    startPassageEmbedding(nodusIds),
  );
  ...
  h("passages:workStatuses", async (_e, nodusIds?: string[]) =>
    getWorkPassageStatuses(nodusIds),
  );
  h("passages:get", async (_e, passageId: string) =>
    getPassageDetail(passageId),
  );

  // semantic bridge discovery
  h("bridges:discover", async (_e, model?: ModelRef | null) =>
    discoverSemanticBridges(model),
  );
  ...
  h("data:exportSync", async () => {
    if (getActiveVault().type === "estudio" && !getSettings().studySyncEnabled)
      throw new Error(
        "La sincronización del vault de estudio está desactivada en Ajustes.",
      );
    const { canceled, filePath } = await dialog.showSaveDialog({
      title: "Exportar paquete de sincronización",
      defaultPath: path.join(
        app.getPath("documents"),
        `nodus-sync-${new Date().toISOString().slice(0, 10)}.nodussync`,
      ),
    ...
    if (!passphrase) {
      throw new Error(
        "Configura una frase de sincronización en Ajustes: los paquetes van cifrados y la necesitarás en el otro equipo.",
      );
    }
  ...
  h("data:importSync", async (_e, passphrase?: string) => {
    ...
    return mergeSyncPackage(
      fs.readFileSync(filePaths[0]),
      passphrase?.trim() || getSyncPassphrase() || undefined,
    );
  });
  h("study:data:overview", async () => studyDataAdmin.getStudyDataOverview());
  h("study:data:exportScope", async (_e, scope, format) => {
    if (!getSettings().studySharingEnabled)
      throw new Error(
        "La exportación para compartir está desactivada en Ajustes.",
      );
  ...
  h(
    ...
    async (
      _e,
      action: "rebuild-indexes" | "clear-embeddings" | "empty-trash" | "repair",
    ) => {
      if (action === "rebuild-indexes")
        return studyDataAdmin.rebuildStudyIndexes();
      if (action === "clear-embeddings") {
        studySearch.deleteStudySearchIndex();
        return studyDataAdmin.clearStudyEmbeddingCache();
      }
  ...
  h("study:data:diagnostic", async () => {
    const picked = await dialog.showSaveDialog(getWindow() ?? undefined!, {
      title: "Exportar diagnóstico del vault de estudio",
      defaultPath: "nodus-estudio-diagnostico.json",
      filters: [{ name: "JSON", extensions: ["json"] }],
    });
    if (picked.canceled || !picked.filePath) return null;
    fs.writeFileSync(
      picked.filePath,
      JSON.stringify(studyDataAdmin.buildStudyDiagnostic(), null, 2),
      "utf8",
    );
    return { path: picked.filePath };
  });
    ...
    if (willGeneratePortraits) {
      void generateDemoPortraits({
        onProgress: (done, total) =>
          getWindow()?.webContents.send("demo:portraits", { done, total }),
      }).catch(() => undefined);
    }
