...
export function registerLibraryIpc({ h }: IpcContext): void {
  ...
  'zh-TW': '為參考文獻新增附件', }), properties: ['openFile', 'multiSelections'] as Array<'openFile' | 'multiSelections'>,
  ...
  'zh-TW': '文件、資料和圖片', }), extensions: ['pdf', 'epub', 'md', 'markdown', 'txt', 'html', 'htm', 'xml', 'jats', 'doc', 'docx', 'odt', 'rtf', 'ppt', 'pptx', 'odp', 'csv', 'tsv', 'xlsx', 'xls', 'ods', 'png', 'jpg', 'jpeg', 'gif', 'webp', 'bmp', 'svg', 'heic', 'tif', 'tiff'] }],
    };
    const selected = owner ? await showImportOpenDialog(owner, options) : await showImportOpenDialog(options);
  ...
  'zh-TW': '替換附件', }), properties: ['openFile'] as Array<'openFile'> };
    const selected = owner ? await showImportOpenDialog(owner, options) : await showImportOpenDialog(options);
  ...
  'zh-TW': '匯入CSL樣式', }), properties: ['openFile', 'multiSelections'] as Array<'openFile' | 'multiSelections'>, filters: [{ name: 'Citation Style Language', extensions: ['csl'] }] };
    const selected = owner ? await showImportOpenDialog(owner, options) : await showImportOpenDialog(options);
  ...
  'zh-TW': '選擇Zotero的styles資料夾', }), properties: ['openDirectory'] as Array<'openDirectory'> };
    const selected = owner ? await showImportOpenDialog(owner, options) : await showImportOpenDialog(options);
  ...
  'zh-TW': '匯出參考文獻', }), defaultPath: `nodus-library.${extension}`, filters: [{ name: request.format, extensions: [extension] }] };
