...
export function registerLibraryIpc({ h }: IpcContext): void {
  ...
  h('library:replaceAttachment', async (event, itemId, attachmentId) => {
    const owner = BrowserWindow.fromWebContents(event.sender) ?? undefined;
    const options = { title: dialogText({ es: 'Sustituir adjunto', en: 'Replace attachment', fr: 'Remplacer la pièce jointe', de: 'Anhang ersetzen', pt: 'Substituir anexo', 'pt-BR': 'Substituir anexo', it: 'Sostituisci allegato', tr: 'Ek dosyayı değiştir', 'zh-CN': '替换附件' }), properties: ['openFile'] as Array<'openFile'> };
    const selected = owner ? await showImportOpenDialog(owner, options) : await showImportOpenDialog(options);
  ...
  h('library:importCitationStyles', async (event) => {
    const owner = BrowserWindow.fromWebContents(event.sender) ?? undefined;
    const options = { title: dialogText({ es: 'Importar estilos CSL', en: 'Import CSL styles', fr: 'Importer des styles CSL', de: 'CSL-Stile importieren', pt: 'Importar estilos CSL', 'pt-BR': 'Importar estilos CSL', it: 'Importa stili CSL', tr: 'CSL stillerini içe aktar', 'zh-CN': '导入CSL样式' }), properties: ['openFile', 'multiSelections'] as Array<'openFile' | 'multiSelections'>, filters: [{ name: 'Citation Style Language', extensions: ['csl'] }] };
    const selected = owner ? await showImportOpenDialog(owner, options) : await showImportOpenDialog(options);
  ...
  h('library:importZoteroCitationStyles', async (event) => {
    ...
    const owner = BrowserWindow.fromWebContents(event.sender) ?? undefined;
    const options = { title: dialogText({ es: 'Seleccionar la carpeta styles de Zotero', en: 'Select the Zotero styles folder', fr: 'Sélectionner le dossier styles de Zotero', de: 'Zotero-Ordner „styles“ auswählen', pt: 'Selecionar a pasta styles do Zotero', 'pt-BR': 'Selecionar a pasta styles do Zotero', it: 'Seleziona la cartella styles di Zotero', tr: 'Zotero styles klasörünü seç', 'zh-CN': '选择Zotero的styles文件夹' }), properties: ['openDirectory'] as Array<'openDirectory'> };
    const selected = owner ? await showImportOpenDialog(owner, options) : await showImportOpenDialog(options);
  ...
  h('library:exportBibliography', async (event, request) => {
    ...
    const owner = BrowserWindow.fromWebContents(event.sender) ?? undefined;
    const options = { title: dialogText({ es: 'Exportar referencias bibliográficas', en: 'Export bibliographic references', fr: 'Exporter les références bibliographiques', de: 'Literaturangaben exportieren', pt: 'Exportar referências bibliográficas', 'pt-BR': 'Exportar referências bibliográficas', it: 'Esporta riferimenti bibliografici', tr: 'Kaynakça kayıtlarını dışa aktar', 'zh-CN': '导出参考文献' }), defaultPath: `nodus-library.${extension}`, filters: [{ name: request.format, extensions: [extension] }] };
