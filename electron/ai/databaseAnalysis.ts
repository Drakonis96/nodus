...
追加のテキストやマークダウンを含めず、JSON配列のみを返してください。4-7個のオブジェクトを含み、最も興味深いものから順に並べ、次の形式に厳密に従ってください：
...
추가 텍스트나 마크다운 없이 JSON 배열만 반환하십시오. 4~7개의 객체를 포함하고 가장 흥미로운 것부터 순서대로 정렬하며 다음 형식을 정확히 따르십시오:
