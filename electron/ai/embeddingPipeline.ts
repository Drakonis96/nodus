...
export async function startEmbedding(nodusIds?: string[]): Promise<void> {
  ...
  try {
    ...
    if (!state.stopRequested && state.totalIdeas > 0) {
      ...
      addNotification({
        title: state.error
          ? uiText(language, { es: 'La indexación semántica necesita atención', en: 'Semantic indexing needs attention', fr: 'L’indexation sémantique nécessite votre attention', de: 'Die semantische Indexierung erfordert Aufmerksamkeit', pt: 'A indexação semântica requer atenção', 'pt-BR': 'A indexação semântica requer atenção', it: 'L’indicizzazione semantica richiede attenzione', tr: 'Anlamsal indeksleme ilgilenmenizi gerektiriyor' })
          : uiText(language, { es: 'Embeddings de ideas completados', en: 'Idea embeddings completed', fr: 'Embeddings d’idées terminés', de: 'Ideen-Embeddings abgeschlossen', pt: 'Embeddings de ideias concluídos', 'pt-BR': 'Embeddings de ideias concluídos', it: 'Embedding delle idee completati', tr: 'Fikir gömme işlemleri tamamlandı' }),
        body: state.error ? state.error : uiText(language, { es: `${state.ideasEmbedded} ideas indexadas en ${state.works.length} obra(s).`, en: `${state.ideasEmbedded} ideas indexed across ${state.works.length} work(s).`, fr: `${state.ideasEmbedded} idées indexées dans ${state.works.length} œuvre(s).`, de: `${state.ideasEmbedded} Ideen in ${state.works.length} Werk(en) indexiert.`, pt: `${state.ideasEmbedded} ideias indexadas em ${state.works.length} obra(s).`, 'pt-BR': `${state.ideasEmbedded} ideias indexadas em ${state.works.length} obra(s).`, it: `${state.ideasEmbedded} idee indicizzate in ${state.works.length} opere.`, tr: `${state.ideasEmbedded} fikir ${state.works.length} eserde indekslendi.` }),
