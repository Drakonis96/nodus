...
Return {"claims":[{"index":0,"kind":"fact|attributed|inference|nonfactual","dependsOnContext":false,"premises":[{"text":"one atomic proposition","type":"fact|attribution|absence|relation|inference","entailed":false,"evidence":[{"id":"authorized evidence ID","quote":"literal excerpt"}],"from":[]}],"unsupportedParts":[],"explicitInference":false,"supported":false,"reason":"short diagnostic"}]} with actual enum values and one result per index. Work in this order:
...
dependsOnContext: true when the sentence cannot be understood without the preceding context sentence (a pronoun, "this", "the first", "the same", an elided subject).
