...
export function runCompassWorker<T>(operation: CompassWorkerOperation, args: unknown[], storeFile?: string, fallback?: () => T): Promise<T> {
  const execute = async (): Promise<T> => { if (!compassWorkerAvailable()) { if (fallback) return fallback(); throw new Error('Compass worker is unavailable.'); } const worker = new Worker(workerFile()); active.add(worker); return new Promise<T>((resolve, reject) => { let settled = false; const finish = (error?: Error, value?: T) => { if (settled) return; settled = true; active.delete(worker); void worker.terminate().catch(() => undefined); error ? reject(error) : resolve(value!); }; worker.once('message', (m: { ok: boolean; result?: T; error?: string }) => m.ok ? finish(undefined, m.result) : finish(new Error(m.error || 'Compass worker failed.'))); worker.once('error', finish); worker.once('exit', (code) => { if (!settled) finish(new Error(`Compass worker exited with code ${code}.`)); }); worker.postMessage({ operation, args, storeFile }); }); };
