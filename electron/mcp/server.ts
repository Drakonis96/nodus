...
function makeMcpServer(): McpServer {
  const server = new McpServer({
    ...
    instructions: [
      ...
      'Deep Research has two distinct views. nodus_list_deep_research_jobs is only the temporary generation lane; an empty lane says nothing about saved reports. Use nodus_list_deep_research_reports to browse the persistent gallery page by page and nodus_get_deep_research_report to read one full report. To create a report, prefer nodus_enqueue_deep_research (returns a job id at once, survives this session ending, notifies the user in Nodus) over the blocking nodus_generate_deep_research, unless the report is needed in this very turn. Reports generate one at a time in a lane shared with the Nodus window, and each is bound to the vault active when it was queued. Follow one with nodus_get_deep_research_job; never present a queued report as written until its status is "completed".',
      ...
      'For broad questions, nodus_search_documents locates globally relevant works and nodus_search_hybrid combines document, idea and passage lanes. Document profiles are generated orientation metadata and must never be cited; use nodus_search_passages or the original work for evidence. nodus_search_ideas searches derived claims.',
      ...
      'Every read-only tool remains listed across vault switches and reads the active vault, so clients that cache tools/list never lose access to ideas, works, passages, notes or another readable layer. A layer that has no data in the active vault returns an empty page or not-found. Write and action tools remain scoped to compatible vault types. Genealogy / primary-source reads cover persons, kinship, timeline and evidence archives; database reads cover schemas and rows; study/teaching reads cover courses, documents, questions, progress, schedules, groups, gradebooks, exams and rubrics; the namespaced prosopography and worldbuilding read tools expose their complete local records.',
