...
function authPage(form:FormDefinition,dark:boolean,error=''):string { return shell(form.title,`<main class="card"><div class="brand"><span aria-hidden="true">◆</span> Nodus Research</div><h1>${escapeHtml(form.title)}</h1><p>${escapeHtml(form.description)}</p>${error?`<div class="error" role="alert">${escapeHtml(error)}</div>`:''}<form method="get"><div class="field"><label for="token">Token de acceso</label><input id="token" name="token" type="password" required autocomplete="current-password"></div><div class="actions"><button type="submit">Acceder</button></div></form></main>`,dark); }
...
function formPage(form:FormDefinition,token:string|null,dark:boolean,error=''):string { return shell(form.title,`<main class="card"><div class="brand"><span aria-hidden="true">◆</span> Nodus Research</div><h1>${escapeHtml(form.title)}</h1><p>${escapeHtml(form.description)}</p>${error?`<div class="error" role="alert">${escapeHtml(error)}</div>`:''}<form method="post">${token?`<input type="hidden" name="_token" value="${escapeHtml(token)}">`:''}<div class="grid">${fieldHtml(form)}</div><div class="actions"><button type="submit">Enviar respuesta</button></div></form></main>`,dark); }
function confirmationPage(form:FormDefinition,dark:boolean):string { return shell(form.confirmationTitle,`<main class="card"><div class="brand"><span aria-hidden="true">✓</span> Nodus Research</div><h1>${escapeHtml(form.confirmationTitle)}</h1><p>${escapeHtml(form.confirmationBody)}</p></main>`,dark); }
