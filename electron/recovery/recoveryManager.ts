...
export function inspectRecoveryFolder(folder: string, language: AppLanguage = 'es'): RecoveryFolderInspection {
  ...
  if (!fs.existsSync(clean)) return { path: clean, kind: 'missing', message: tr(language, { es: 'La carpeta no existe.', en: 'The folder does not exist.', fr: "Le dossier n'existe pas.", de: 'Der Ordner existiert nicht.', pt: 'A pasta não existe.', 'pt-BR': 'A pasta não existe.', it: 'La cartella non esiste.', tr: 'Klasör mevcut değil.' }), snapshots: [] };
  let stat: fs.Stats;
  try {
    stat = fs.statSync(clean);
  } catch {
    return { path: clean, kind: 'missing', message: tr(language, { es: 'No se puede acceder a la carpeta.', en: 'The folder cannot be accessed.', fr: "Impossible d'accéder au dossier.", de: 'Auf den Ordner kann nicht zugegriffen werden.', pt: 'Não é possível aceder à pasta.', 'pt-BR': 'Não é possível acessar a pasta.', it: 'Impossibile accedere alla cartella.', tr: 'Klasöre erişilemiyor.' }), snapshots: [] };
  }
  if (!stat.isDirectory()) return { path: clean, kind: 'invalid', message: tr(language, { es: 'La ruta seleccionada no es una carpeta.', en: 'The selected path is not a folder.', fr: "Le chemin sélectionné n'est pas un dossier.", de: 'Der ausgewählte Pfad ist kein Ordner.', pt: 'O caminho selecionado não é uma pasta.', 'pt-BR': 'O caminho selecionado não é uma pasta.', it: 'Il percorso selezionato non è una cartella.', tr: 'Seçilen yol bir klasör değil.' }), snapshots: [] };
  ...
  if (manifest) {
    ...
    return {
      ...
      message: snapshots.length
        ? tr(language, { es: `${snapshots.length} copia(s) válida(s) encontrada(s).`, en: `${snapshots.length} valid snapshot(s) found.`, fr: `${snapshots.length} sauvegarde(s) valide(s) trouvée(s).`, de: `${snapshots.length} gültige Sicherung(en) gefunden.`, pt: `${snapshots.length} cópia(s) de segurança válida(s) encontrada(s).`, 'pt-BR': `${snapshots.length} backup(s) válido(s) encontrado(s).`, it: `Trovate ${snapshots.length} copie valide.`, tr: `${snapshots.length} geçerli anlık görüntü bulundu.` })
  ...
  return {
    ...
    message: result.usedRecoveryKey
      ? tr(language, {
        ...
        tr: 'Veriler kurtarma anahtarıyla geri yüklendi. Ayarlar’dan yeni bir parola belirleyebilirsiniz.',
      })
      : tr(language, {
        ...
        tr: 'Veriler geri yüklendi ve güvenlik için geri yükleme öncesi anlık görüntü saklandı.',
      }),
    snapshot,
  };
}
