...
export function inspectRecoveryFolder(folder: string, language: AppLanguage = 'es'): RecoveryFolderInspection {
  ...
  if (!fs.existsSync(clean)) return { path: clean, kind: 'missing', message: tr(language, { es: 'La carpeta no existe.', en: 'The folder does not exist.', fr: "Le dossier n'existe pas.", de: 'Der Ordner existiert nicht.', pt: 'A pasta não existe.', 'pt-BR': 'A pasta não existe.', it: 'La cartella non esiste.', tr: 'Klasör mevcut değil.', 'zh-CN': '该文件夹不存在。' }), snapshots: [] };
  let stat: fs.Stats;
  try {
    stat = fs.statSync(clean);
  } catch {
    return { path: clean, kind: 'missing', message: tr(language, { es: 'No se puede acceder a la carpeta.', en: 'The folder cannot be accessed.', fr: "Impossible d'accéder au dossier.", de: 'Auf den Ordner kann nicht zugegriffen werden.', pt: 'Não é possível aceder à pasta.', 'pt-BR': 'Não é possível acessar a pasta.', it: 'Impossibile accedere alla cartella.', tr: 'Klasöre erişilemiyor.', 'zh-CN': '无法访问该文件夹。' }), snapshots: [] };
  }
  if (!stat.isDirectory()) return { path: clean, kind: 'invalid', message: tr(language, { es: 'La ruta seleccionada no es una carpeta.', en: 'The selected path is not a folder.', fr: "Le chemin sélectionné n'est pas un dossier.", de: 'Der ausgewählte Pfad ist kein Ordner.', pt: 'O caminho selecionado não é uma pasta.', 'pt-BR': 'O caminho selecionado não é uma pasta.', it: 'Il percorso selezionato non è una cartella.', tr: 'Seçilen yol bir klasör değil.', 'zh-CN': '所选路径不是文件夹。' }), snapshots: [] };
...
function inspectionFromProbe(probe: RecoveryFolderProbe, language: AppLanguage): RecoveryFolderInspection {
  ...
  return {
    ...probe,
    message: tr(language, { es: `La carpeta debe estar vacía o contener una recuperación de Nodus válida. Contiene ${entries} elemento(s).`, en: `The folder must be empty or contain a valid Nodus recovery. It contains ${entries} item(s).`, fr: `Le dossier doit être vide ou contenir une récupération Nodus valide. Il contient ${entries} élément(s).`, de: `Der Ordner muss leer sein oder eine gültige Nodus-Wiederherstellung enthalten. Er enthält ${entries} Element(e).`, pt: `A pasta deve estar vazia ou conter uma recuperação válida do Nodus. Contém ${entries} elemento(s).`, 'pt-BR': `A pasta deve estar vazia ou conter uma recuperação válida do Nodus. Ela contém ${entries} elemento(s).`, it: `La cartella deve essere vuota o contenere un ripristino Nodus valido. Contiene ${entries} elementi.`, tr: `Klasör boş olmalı veya geçerli bir Nodus kurtarması içermelidir. ${entries} öğe içeriyor.`, 'zh-CN': `文件夹必须为空或包含有效的 Nodus 恢复。其中包含 ${entries} 个项目。` }),
  };
}
