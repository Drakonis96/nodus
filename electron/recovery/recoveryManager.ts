...
export function inspectRecoveryFolder(folder: string, language: AppLanguage = 'es'): RecoveryFolderInspection {
  ...
  if (manifest) {
    ...
    return {
      ...
      message: snapshots.length
        ? tr(language, { es: `${snapshots.length} copia(s) válida(s) encontrada(s).`, en: `${snapshots.length} valid snapshot(s) found.`, fr: `${snapshots.length} sauvegarde(s) valide(s) trouvée(s).`, de: `${snapshots.length} gültige Sicherung(en) gefunden.`, pt: `${snapshots.length} cópia(s) de segurança válida(s) encontrada(s).`, 'pt-BR': `${snapshots.length} backup(s) válido(s) encontrado(s).`, it: `Trovate ${snapshots.length} copie valide.` })
  ...
  if (entries.length === 0) return { path: clean, kind: 'empty', message: tr(language, { es: 'Carpeta vacía y disponible.', en: 'Empty folder, ready to use.', fr: 'Dossier vide et disponible.', de: 'Ordner leer und verfügbar.', pt: 'Pasta vazia e disponível.', 'pt-BR': 'Pasta vazia e disponível.', it: 'Cartella vuota e disponibile.' }), snapshots: [] };
  return {
    path: clean,
    kind: 'invalid',
...
export async function initializeRecoveryFolder(
  ...
  if (cleanPassword.length < 8) return { ok: false, message: tr(language, { es: 'La contraseña debe tener al menos 8 caracteres.', en: 'The password must be at least 8 characters long.', fr: 'Le mot de passe doit contenir au moins 8 caractères.', de: 'Das Passwort muss mindestens 8 Zeichen lang sein.', pt: 'A palavra-passe deve ter pelo menos 8 caracteres.', 'pt-BR': 'A senha deve ter pelo menos 8 caracteres.', it: 'La password deve contenere almeno 8 caratteri.' }) };
