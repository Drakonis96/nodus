...
- The migration is resumable and works offline. Every phase is recorded before the next begins, so a crash resumes rather than restarts, and the signed packages travel inside the update itself so an upgrade with no connection still completes. On macOS, Chemistry Studio is the one exception: its package vendors prebuilt binaries for platforms it never runs on, an application carrying them cannot be notarized, and it is installed from the catalog the first time the migration runs with a connection. A failure leaves the skill with the activation you chose, the previous data intact and a retry, and never holds up the window.
