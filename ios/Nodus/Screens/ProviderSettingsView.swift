...
/// The live model list for one task's provider.
struct ModelPickerView: View {
    ...
    var body: some View {
        List {
            Section("Proveedor") {
                ForEach(settings.availableProviders(for: task), id: \.self) { candidate in
                    Button {
                        provider = candidate
                        Task { await loadModels(candidate) }
                    } label: {
                        HStack {
                ...
                if settings.availableProviders(for: task).isEmpty {
                    Text("Añade la clave de algún proveedor para poder elegir modelo.")
                        .font(.caption).foregroundStyle(.secondary)
                }
            }
            ...
            if !models.isEmpty, let provider {
                Section("Modelos") {
                    ForEach(models, id: \.self) { model in
                        Button {
                            settings.setModel(ModelRef(provider: provider, model: model), for: task)
                            dismiss()
                        } label: {
                            HStack {
                                Text(model).font(.callout.monospaced()).foregroundStyle(.primary)
                                Spacer()
                                if settings.model(for: task)?.model == model {
                                    Image(systemName: "checkmark").foregroundStyle(accent)
                                }
                            }
                        }
        ...
        .navigationBarTitleDisplayMode(.inline)
        .task {
            provider = settings.model(for: task)?.provider ?? settings.availableProviders(for: task).first
            if let provider { await loadModels(provider) }
        }
    }

    private func loadModels(_ provider: AIProvider) async {
        isLoading = true
        defer { isLoading = false }
        do {
            let catalogue = ModelCatalogue(keyProvider: settings.keyProvider)
            let all = try await catalogue.models(for: provider)
            models = task == .image ? all : all.filter(RequestShaping.isChatModel)
            error = nil
        } catch {
            models = []
            self.error = error.localizedDescription
