...
try {
  ...
  // Capability API v2. The marketplace validates and signs against the same contract the
  // application enforces, so it is generated here rather than copied or re-typed.
  //
  // The banner carries no trailing newline: esbuild adds one, and `verify-cross-repo.mjs`
  // regenerates this file to compare it byte for byte. A newline here and not there meant
  // the writer and the checker could never agree on the same contract.
  await build({ entryPoints: ['packages/capability-api/src/index.ts'], bundle: true, platform: 'node', format: 'esm', outfile: path.join(target, 'scripts/contract-v2.mjs'), banner: { js: '// GENERATED by Nodus scripts/sync-skill-marketplace.mjs from @nodusresearch/capability-api; do not edit.' } });
  // Native API additions also need the validators published by the marketplace.
  // Do not rewrite curated Skill instructions when only synchronizing contracts.
  if (!contractsOnly) {
