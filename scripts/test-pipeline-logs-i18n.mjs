...
test('the language selector offers every supported language, ordered like Settings', () => {
  ...
  const fromSelect = [...uiLanguageSelect.matchAll(/<option value="(es|en|fr|de|pt|pt-BR|it|tr|zh-CN|zh-TW|ja)">([^<]+)<\/option>/g)]
    .map((match) => `${match[1]}:${match[2]}`);
  assert.equal(fromList.length, 11, 'the log language selector must offer all eleven languages');
  assert.equal(fromSelect.length, 11, 'the Settings selector must offer all eleven languages');
