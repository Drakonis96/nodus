...
test('#9: build-zotero-xpi produces a valid xpi + updates.json', () => {
  ...
  assert.equal(manifest.version, '4.0.1', 'the add-on shares the current Nodus 4 release version');
  assert.equal(manifest.license, 'AGPL-3.0-only');
  assert.match(zip.readAsText('SOURCE_CODE.md'), /releases\/tag\/v4\.0\.1/);
