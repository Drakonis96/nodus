...
test('the app performs one visible update check per launch after the update guides settle', () => {
  ...
  assert.match(app, /whatsNewSettled && pdfPresenterTutorialSettled && mobileTeaserSettled && platformHighlightsSettled && toolkitBetaTourSettled && tutorialVideosSettled && !manualWhatsNewOpen && !updateSettled && \(\s*<StartupUpdateModal[\s\S]*?\/>/);
