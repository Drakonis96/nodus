...
try {
  ...
  assert.equal(pluginManifest.version, '5.0.1');
  ...
  assert.match(serverVersion, /export const NODUS_VERSION = '5\.0\.1'/);
  assert.match(serverVersion, /tree\/v\$\{NODUS_VERSION\}/);
  assert.match(sourceOffer, /archive\/refs\/tags\/v5\.0\.1\.tar\.gz/);
  assert.match(citation, /^date-released: "2026-08-25"$/m);
