...
try {
  ...
  assert.equal(pluginManifest.version, '4.2.2');
  ...
  assert.match(serverVersion, /export const NODUS_VERSION = '4\.2\.2'/);
  assert.match(serverVersion, /tree\/v\$\{NODUS_VERSION\}/);
  assert.match(sourceOffer, /archive\/refs\/tags\/v4\.2\.2\.tar\.gz/);
  assert.match(citation, /^date-released: "2026-08-21"$/m);
