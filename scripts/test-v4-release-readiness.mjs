...
try {
  ...
  assert.equal(pluginManifest.version, '5.2.0');
  ...
  assert.match(serverVersion, /export const NODUS_VERSION = '5\.2.0'/);
  assert.match(serverVersion, /tree\/v\$\{NODUS_VERSION\}/);
  assert.match(sourceOffer, /archive\/refs\/tags\/v5\.2.0\.tar\.gz/);
