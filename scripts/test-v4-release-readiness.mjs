...
try {
  ...
  assert.equal(pluginManifest.version, '5.4.2');
  ...
  assert.match(serverVersion, /export const NODUS_VERSION = '5\.4\.2'/);
  assert.match(serverVersion, /tree\/v\$\{NODUS_VERSION\}/);
  assert.match(sourceOffer, /archive\/refs\/tags\/v5\.4\.2\.tar\.gz/);
  assert.match(citation, /^date-released: "2026-09-13"$/m);
