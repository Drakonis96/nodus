...
try {
  ...
  assert.equal(pluginManifest.version, '4.0.0');
  ...
  assert.match(serverVersion, /export const NODUS_VERSION = '4\.0\.0'/);
  assert.match(serverVersion, /tree\/v\$\{NODUS_VERSION\}/);
  assert.match(sourceOffer, /archive\/refs\/tags\/v4\.0\.0\.tar\.gz/);
  assert.match(citation, /^date-released: "2026-08-11"$/m);
