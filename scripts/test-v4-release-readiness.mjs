...
try {
  ...
  assert.equal(pluginManifest.version, '5.6.0');
  ...
  assert.match(serverVersion, /export const NODUS_VERSION = '5\.6\.0'/);
  assert.match(serverVersion, /tree\/v\$\{NODUS_VERSION\}/);
  assert.match(sourceOffer, /archive\/refs\/tags\/v5\.6\.0\.tar\.gz/);
  assert.match(citation, /^date-released: "2026-09-20"$/m);
