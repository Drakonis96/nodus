...
try {
  ...
  assert.equal(pluginManifest.version, '4.1.2');
  ...
  assert.match(serverVersion, /export const NODUS_VERSION = '4\.1\.2'/);
  assert.match(serverVersion, /tree\/v\$\{NODUS_VERSION\}/);
  assert.match(sourceOffer, /archive\/refs\/tags\/v4\.1\.2\.tar\.gz/);
