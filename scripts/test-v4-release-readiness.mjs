...
try {
  ...
  assert.equal(pluginManifest.version, '4.2.3');
  ...
  assert.match(serverVersion, /export const NODUS_VERSION = '4\.2\.3'/);
  assert.match(serverVersion, /tree\/v\$\{NODUS_VERSION\}/);
  assert.match(sourceOffer, /archive\/refs\/tags\/v4\.2\.3\.tar\.gz/);
