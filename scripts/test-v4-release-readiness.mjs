...
try {
  ...
  assert.equal(pluginManifest.version, '4.1.6');
  ...
  assert.match(serverVersion, /export const NODUS_VERSION = '4\.1\.6'/);
  assert.match(serverVersion, /tree\/v\$\{NODUS_VERSION\}/);
  assert.match(sourceOffer, /archive\/refs\/tags\/v4\.1\.6\.tar\.gz/);
  assert.match(citation, /^date-released: "2026-08-18"$/m);
