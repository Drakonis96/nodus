...
test('Server Web keeps every navigation entry on a reloadable URL contract', () => {
  ...
  assert.match(app, /onOpenRecord=\{\(collection, id\) => navigate\(`\/detail\/\$\{route\.view\}\/\$\{encodeURIComponent\(collection\)\}\/\$\{encodeURIComponent\(id\)\}`\)\}/);
...
test('Server Web preserves safe new-tab semantics for published readers', () => {
  ...
  assert.match(personal, /window\.open\([^\n]*, '_blank', 'noopener,noreferrer'\)/);
