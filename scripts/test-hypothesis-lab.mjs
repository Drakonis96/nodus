...
try {
  ...
  // Every supported prompt language must survive the runtime normalisation and
  // produce native deterministic scaffolding (the source data itself is English,
  // so assert the generated labels/instructions rather than corpus excerpts).
  const localeMarkers = {
  ...
  for (const [language, marker] of Object.entries(localeMarkers)) {
    const localized = buildHypothesisLabFallback({
      ...argumentsForTest(),
      request: { objective: '', mode: 'comparative', language, maxCandidates: 1 },
    });
    const candidate = localized.candidates[0];
    assert.ok(candidate.title.includes(marker), `${language} title is localized`);
    assert.ok(candidate.methods.every((item) => typeof item === 'string' && item.length > 0), `${language} methods exist`);
    assert.ok(candidate.predictions.every((item) => typeof item === 'string' && item.length > 0), `${language} predictions exist`);
    assert.equal(candidate.evidence[0].citation, 'nodus://gap/gap-1', `${language} citation contract is stable`);
  }
...
function argumentsForTest() {
  return {
    generatedAt: '2026-07-06T00:00:00.000Z',
    ideas: [{ id: 'g-1', label: 'Tourism as heritage mediation', statement: 'Tourism mediates public memory through heritage routes.', type: 'claim', themes: ['tourism', 'heritage'], workIds: ['w-1'], workCount: 4, evidenceCount: 6 }],
    gaps: [{ id: 'gap-1', kind: 'open_question', statement: 'The corpus does not explain how heritage tourism changes memory practices across cases.', confidence: 0.86, relatedIdeaId: 'g-1', workId: 'w-1', workTitle: 'Heritage Routes', authors: ['Rivera'], year: 2020, evidenceQuote: 'Future work should compare how routes produce memory.' }],
    debates: [{ id: 'edge-1', fromId: 'g-1', toId: 'g-2', fromLabel: 'Tourism mediates memory', toLabel: 'Tourism commodifies memory', explanation: 'The corpus contains a tension between mediation and commodification.', confidence: 0.78 }],
    works: [{ id: 'w-1', title: 'Heritage Routes', authors: ['Rivera'], year: 2020, themes: ['tourism', 'heritage'], deepStatus: 'done', ideaCount: 8, gapCount: 3, summary: 'A study of tourism routes and memory.' }],
    passages: 12,
    project: { id: 'p-1', title: 'Thesis', brief: 'A thesis about tourism and memory.', linkLabels: ['Tourism as heritage mediation'] },
  };
}
