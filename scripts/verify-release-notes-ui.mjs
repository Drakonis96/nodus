...
const indices = [0,1,2,3,4,8,9,5,6,7,10];
const expected = indices.map(index => current.highlights[index]);
const output = 'artifacts/release-5.4.2';
...
try {
  ...
  for (const theme of ['light', 'dark']) for (const lang of ['es','en','fr','de','pt','pt-BR','it','tr']) {
    ...
    assert.equal(await release.locator('.whats-new-release-version').textContent(), 'v5.4.2');
  ...
  await fs.writeFile(`${output}/modal-order-es.md`, `# Novedades de Nodus 5.4.2\n\n${expected.map((highlight,index)=>`${index+1}. ${highlight.es}`).join('\n\n')}\n`);
  console.log('PASS: 11 release notes in exact displayed order, all eight languages in light/dark, v5.4.2 active and unpublished v5.3.2 absent.');
