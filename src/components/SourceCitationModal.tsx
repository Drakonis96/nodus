...
function PassagePanel({ passageId, onOpenTarget, onTitle, authors, onOpenLibraryWork }: CitationPanelProps & { passageId: string }) {
  ...
  return (
    <div className="space-y-5" data-testid="source-citation-passage">
      <div className="rounded-2xl border border-emerald-500/25 bg-gradient-to-br from-emerald-500/10 via-neutral-900/60 to-neutral-950 p-5 sm:p-6"><div className="flex flex-wrap items-center gap-2"><Badge color="green">{detail.provenance === 'abstract' ? t('Resumen') : t('Pasaje de texto completo')}</Badge>{detail.page_label && <Badge>{detail.page_label}</Badge>}{page !== null && <button type="button" data-testid="source-citation-jump-passage" className="btn btn-ghost h-7 border border-emerald-700/70 px-2 text-[11px] text-emerald-300" title={t('Abrir fuente')} onClick={() => detail.libraryItemId ? requestLibraryDocumentOpen({ itemId: detail.libraryItemId, scope: 'global', page }) : void openEvidenceAtPage(detail.nodus_id, evidenceLocator({ location: detail.page_label, source_ref: detail.source_ref, page_number: detail.page_number }))}><Icon name="external" size={12} /> {tx('Ver página {n}', { n: page })}</button>}</div><h2 className="mt-3 text-xl font-semibold leading-tight text-neutral-100">{detail.work.title}</h2><div className="mt-2"><AuthorLinks names={detail.work.authors} authors={authors} onOpenTarget={onOpenTarget} /></div></div>
