...
export function CompassFilters({ filters, onChange, open, onToggle, onClear }: {
  ...
  return <section className="border-t border-neutral-200 pt-3 dark:border-neutral-800" aria-label={compassT('Filtros')}>
    ...
    {open && <div className="mt-3 grid grid-cols-2 gap-3 md:grid-cols-4 lg:grid-cols-7">
      ...
      <label className="text-xs text-neutral-500">{compassT('Tipo')}<select className="input mt-1 h-8 w-full text-xs" value={filters.types?.[0] ?? ''} onChange={(e) => onChange({ types: e.target.value ? [e.target.value as NonNullable<Filters['types']>[number]] : undefined })}><option value="" />{['article', 'book', 'chapter', 'thesis', 'report', 'dataset', 'preprint', 'photograph', 'newspaper', 'map', 'manuscript', 'audio', 'video', 'archive-item'].map((x) => <option key={x} value={x}>{compassT(x)}</option>)}</select></label>
      ...
      <label className="flex items-end gap-2 pb-1 text-xs text-neutral-600 dark:text-neutral-300"><input type="checkbox" checked={filters.digitallyAvailableOnly ?? false} onChange={(e) => onChange({ digitallyAvailableOnly: e.target.checked })} /> {compassT('Disponible digitalmente')}</label>
    </div>}
  </section>;
}
