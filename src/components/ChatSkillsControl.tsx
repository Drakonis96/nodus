  ...
  return <div className={`chat-skills-control ${compact ? 'compact' : ''}`} ref={root}>
    ...
    {open && renderPanel(<div ref={panelRef} style={compact ? undefined : panelStyle} className="chat-skills-panel" data-nodi-interactive role="region" aria-label="Skills">
      ...
      {draft ? <form className="chat-skill-editor" onSubmit={event => { event.preventDefault(); void mutate(() => window.nodus.saveChatSkill(draft)).then(saved => { if (saved) { setDraft(null); setQuery(''); } }); }}>
        ...
        <div className="chat-skills-list">{visibleSkills.map(skill => <div key={skill.id} className={`chat-skill-item ${skill.enabled[surface] ? 'enabled' : ''} ${skill.builtin === 'svg' || skill.builtin === 'image' || skill.builtin === 'genomics' || skill.builtin === 'legal' ? 'tool-skill' : ''}`} data-skill-kind={skill.builtin === 'svg' || skill.builtin === 'image' || skill.builtin === 'genomics' || skill.builtin === 'legal' ? 'tool' : 'prompt'}>
          <div className="chat-skill-main"><span className={`chat-skill-symbol ${skill.builtin ?? 'custom'}`}><Icon name={skill.builtin === 'svg' ? 'code' : skill.builtin === 'image' ? 'image' : skill.builtin === 'socratic' ? 'graduation' : 'sparkles'} size={20} /></span><div><b>{skill.name}</b>{(skill.builtin === 'svg' || skill.builtin === 'image' || skill.builtin === 'genomics' || skill.builtin === 'legal') && <span className="chat-skill-tool-badge">{t('Con herramientas')}</span>}<p>{skill.description}</p></div><button type="button" role="switch" aria-checked={skill.enabled[surface]} aria-label={`${t('Activar')} ${skill.name}`} disabled={busy} className="chat-skill-switch" onClick={() => void toggle(skill)}><span /></button></div>
