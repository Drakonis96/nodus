  ...
  return <div className={`chat-skills-control ${compact ? 'compact' : ''}`} ref={root} style={accentStyle}>
    ...
    {open && renderPanel(<div ref={panelRef} style={compact ? undefined : panelStyle} className="chat-skills-panel" data-nodi-interactive role="region" aria-label="Skills">
      ...
      {marketplace && !draft ? <SkillMarketplacePanel skills={skills} accent={accent} /> : draft ? <form className="chat-skill-editor" onSubmit={event => { event.preventDefault(); void mutate(() => window.nodus.saveChatSkill(draft)).then(saved => { if (saved) { setDraft(null); setQuery(''); } }); }}>
        ...
        <fieldset><legend>Native capabilities</legend>{SUPPORTED_SKILL_CAPABILITIES.map(capability => <label key={capability}><input type="checkbox" checked={draft.builtin === capability || draft.capabilities?.includes(capability) === true} disabled={draft.builtin === capability} onChange={e => setDraft({ ...draft, capabilities: e.target.checked ? [...(draft.capabilities ?? []), capability] : draft.capabilities?.filter(c => c !== capability) })} />{capability}</label>)}</fieldset>
        ...
        <div className="chat-skills-list">{visibleSkills.map(skill => <div key={skill.id} className={`chat-skill-item ${skill.enabled[surface] ? 'enabled' : ''} ${skill.builtin === 'svg' || skill.builtin === 'image' || skill.builtin === 'chemistry' || skill.capabilities?.length || skill.tools?.length ? 'tool-skill' : ''}`} data-skill-kind={skill.builtin === 'svg' || skill.builtin === 'image' || skill.builtin === 'chemistry' || skill.capabilities?.length || skill.tools?.length ? 'tool' : 'prompt'}>
          <div className="chat-skill-main"><span className={`chat-skill-symbol ${skill.builtin ?? 'custom'}`}><Icon name={skill.builtin === 'svg' ? 'code' : skill.builtin === 'image' ? 'image' : skill.builtin === 'socratic' ? 'graduation' : 'sparkles'} size={20} /></span><div><b>{skill.name}</b>{(skill.builtin === 'svg' || skill.builtin === 'image' || skill.builtin === 'chemistry' || !!skill.capabilities?.length || !!skill.tools?.length) && <span className="chat-skill-tool-badge">{t('Con herramientas')}</span>}<p>{skill.description}</p></div><button type="button" role="switch" aria-checked={skill.enabled[surface]} aria-label={`${t('Activar')} ${skill.name}`} disabled={busy} className="chat-skill-switch" onClick={() => void mutate(() => window.nodus.saveChatSkill({ ...skill, enabled: { ...skill.enabled, [surface]: !skill.enabled[surface] } }))}><span /></button></div>
          <div className="chat-skill-item-foot"><button type="button" title="Export package directory" aria-label={`Export ${skill.name}`} onClick={() => { void window.nodus.exportSkillPackage(skill.id).then(path => { if (path) setExportNotice(`Package exported to ${path}`); }).catch(e => setError(String(e))); }}>Export</button><small>{skill.builtin === 'image' ? imageModel : skill.builtin === 'svg' ? t('Vectorial · editable · preciso') : skill.builtin === 'socratic' ? t('Aprendizaje guiado · paso a paso') : skill.builtin ? t('Skill incluida') : t('Skill personal')}</small><button type="button" title={t('Editar skill')} aria-label={`${t('Editar')} ${skill.name}`} onClick={() => setDraft(structuredClone(skill))}><Icon name="edit" size={13} /></button><button type="button" title={t('Eliminar skill')} aria-label={`${t('Eliminar')} ${skill.name}`} onClick={() => setRemoveId(skill.id)}><Icon name="trash" size={13} /></button></div>
