...
export function BrowserBookmarksManager({ store, onClose, onEdit, onCreate, onNotice }: {
  ...
  const render = (parentId: string | null, depth: number): React.ReactNode => browserBookmarkChildren(store, parentId).map((ref) => {
    if (ref.kind === 'folder') {
      ...
      return <div key={folder.id}>
        <div className="browser-bookmark-row group flex items-center gap-1 rounded-lg py-1 pr-1 hover:bg-neutral-100 dark:hover:bg-neutral-800" style={{ paddingLeft: `${8 + depth * 18}px` }} draggable onDragStart={() => setDragging(ref)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); void drop(folder.id); }}>
          <button className="rounded p-1" aria-label={open ? t('Contraer carpeta') : t('Expandir carpeta')} onClick={() => setExpanded((current) => { const next = new Set(current); if (open) next.delete(folder.id); else next.add(folder.id); return next; })}><Icon name={open ? 'chevronDown' : 'chevronRight'} size={12} /></button><Icon name="folder" size={15} className="text-indigo-400" /><span className="min-w-0 flex-1 truncate text-sm">{folder.name}</span><span className="opacity-0 group-hover:opacity-100"><RowButton icon="chevronUp" label={t('Subir')} onClick={() => void shift(ref, folder.parentId, -1)} /><RowButton icon="chevronDown" label={t('Bajar')} onClick={() => void shift(ref, folder.parentId, 1)} /><RowButton icon="plus" label={t('Añadir un marcador')} onClick={() => onCreate(folder.id)} /><RowButton icon="folderPlus" label={t('Añadir subcarpeta')} onClick={() => setFolderEditor({ parentId: folder.id, name: '' })} /><RowButton icon="edit" label={t('Renombrar')} onClick={() => setFolderEditor({ id: folder.id, parentId: folder.parentId, name: folder.name })} /><RowButton icon="trash" label={t('Eliminar')} onClick={() => setDeleteConfirmation({ ref, label: folder.name })} /></span>
    ...
    return <div key={bookmark.id} className="browser-bookmark-row group flex items-center gap-2 rounded-lg py-1.5 pr-1 hover:bg-neutral-100 dark:hover:bg-neutral-800" style={{ paddingLeft: `${34 + depth * 18}px` }} draggable onDragStart={() => setDragging(ref)}>
      {bookmark.faviconDataUrl ? <img src={bookmark.faviconDataUrl} className="h-4 w-4" alt="" /> : <Icon name="globe" size={14} className="opacity-50" />}<button className="min-w-0 flex-1 text-left" onClick={() => void window.nodus.openBrowserTab(bookmark.url)}><div className="truncate text-sm">{bookmark.title}</div><div className="truncate text-[11px] text-neutral-500">{bookmark.url}</div></button><span className="opacity-0 group-hover:opacity-100"><RowButton icon="chevronUp" label={t('Subir')} onClick={() => void shift(ref, bookmark.parentId, -1)} /><RowButton icon="chevronDown" label={t('Bajar')} onClick={() => void shift(ref, bookmark.parentId, 1)} /><RowButton icon="external" label={t('Abrir en una pestaña nueva')} onClick={() => void window.nodus.openBrowserTab(bookmark.url)} /><RowButton icon="edit" label={t('Editar')} onClick={() => onEdit(bookmark)} /><RowButton icon="copy" label={t('Copiar URL')} onClick={() => void navigator.clipboard.writeText(bookmark.url)} /><RowButton icon="trash" label={t('Eliminar')} onClick={() => setDeleteConfirmation({ ref, label: bookmark.title })} /></span>
    </div>;
  });
  ...
  const commit = async () => { if (!preview) return; setBusy(true); try { const result = await window.nodus.commitBrowserBookmarksImport(preview.token); setPreview(null); onNotice(tx('Se importaron {bookmarks} marcadores y {folders} carpetas. Se omitieron {duplicates} duplicados.', { bookmarks: result.summary.bookmarks, folders: result.summary.folders, duplicates: result.summary.duplicates })); } catch (cause) { onNotice(cause instanceof Error ? cause.message : String(cause)); } finally { setBusy(false); } };
  ...
  return createPortal(<div className="fixed inset-0 z-[145] flex items-center justify-center bg-black/60 p-5" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) onClose(); }}><section className="card-modal flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden" role="dialog" aria-modal="true" data-testid="browser-bookmarks-manager">
    <header className="flex items-center gap-3 border-b border-neutral-200 p-4 dark:border-neutral-800"><span className="grid h-10 w-10 place-items-center rounded-xl bg-indigo-500/15 text-indigo-400"><Icon name="bookmark" size={19} /></span><div className="min-w-0 flex-1"><h2 className="font-semibold">Nodus Bookmarks</h2><p className="text-xs text-neutral-500">{tx('{bookmarks} marcadores · {folders} carpetas · privado y local', { bookmarks: store.bookmarks.length, folders: store.folders.length })}</p></div><button className="rounded p-2 hover:bg-neutral-100 dark:hover:bg-neutral-800" onClick={onClose}><Icon name="x" /></button></header>
    <div className="flex flex-wrap items-center gap-2 border-b border-neutral-200 p-3 dark:border-neutral-800"><div className="flex min-w-48 flex-1 items-center gap-2 rounded-lg border border-neutral-300 px-2 dark:border-neutral-700"><Icon name="search" size={14} /><input className="h-9 min-w-0 flex-1 bg-transparent text-sm outline-none" placeholder={t('Buscar marcadores…')} value={query} onChange={(event) => setQuery(event.target.value)} /></div><button className="btn btn-ghost border border-neutral-300 dark:border-neutral-700" onClick={() => onCreate(null)}><Icon name="plus" size={13} /> {t('Marcador')}</button><button className="btn btn-ghost border border-neutral-300 dark:border-neutral-700" onClick={() => setFolderEditor({ parentId: null, name: '' })}><Icon name="folderPlus" size={13} /> {t('Carpeta')}</button><button className="btn btn-ghost border border-neutral-300 dark:border-neutral-700" disabled={busy} onClick={() => void importFile()}><Icon name="upload" size={13} /> {t('Importar')}</button><button className="btn btn-ghost border border-neutral-300 dark:border-neutral-700" onClick={() => void window.nodus.exportBrowserBookmarks('json')}><Icon name="download" size={13} /> JSON</button><button className="btn btn-ghost border border-neutral-300 dark:border-neutral-700" onClick={() => void window.nodus.exportBrowserBookmarks('html')}><Icon name="download" size={13} /> HTML</button></div>
    {preview && <div className="m-3 rounded-xl border border-indigo-500/40 bg-indigo-500/10 p-3 text-sm"><p className="font-semibold">{tx('Vista previa de importación · {fileName}', { fileName: preview.fileName })}</p><p className="mt-1 text-xs text-neutral-500">{tx('{bookmarks} marcadores · {folders} carpetas · {duplicates} duplicados · {invalidUrls} URL no válidas omitidas', { bookmarks: preview.bookmarks, folders: preview.folders, duplicates: preview.duplicates, invalidUrls: preview.invalidUrls })}{preview.truncated ? ` · ${t('límites aplicados')}` : ''}</p><div className="mt-3 flex gap-2"><button className="btn btn-ghost" onClick={() => setPreview(null)}>{t('Cancelar')}</button><button className="btn btn-primary" disabled={busy} onClick={() => void commit()}>{t('Importar sin sobrescribir')}</button></div></div>}
    <div className="browser-bookmarks-tree min-h-60 flex-1 p-3" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { if (event.target === event.currentTarget) { event.preventDefault(); void drop(null); } }}>{render(null, 0)}{store.bookmarks.length === 0 && store.folders.length === 0 && <div className="grid min-h-56 place-items-center text-center text-sm text-neutral-500"><div><Icon name="bookmark" size={28} className="mx-auto mb-3 opacity-40" /><p>{t('Aún no hay marcadores.')}</p><button className="btn btn-primary mt-3" onClick={() => onCreate(null)}>{t('Añade tu primer marcador')}</button></div></div>}</div>
    {folderEditor && <div className="absolute inset-0 z-10 grid place-items-center bg-black/55 p-5"><form className="card-modal w-full max-w-md p-5" onSubmit={(event) => { event.preventDefault(); void saveFolder(); }}><h3 className="text-lg font-semibold">{t(folderEditor.id ? 'Renombrar carpeta' : 'Nueva carpeta')}</h3><p className="mt-1 text-xs text-neutral-500">{t('Las carpetas son datos privados de Nodus y pueden contener subcarpetas.')}</p><label className="mt-4 block text-sm font-medium" htmlFor="browser-folder-name">{t('Nombre de la carpeta')}</label><input id="browser-folder-name" autoFocus className="input mt-1 w-full" value={folderEditor.name} maxLength={160} onChange={(event) => setFolderEditor({ ...folderEditor, name: event.target.value })} /><div className="mt-5 flex justify-end gap-2"><button type="button" className="btn btn-ghost" disabled={busy} onClick={() => setFolderEditor(null)}>{t('Cancelar')}</button><button type="submit" className="btn btn-primary" disabled={busy || !folderEditor.name.trim()}>{t('Guardar')}</button></div></form></div>}
    {deleteConfirmation && <div className="absolute inset-0 z-10 grid place-items-center bg-black/55 p-5"><section className="card-modal w-full max-w-md p-5" role="alertdialog" aria-modal="true"><h3 className="text-lg font-semibold">{tx('¿Eliminar «{label}»?', { label: deleteConfirmation.label })}</h3><p className="mt-2 text-sm text-neutral-500">{deleteConfirmation.ref.kind === 'folder' ? t('Se eliminará la carpeta y todo su contenido.') : t('Este marcador se eliminará de Nodus Bookmarks.')}</p><div className="mt-5 flex justify-end gap-2"><button type="button" className="btn btn-ghost" disabled={busy} onClick={() => setDeleteConfirmation(null)}>{t('Cancelar')}</button><button type="button" className="btn btn-danger" disabled={busy} onClick={() => void remove()}>{t('Eliminar')}</button></div></section></div>}
