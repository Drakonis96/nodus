...
type PdfTool = 'highlight' | 'underline' | 'brush' | 'sticky' | 'comment';
...
export function PdfViewer({ content, material, onAnnotation, onDeleteAnnotation, onCreateNote }: {
  ...
  const toolButton = (value: PdfTool, icon: string, label: string) => <button data-testid={`study-pdf-tool-${value}`} className={`btn h-8 px-2.5 ${tool === value ? 'btn-secondary' : 'btn-ghost'}`} title={t(label)} onClick={() => setTool(value)}><Icon name={icon} size={14} /><span className="hidden 2xl:inline">{t(label)}</span></button>;
  ...
  return <div className="flex h-full min-h-0 flex-col" data-testid="study-pdf-viewer">
    <div className="flex flex-wrap items-center gap-1.5 border-b border-neutral-200 bg-white px-3 py-2 dark:border-neutral-800 dark:bg-neutral-950">
      ...
      {toolButton('highlight', 'edit', 'Resaltar')}{toolButton('underline', 'minus', 'Subrayar')}{toolButton('brush', 'palette', 'Pincel')}{toolButton('sticky', 'star', 'Sticker con nota')}{toolButton('comment', 'chat', 'Comentario lateral')}
      <div className="flex items-center gap-1 rounded-lg border border-neutral-200 px-1.5 py-1 dark:border-neutral-800" data-testid="study-pdf-color-picker">{activeColors.map((color) => <button key={color} aria-label={`${t('Color')} ${color}`} className={`h-5 w-5 rounded-full border ${activeColor === color ? 'ring-2 ring-teal-500 ring-offset-1 dark:ring-offset-neutral-950' : 'border-black/10'}`} style={{ backgroundColor: color }} onClick={() => tool === 'highlight' ? setHighlightColor(color) : setInkColor(color)} />)}</div>
...
function AnnotationOverlay({ annotation }: { annotation: StudyMaterialAnnotation }) {
  ...
  if (annotation.kind === 'sticky' || annotation.kind === 'comment') return annotation.rect ? <span className={`pointer-events-none absolute z-10 grid h-6 w-6 place-items-center rounded shadow ${annotation.kind === 'sticky' ? 'bg-amber-200 text-amber-900' : 'bg-indigo-200 text-indigo-900'}`} style={{ left: `${annotation.rect.x * 100}%`, top: `${annotation.rect.y * 100}%` }}>{annotation.kind === 'sticky' ? '★' : '💬'}</span> : null;
  const rects = annotation.rects.length ? annotation.rects : annotation.rect ? [annotation.rect] : [];
  return <>{rects.map((rect, index) => <span key={index} className="pointer-events-none absolute z-10" style={annotation.kind === 'underline' ? { left: `${rect.x * 100}%`, top: `${(rect.y + rect.height) * 100}%`, width: `${rect.width * 100}%`, borderBottom: `${annotation.thickness}px solid ${annotation.color}` } : { left: `${rect.x * 100}%`, top: `${rect.y * 100}%`, width: `${rect.width * 100}%`, height: `${rect.height * 100}%`, backgroundColor: annotation.color, opacity: .42, mixBlendMode: 'multiply' }} />)}</>;
}
...
function AnnotationCard({ annotation, onOpen, onCreateNote, onDelete }: { annotation: StudyMaterialAnnotation; onOpen: () => void; onCreateNote: () => Promise<void>; onDelete: () => Promise<void> }) {
  ...
  return <article className="mb-2 rounded-lg border border-neutral-200 p-2.5 dark:border-neutral-800"><button className="w-full text-left" onClick={onOpen}><span className="flex items-center gap-2 text-[10px] text-neutral-500"><i className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: annotation.color }} />{label}{annotation.pageNumber ? ` · ${t('Página')} ${annotation.pageNumber}` : ''}</span>{annotation.selectedText && <p className="mt-1 line-clamp-3 text-xs italic text-neutral-500">“{annotation.selectedText}”</p>}{annotation.note && <p className="mt-1 text-xs leading-5 text-neutral-700 dark:text-neutral-300">{annotation.note}</p>}</button><div className="mt-2 flex text-[10px]"><button className="text-teal-600 dark:text-teal-400" onClick={() => void onCreateNote()}>{t('Crear apunte')}</button><button className="ml-auto text-red-600" onClick={() => void onDelete()}>{t('Eliminar')}</button></div></article>;
}
