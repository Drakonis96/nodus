...
function PdfViewer(props: ViewerProps) {
  ...
  return <section className="flex min-h-0 flex-1 flex-col" data-testid="library-reader-pdf-viewer">
    <div className="flex flex-wrap items-center justify-center gap-2 border-b border-neutral-800 px-3 py-2">
      ...
      <div className="flex items-center gap-1 rounded-lg border border-neutral-800 p-0.5">
        <button className="btn btn-ghost h-8 w-8 p-0" aria-label={t('Alejar')} title={t('Alejar')} onClick={() => setScale((value) => Math.max(.6, value - .15))}><Icon name="minus" size={13} /></button><button className="min-w-12 text-xs tabular-nums text-neutral-500" onClick={() => setScale(1.25)}>{Math.round(scale * 100)}%</button><button className="btn btn-ghost h-8 w-8 p-0" aria-label={t('Acercar')} title={t('Acercar')} onClick={() => setScale((value) => Math.min(2.5, value + .15))}><Icon name="plus" size={13} /></button>
      </div>
