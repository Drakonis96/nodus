...
export function ConflictsView({ onNavigate }: { onNavigate?: (view: View) => void }) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex shrink-0 items-center gap-2 border-b border-neutral-800 px-4 pt-3">
        {(['board', 'list'] as const).map((entry) => (
          <button
            ...
            className={`rounded-t-lg px-3 py-1.5 text-sm ${
              tab === entry ? 'bg-neutral-900 text-neutral-100' : 'text-neutral-500 hover:text-neutral-300'
            }`}
          >
  ...
  return (
    ...
    >
      <span className="flex items-baseline gap-2">
        ...
        <span className="shrink-0 text-[10px] text-neutral-600">{t(THREAD_STATUS_LABEL[item.status])}</span>
      </span>
      ...
      {!compact && item.pitch && (
        <span className="mt-0.5 line-clamp-1 block text-[11px] text-neutral-600">{item.pitch}</span>
      )}
    </button>
  );
}
...
function ConflictBoard({ onNavigate }: { onNavigate?: (view: View) => void }) {
  ...
  if (!board) return <p className="p-6 text-sm text-neutral-500">{t('Cargando…')}</p>;

  if (board.columns.length === 0) {
    return (
      <div className="p-6" data-testid="conflicts-board">
        <p className="text-sm text-neutral-400">{t('Todavía no hay conflictos.')}</p>
  ...
  return (
    <div className="min-h-0 flex-1 overflow-auto p-4" data-testid="conflicts-board">
      <table className="w-full border-separate border-spacing-0 text-xs">
        <thead>
          <tr>
            <th className="sticky left-0 z-10 bg-neutral-950 p-2 text-left text-[10px] uppercase tracking-wider text-neutral-600">
              {t('Reparto')}
            </th>
            {board.columns.map((thread) => (
              <th key={thread.threadId} className="p-2 text-left align-bottom">
                <span className="block max-w-28 truncate text-[11px] font-normal text-neutral-300" title={thread.title}>
                  {thread.title}
                </span>
              </th>
            ))}
            ...
            <th className="p-2 text-left text-[10px] uppercase tracking-wider text-neutral-600">{t('Quiere')}</th>
          </tr>
        </thead>
        <tbody>
          {board.rows.map((row) => (
            <tr key={row.person.personId} data-testid="conflicts-board-row" data-stakes={row.stakes}>
              <th className="sticky left-0 z-10 bg-neutral-950 p-2 text-left font-normal">
                <span className="block max-w-40 truncate text-neutral-200">{row.person.displayName}</span>
                <span className="block text-[10px] text-neutral-600">
                  {tx('{count} escenas', { count: String(row.person.sceneCount) })}
                </span>
              </th>
              {row.cells.map((side, index) => (
                <td key={board.columns[index].threadId} className="border-t border-neutral-900 p-1 text-center">
                  {side ? (
                    <span
              ...
              <td className="max-w-48 border-t border-neutral-900 p-2 text-[11px] text-neutral-500">
      ...
      {crossed.length > 0 && (
        <section className="mt-4 rounded-lg border border-neutral-800 p-3" data-testid="conflicts-crossed">
          ...
          <ul className="space-y-0.5">
            {crossed.map((entry) => (
              <li key={`${entry.threadId}:${entry.personId}:${entry.groupId}`} className="text-[11px] text-neutral-400">
...
function StakeGaps({ gaps, onNavigate }: { gaps: StakeGap[]; onNavigate?: (view: View) => void }) {
  ...
  return (
    <section className="mt-4 rounded-lg border border-amber-900/50 bg-amber-950/10 p-3" data-testid="conflicts-gaps">
      ...
      <ul className="space-y-0.5">
        {gaps.slice(0, 10).map((gap) => (
          <li key={gap.personId} className="text-[11px] text-neutral-300">
            · {gap.displayName}
    ...
    <div className="space-y-5 p-6" data-testid="conflict-sheet">
      ...
      <section className={PERSON_DOSSIER_SECTION_CLASS} data-testid="conflict-parties">
        <div className="grid gap-3 sm:grid-cols-3">
          {SIDES.map((side) => (
            <div key={side}>
              <h3 className="mb-1 text-[10px] uppercase tracking-wide text-neutral-500">{t(PARTY_SIDE_LABEL[side])}</h3>
              <ul className="space-y-0.5">
                {thread.parties.filter((party) => party.side === side).map((party) => (
                  <li key={`${party.partyKind}:${party.partyId}`} className="truncate text-xs text-neutral-200">
                    {party.partyName}
                  </li>
                ))}
                {thread.parties.every((party) => party.side !== side) && (
                  <li className="text-[11px] text-neutral-700">{t('Nadie')}</li>
                )}
              </ul>
            </div>
          ))}
      ...
      <section className={PERSON_DOSSIER_SECTION_CLASS} data-testid="conflict-beats">
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-neutral-500">{t('Cómo se mueve')}</h3>
        {beats.length === 0 ? (
            ...
            {[...beats]
              ...
              .map((beat) => (
                <li key={beat.sceneId} className="flex items-baseline gap-2 text-xs">
                  ...
                  {beat.subjectName && <span className="shrink-0 text-[10px] text-neutral-600">{beat.subjectName}</span>}
                </li>
              ))}
          </ul>
        )}
      ...
      {suggestions.length > 0 && (
        <section className={PERSON_DOSSIER_SECTION_CLASS} data-testid="conflict-suggestions">
          <h3 className="mb-1 text-xs font-semibold uppercase tracking-wider text-neutral-500">{t('Podría pasar en')}</h3>
          <p className="mb-2 text-[10px] text-neutral-600">
            {t('Escenas donde estaban los dos bandos y no has dicho que pasara nada.')}
          </p>
          <ul className="space-y-0.5">
            {suggestions.map((suggestion) => (
              <li key={suggestion.sceneId} className="truncate text-[11px] text-neutral-400">
                · {suggestion.title}
                <span className="ml-1 text-neutral-600">{suggestion.present.join(', ')}</span>
              </li>
            ))}
          </ul>
          <button
...
export function CharacterThreadsSection({ personId }: { personId: string }) {
  ...
  if (conflicts.length === 0) {
    return (
      <section className={PERSON_DOSSIER_SECTION_CLASS} data-testid="character-threads">
        <h3 className="mb-1 text-xs font-semibold uppercase tracking-wider text-neutral-500">{t('Qué tiene en juego')}</h3>
        <p className="text-[11px] leading-4 text-neutral-600">{t('No tiene nada en juego con nadie.')}</p>
      </section>
    );
  }
  ...
  return (
    <section className={PERSON_DOSSIER_SECTION_CLASS} data-testid="character-threads">
      <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-neutral-500">{t('Qué tiene en juego')}</h3>
      <ul className="space-y-1">
        {conflicts.map((thread) => {
          ...
          return (
            <li key={thread.threadId} className="flex items-baseline gap-2 text-xs">
              <span className="min-w-0 flex-1 truncate text-neutral-200">{thread.title}</span>
              {side && <span className="shrink-0 text-[10px] text-neutral-600">{t(PARTY_SIDE_LABEL[side.side])}</span>}
              <span className="shrink-0 text-[10px] text-neutral-700">{t(THREAD_STATUS_LABEL[thread.status])}</span>
            </li>
          );
        })}
      </ul>
