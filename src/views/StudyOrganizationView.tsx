...
function StudyBrowserActions({ item, onRename, onMove, onDuplicate, onDelete }: { item: StudyBrowserItem; onRename: () => void; onMove: () => void; onDuplicate: () => void; onDelete: () => void }) {
  return (
    <span className="flex items-center justify-end gap-0.5">
      ...
      {item.kind !== 'course' && <button className="btn btn-ghost h-7 px-2" title={t('Mover a otra ubicación')} aria-label={t('Mover a otra ubicación')} onClick={onMove}><Icon name="folderMove" size={12} /></button>}
...
function StudyMaterialCollection({ materials, layout, onOpen, onRename, onMove, onDelete }: { materials: StudyMaterialSummary[]; layout: 'grid' | 'list'; onOpen: (material: StudyMaterialSummary) => void; onRename: (material: StudyMaterialSummary) => void; onMove: (material: StudyMaterialSummary) => void; onDelete: (material: StudyMaterialSummary) => void }) {
...
function StudyEntityLocationDialog({ item, mode, workspace, onSave, onCancel }: { item: StudyBrowserItem; mode: 'move' | 'duplicate'; workspace: StudyWorkspace; onSave: (input: { courseId: string; subjectId: string; folderId: string | null; parentId: string | null }) => Promise<void>; onCancel: () => void }) {
  ...
  const initialCourseId = subject?.courseId ?? folder?.courseId ?? workspace.subjects.find((entry) => entry.id === initialSubjectId)?.courseId ?? '';
  ...
  const hierarchy = item.kind === 'folder' ? workspace.folders : item.kind === 'topic' ? workspace.topics : [];
  let changed = true;
  while (changed) {
    changed = false;
    for (const entry of hierarchy) if (entry.parentId && descendants.has(entry.parentId) && !descendants.has(entry.id)) { descendants.add(entry.id); changed = true; }
  }
  const allowed = (id: string) => mode === 'duplicate' || !descendants.has(id);
  const needsSubject = item.kind === 'topic' || (item.kind === 'folder' && workspace.topics.some((entry) => entry.folderId && descendants.has(entry.folderId)));
  const subjects = workspace.subjects.filter((entry) => entry.courseId === courseId);
  const folders = workspace.folders.filter((entry) => (entry.subjectId ?? '') === subjectId && (entry.courseId ?? '') === courseId && (item.kind !== 'folder' || allowed(entry.id)));
  const topics = workspace.topics.filter((entry) => entry.subjectId === subjectId && entry.folderId === (folderId || null) && allowed(entry.id));
  const valid = item.kind === 'subject' ? Boolean(courseId) : (!needsSubject || Boolean(subjectId));
  return createPortal(<div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/60 p-6" onClick={() => { if (!busy) onCancel(); }}><form ref={form} role="dialog" aria-modal="true" aria-labelledby={titleId} onKeyDown={(event) => {
  ...
  }} data-testid="study-entity-location-dialog" className="card w-full max-w-md space-y-3 p-5" onClick={(event) => event.stopPropagation()} onSubmit={(event) => { event.preventDefault(); if (!valid || busy) return; setBusy(true); setError(''); void onSave({ courseId, subjectId, folderId: folderId || null, parentId: parentId || null }).catch((reason) => { setError(reason instanceof Error ? reason.message : String(reason)); setBusy(false); }); }}>
    <h2 id={titleId} className="text-base font-semibold">{t(mode === 'move' ? 'Mover elemento' : 'Duplicar en otra ubicación')}</h2><p className="text-xs text-neutral-500">{item.name}</p>
    <fieldset disabled={busy} className="space-y-3"><label className="block text-xs text-neutral-500">{t('Curso')}<select aria-label={t('Curso')} className="input mt-1 w-full" value={courseId} onChange={(event) => { setCourseId(event.target.value); setSubjectId(''); setFolderId(''); setParentId(''); }}><option value="">{t(item.kind === 'folder' ? 'Sin curso' : 'Selecciona un curso')}</option>{workspace.courses.filter((entry) => entry.id !== item.id).map((entry) => <option key={entry.id} value={entry.id}>{entry.name}</option>)}</select></label>
    {item.kind !== 'subject' && <label className="block text-xs text-neutral-500">{t('Asignatura')}<select aria-label={t('Asignatura')} className="input mt-1 w-full" value={subjectId} onChange={(event) => { setSubjectId(event.target.value); setFolderId(''); setParentId(''); }}><option value="">{t(needsSubject ? 'Selecciona una asignatura' : 'Sin asignatura')}</option>{subjects.map((entry) => <option key={entry.id} value={entry.id}>{entry.name}</option>)}</select></label>}
    {item.kind === 'folder' && <label className="block text-xs text-neutral-500">{t('Carpeta superior (opcional)')}<select aria-label={t('Carpeta superior (opcional)')} className="input mt-1 w-full" value={parentId} onChange={(event) => setParentId(event.target.value)}><option value="">{t('Sin carpeta')}</option>{folders.map((entry) => <option key={entry.id} value={entry.id}>{paths.label({ courseId: entry.courseId, subjectId: entry.subjectId, folderId: entry.id, topicId: null })}</option>)}</select></label>}
    {item.kind === 'topic' && <><label className="block text-xs text-neutral-500">{t('Carpeta (opcional)')}<select aria-label={t('Carpeta (opcional)')} className="input mt-1 w-full" value={folderId} onChange={(event) => { setFolderId(event.target.value); setParentId(''); }}><option value="">{t('Directamente en la asignatura')}</option>{folders.map((entry) => <option key={entry.id} value={entry.id}>{entry.name}</option>)}</select></label><label className="block text-xs text-neutral-500">{t('Tema superior (opcional)')}<select aria-label={t('Tema superior (opcional)')} className="input mt-1 w-full" value={parentId} onChange={(event) => setParentId(event.target.value)}><option value="">{t('Tema principal')}</option>{topics.map((entry) => <option key={entry.id} value={entry.id}>{entry.name}</option>)}</select></label></>}
    </fieldset>
    {error && <p role="alert" className="text-xs text-red-400">{error}</p>}<div className="flex justify-end gap-2"><button type="button" className="btn btn-ghost" disabled={busy} onClick={onCancel}>{t('Cancelar')}</button><button className="btn btn-primary" disabled={!valid || busy}>{busy ? t('Guardando…') : t(mode === 'move' ? 'Mover' : 'Duplicar')}</button></div>
...
export function StudyOrganizationView({
  ...
  const [movingMaterial, setMovingMaterial] = useState<StudyMaterialSummary | null>(null);
  ...
  const browserItems = useMemo<StudyBrowserItem[]>(() => {
    ...
    const folderItems = (parentId: string | null, subjectId: string | null, courseId: string | null): StudyBrowserItemDraft[] => workspace.folders
      .filter((folder) => folder.parentId === parentId && folder.subjectId === subjectId && folder.courseId === courseId)
      .map((folder) => ({
    ...
    if (!target) {
      return filter([...folderItems(null, null, null), ...workspace.courses.map<StudyBrowserItemDraft>((course) => ({
    ...
    if (target.kind === 'course') {
      return filter([...folderItems(null, null, target.id), ...workspace.subjects.filter((subject) => subject.courseId === target.id).map<StudyBrowserItemDraft>((subject) => ({
    ...
    if (target.kind === 'subject') {
      ...
      const topics: StudyBrowserItemDraft[] = workspace.topics.filter((topic) => topic.subjectId === target.id && !topic.folderId && !topic.parentId).map<StudyBrowserItemDraft>((topic) => ({
    ...
    if (target.kind === 'folder') {
      const folder = workspace.folders.find((entry) => entry.id === target.id);
      return filter([...folderItems(target.id, folder?.subjectId ?? null, folder?.courseId ?? null), ...workspace.topics.filter((topic) => topic.folderId === target.id && !topic.parentId).map<StudyBrowserItemDraft>((topic) => ({
        kind: 'topic', id: topic.id, name: topic.name, description: topic.description, icon: topic.icon || 'hash',
        emoji: topic.emoji, imageData: topic.imageData, year: topic.year, color: topic.color, position: topic.position, createdAt: topic.createdAt, updatedAt: topic.updatedAt,
        courseId: workspace.subjects.find((subject) => subject.id === topic.subjectId)?.courseId ?? null, subjectId: topic.subjectId, topicId: topic.id, folderId: topic.folderId,
    ...
    if (target.kind === 'topic') {
      return filter(workspace.topics.filter((topic) => topic.parentId === target.id).map<StudyBrowserItemDraft>((topic) => ({
        kind: 'topic', id: topic.id, name: topic.name, description: topic.description, icon: topic.icon || 'hash',
        emoji: topic.emoji, imageData: topic.imageData, year: topic.year, color: topic.color, position: topic.position, createdAt: topic.createdAt, updatedAt: topic.updatedAt,
        courseId: workspace.subjects.find((subject) => subject.id === topic.subjectId)?.courseId ?? null, subjectId: topic.subjectId, topicId: topic.id, folderId: topic.folderId,
        meta: t('Subtema'),
  ...
  return (
    ...
    >
      ...
      {movingDocument && workspace && <StudyDocumentMoveDialog document={movingDocument} workspace={workspace} initialPlacementId={workspace.placements.find((p) => p.documentId === movingDocument.id && target && p[`${target.kind}Id` as 'courseId' | 'subjectId' | 'folderId' | 'topicId'] === target.id)?.id} onCancel={() => setMovingDocument(null)} onMoved={() => { setMovingDocument(null); }} />}
      {movingMaterial && workspace && <StudyMaterialMoveDialog material={movingMaterial} workspace={workspace} initialPlacementId={movingMaterial.placements.find((p) => target && p[`${target.kind}Id` as 'courseId' | 'subjectId' | 'folderId' | 'topicId'] === target.id)?.id} onCancel={() => setMovingMaterial(null)} onMoved={() => { setMovingMaterial(null); }} />}
