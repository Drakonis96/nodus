 ...
 *
 * A report takes minutes, so a spinner and a sentence left no way to tell a report
 * that had just started from one about to land — least of all with several queued
 * behind it. The percentage comes from the phases the pipeline already reports
 * (see shared/deepResearchProgress.ts), so it is measured work, not a timer.
 */
  ...
  return (
    ...
    >
      <div
        ...
        // A bar at literal 0% looks broken rather than early, so the first sliver
        // is always visible once the report is actually running.
    ...
    <div className="border-b border-neutral-800 bg-indigo-950/15 px-4 py-2.5">
      ...
      <div className="flex flex-col gap-1.5">
        {active.map((item, index) => {
          const ahead = active.slice(0, index).filter((other) => other.status !== 'failed').length;
