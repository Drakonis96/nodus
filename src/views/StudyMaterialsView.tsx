...
import type { StudyMaterialNavigationTarget } from '../navigation';
...
export function StudyMaterialsView({ onOpenDocument, target }: { onOpenDocument: (id: string) => void; target?: StudyMaterialNavigationTarget | null }) {
  ...
  // A citation names the material and the page or slide it was anchored to. The
  // locator is kept beside the selection so the viewer opens there; selecting the
  // material alone is what left every study citation on page 1.
  const [materialLocator, setMaterialLocator] = useState<StudyMaterialNavigationTarget | null>(null);
  useEffect(() => {
    if (!target?.id) return;
    setSelected(target.id);
    setMaterialLocator(target);
  }, [target]);
  const openMaterial = (id: string) => { setMaterialLocator(null); setSelected(id); };
  ...
  >
    ...
    <main className="relative min-h-0 flex-1 overflow-auto">
      ...
      {materials.length === 0 ? null : <MaterialTable
        ...
        onOpen={(material) => { if (material.origin === 'zotero_link') void window.nodus.openStudyMaterialInZotero(material.id); else openMaterial(material.id); }}
...
function MaterialViewer({ materialId, locator, workspace, onClose, onChanged, onOpenDocument, onRequestDelete }: { materialId: string; locator?: StudyMaterialNavigationTarget | null; workspace: StudyWorkspace | null; onClose: () => void; onChanged: () => Promise<void>; onOpenDocument: (id: string) => void; onRequestDelete: (material: StudyMaterialDetail) => void }) {
  ...
  return <div className="absolute inset-0 z-40 flex flex-col bg-neutral-50 dark:bg-neutral-950" data-testid="study-material-viewer">
    ...
    <main className="relative min-h-0 flex-1 overflow-hidden">
      {tab === 'preview' && <Preview material={material} content={content} locator={locator} objectUrl={objectUrl} onAnnotation={createAnnotation} onUpdateAnnotation={updateAnnotation} onDeleteAnnotation={async (id) => { await window.nodus.deleteStudyMaterialAnnotation(id); await changed(); }} onCreateNote={createNote} onSelectText={captureTextSelection} />}
...
function Preview({ material, content, locator, objectUrl, onAnnotation, onUpdateAnnotation, onDeleteAnnotation, onCreateNote, onSelectText }: { material: StudyMaterialDetail; content: StudyMaterialContent; locator?: StudyMaterialNavigationTarget | null; objectUrl: string; onAnnotation: (input: Parameters<typeof window.nodus.createStudyMaterialAnnotation>[1]) => Promise<void>; onUpdateAnnotation: (id: string, patch: Parameters<typeof window.nodus.updateStudyMaterialAnnotation>[1]) => Promise<void>; onDeleteAnnotation: (id: string) => Promise<void>; onCreateNote: (annotationId?: string | null) => Promise<void>; onSelectText: () => void }) {
  if (material.previewKind === 'pdf') return <Suspense fallback={<div className="grid h-full place-items-center"><Spinner label={t('Cargando visor PDF…')} /></div>}><PdfViewer content={content} material={material} initialPage={locator?.pageNumber ?? null} onAnnotation={onAnnotation} onUpdateAnnotation={onUpdateAnnotation} onDeleteAnnotation={onDeleteAnnotation} onCreateNote={onCreateNote} /></Suspense>;
  ...
  if (material.previewKind === 'presentation') return <PresentationPreview material={material} initialSlide={locator?.slideNumber ?? null} onSelectText={onSelectText} />;
...
function PresentationPreview({ material, initialSlide, onSelectText }: { material: StudyMaterialDetail; initialSlide?: number | null; onSelectText: () => void }) {
  const markers = parseStudyMaterialMarkers(material.extractedText).filter((marker) => marker.kind === 'slide');
  const [slide, setSlide] = useState(() => Math.max(1, initialSlide ?? 1));
