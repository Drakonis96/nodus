...
export function StudyReviewView() {
  ...
  return <div className="flex h-full min-h-0 flex-col bg-neutral-50 text-neutral-900 dark:bg-neutral-950 dark:text-neutral-100" data-testid="study-review-view">
    ...
    <main className="min-h-0 flex-1 overflow-y-auto p-5">
      ...
      {step === 'session' && <section className="mx-auto max-w-2xl" data-testid="study-review-session">
        <div className="mb-3 flex items-center text-xs text-neutral-500"><span>{index + 1} / {items.length}</span><div className="mx-3 h-1.5 flex-1 overflow-hidden rounded-full bg-neutral-200 dark:bg-neutral-800"><div className="h-full bg-teal-500" style={{ width: `${((index + 1) / items.length) * 100}%` }} /></div><button className="btn btn-ghost h-8" onClick={reset}>{t('Salir')}</button></div>
        ...
        <div role="button" tabIndex={0} aria-expanded={revealed} className="min-h-80 w-full cursor-pointer rounded-2xl border border-neutral-200 bg-white p-8 text-center shadow-sm dark:border-neutral-800 dark:bg-neutral-900/45" onClick={() => setRevealed(true)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setRevealed(true); } }}>
          <div className="text-xl leading-8"><StudyMarkdown content={items[index]?.front ?? ''} /></div>
          {!revealed ? <p className="mt-10 text-xs text-neutral-500">{t('Pulsa para mostrar la respuesta')}</p> : <><div className="mx-auto my-7 h-px max-w-sm bg-neutral-200 dark:bg-neutral-800"/><div className="text-lg text-emerald-700 dark:text-emerald-300"><StudyMarkdown content={items[index]?.back ?? ''} /></div></>}
        </div>
        {revealed && (kind === 'flashcards' ? <div className="mt-4 grid grid-cols-4 gap-2">{([{r:1,l:'Otra vez'},{r:3,l:'Difícil'},{r:4,l:'Bien'},{r:5,l:'Fácil'}] as const).map((entry) => <button key={entry.r} data-testid={`study-review-rate-${entry.r}`} className="rounded-xl border border-neutral-200 bg-white p-3 text-sm dark:border-neutral-800 dark:bg-neutral-900" onClick={() => void next(entry.r)}>{t(entry.l)}</button>)}</div> : <button className="btn btn-primary mt-4 ml-auto" onClick={() => void next()}>{t('Siguiente')}<Icon name="chevronRight" /></button>)}
      </section>}
