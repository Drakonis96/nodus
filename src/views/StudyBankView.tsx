  ...
  return (
    <div className="flex h-full min-h-0 flex-col" data-testid="study-question-bank">
      ...
      {draft && <section className="m-4 grid gap-3 rounded-xl border border-teal-900/60 bg-teal-950/15 p-4 lg:grid-cols-2" data-testid="study-question-editor">
        ...
        <div className="lg:col-span-2 text-xs text-neutral-500"><span className="block">{t('Enunciado')}</span><MarkdownField autoFocus rows={3} testId="study-question-prompt-editor" className="mt-1" value={draft.prompt} onChange={(value) => setDraft({ ...draft, prompt: value })} /></div>
        ...
        <div className="text-xs text-neutral-500"><span className="block">{t('Respuesta correcta / modelo')}</span><MarkdownField rows={3} className="mt-1" value={draft.answer?.text ?? ''} onChange={(value) => setDraft({ ...draft, answer: { ...draft.answer, text: value } })} /></div>
        <div className="text-xs text-neutral-500"><span className="block">{t('Explicación y justificación')}</span><MarkdownField rows={3} className="mt-1" value={draft.explanation ?? ''} onChange={(value) => setDraft({ ...draft, explanation: value, source: { ...draft.source, excerpt: value } })} /></div>
        ...
        {['single_choice', 'multiple_choice'].includes(draft.type) && <div className="lg:col-span-2 text-xs text-neutral-500"><span className="block">{t('Opciones (una por línea; anteponer * a las correctas)')}</span><MarkdownField textareaClassName="font-mono" rows={5} testId="study-question-options-editor" className="mt-1" value={(draft.options ?? []).map((option) => `${option.correct ? '* ' : ''}${option.text}`).join('\n')} onChange={(value) => setDraft({ ...draft, options: value.split('\n').map((line, index) => ({ id: `O${index + 1}`, text: line.replace(/^\*\s*/, '').trim(), correct: /^\*\s*/.test(line) })).filter((option) => option.text) })} preview={(value) => <ul className="space-y-1 text-sm">{value.split('\n').map((line) => line.replace(/^\*\s*/, '').trim()).filter(Boolean).map((text, index) => <li key={`${index}-${text}`} className="flex items-start gap-2"><span className="text-neutral-500">{index + 1}.</span><StudyMarkdownInline content={text} /></li>)}</ul>} /></div>}
      ...
      {showGenerator && <section className="m-4 rounded-xl border border-indigo-900/60 bg-indigo-950/15 p-4" data-testid="study-question-generator">
        ...
        {generated.length > 0 && <div className="mt-3 space-y-2"><div className="flex items-center"><strong className="text-xs">{generated.length} {t('borradores para revisar')}</strong><button className="btn btn-primary ml-auto" onClick={() => void saveGenerated()}>{t('Aprobar y guardar todo')}</button></div>{generated.map((question, index) => <article key={`${question.prompt}-${index}`} className="rounded-lg border border-neutral-800 bg-neutral-950/50 p-3"><StudyMarkdown content={question.prompt} className="text-sm" /><StudyMarkdown content={question.answer?.text ?? String(question.answer?.value ?? '')} className="mt-1 text-xs text-emerald-300" /><p className="mt-1 text-[10px] text-neutral-600">{question.source?.title} · <StudyMarkdownInline content={question.explanation ?? ''} /></p></article>)}</div>}
      </section>}
      ...
      {!draft && !showGenerator && <div className="min-h-0 flex-1 overflow-y-auto p-4">
          ...
          <div className="overflow-x-auto"><table className="w-full min-w-[920px] border-collapse text-left text-xs">
            ...
            <tbody className="divide-y divide-neutral-800">{tab === 'questions' && questions.map((question) => {
              ...
              return <tr data-testid={`study-question-${question.id}`} key={question.id} className={`study-question-table-row cursor-pointer ${selectedId === question.id ? 'is-selected bg-teal-950/25' : 'hover:bg-neutral-900/40'}`} onClick={() => { setSelectedId(question.id); setSelectedCardId(null); }}>
                ...
                <td className="max-w-[420px] px-3 py-3"><StudyMarkdownInline className="line-clamp-2 font-medium text-neutral-200" content={question.prompt} /><span className="mt-1 block text-[10px] text-neutral-600">{question.shortId}{question.favorite ? ` · ${t('Favorita')}` : ''}</span></td><td className="whitespace-nowrap px-3 py-3">{t(TYPE_LABELS[question.type])}</td><td className="whitespace-nowrap px-3 py-3">{t(DIFFICULTY_LABELS[question.difficulty])}</td><td className="whitespace-nowrap px-3 py-3"><span className="rounded-full bg-teal-900/30 px-2 py-1 text-[10px] text-teal-300">{t(STATUS_LABELS[question.status])}</span></td><td className="max-w-40 truncate px-3 py-3 text-neutral-400">{subject?.name ?? '—'}</td><td className="max-w-48 truncate px-3 py-3 text-neutral-400">{question.source.title || t('Sin fuente enlazada')}</td><td className="whitespace-nowrap px-3 py-3 text-neutral-500">{new Date(question.createdAt).toLocaleDateString()}</td>
              </tr>;
        ...
        {tab === 'questions' && <main className="study-question-detail mt-4 rounded-xl border border-neutral-800 bg-white p-4 dark:bg-transparent" data-testid="study-question-detail">{selected ? <>
          ...
          <div className="mt-3 text-lg font-medium leading-7"><StudyMarkdown content={selected.prompt} /></div>
          ...
          {selected.options.length > 0 && <div className="mt-4 space-y-2">{selected.options.map((option) => {
            ...
            return <div key={option.id} className={`flex items-center rounded-lg border px-3 py-2 text-sm ${option.correct ? 'border-emerald-800 bg-emerald-950/20 text-emerald-200' : 'border-neutral-800 text-neutral-400'}`}><span className="min-w-0 flex-1"><StudyMarkdownInline content={option.text} /></span><span className="ml-auto pl-2 text-[10px] text-neutral-600">{selection?.selectedCount ?? 0} {t('selecciones')}</span></div>;
          })}</div>}
          <section className="study-question-answer-card mt-5 rounded-xl border border-neutral-800 bg-neutral-900/30 p-4"><h3 className="text-xs font-semibold uppercase tracking-wider text-neutral-500">{t('Respuesta y explicación')}</h3><div className="mt-2 text-sm text-emerald-200"><StudyMarkdown content={selected.answer.text ?? String(selected.answer.value ?? '')} /></div><div className="mt-2 text-sm text-neutral-400"><StudyMarkdown content={selected.explanation} /></div></section>
          {(selected.lastAnsweredAt || selected.lastResponse || selected.lastScore != null) && <section className="mt-3 rounded-xl border border-indigo-900/50 bg-indigo-950/15 p-4"><div className="flex items-center gap-2"><h3 className="text-xs font-semibold uppercase tracking-wider text-indigo-300">{t('Último intento')}</h3>{selected.lastAnsweredAt && <time className="ml-auto text-[10px] text-neutral-600">{new Date(selected.lastAnsweredAt).toLocaleString()}</time>}</div>{selected.lastResponse ? <div className="mt-2 text-sm text-neutral-300"><StudyMarkdown content={selected.lastResponse} /></div> : <p className="mt-2 text-sm text-neutral-300">{t('Sin respuesta guardada')}</p>}<div className="mt-3 flex items-center gap-3"><strong className="text-lg text-indigo-200">{selected.lastScore == null ? '—' : `${selected.lastScore.toFixed(2)} / ${(selected.lastMaxScore ?? 0).toFixed(2)}`}</strong><span className="text-xs text-neutral-500">{t('Última calificación')}</span></div>{selected.lastFeedback && <div className="mt-2 text-sm text-neutral-400"><StudyMarkdown content={selected.lastFeedback} /></div>}</section>}
          <section className="mt-3 rounded-xl border border-neutral-800 p-4"><div className="flex items-center gap-2"><h3 className="text-xs font-semibold uppercase tracking-wider text-neutral-500">{t('Fuente justificativa')}</h3>{(selected.documentId || selected.materialId || selected.recordingId) && <button className="btn btn-ghost ml-auto h-7 text-xs" onClick={() => openSource(selected)}><Icon name="external" size={11} />{t('Abrir fuente')}</button>}</div><p className="mt-2 text-xs text-neutral-500">{selected.source.title}</p><blockquote className="mt-2 border-l-2 border-teal-800 pl-3 text-sm leading-6 text-neutral-300"><StudyMarkdown content={selected.source.excerpt ?? ''} /></blockquote></section>
      ...
      {(selectedCard || cardDraft) && createPortal(<div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/70 p-4" data-testid="study-bank-flashcard-modal" role="dialog" aria-modal="true" aria-labelledby="study-bank-flashcard-title" onMouseDown={(event) => { if (event.target === event.currentTarget) { setSelectedCardId(null); setCardDraft(null); setCardDraftId(null); } }}>
        <div className="card max-h-[90vh] w-full max-w-3xl overflow-y-auto p-5 shadow-2xl">
          {cardDraft ? <section data-testid="study-bank-flashcard-editor">
            ...
            <section className="mt-5 rounded-2xl border border-neutral-200 bg-neutral-50 p-7 text-center shadow-sm dark:border-neutral-800 dark:bg-neutral-900/45"><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-teal-600 dark:text-teal-400">{t('Anverso')}</p><div id="study-bank-flashcard-title" className="mt-4 text-xl font-semibold leading-8"><StudyMarkdown content={selectedCard.front} /></div><div className="mx-auto my-6 h-px max-w-md bg-neutral-200 dark:bg-neutral-700"/><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-emerald-600 dark:text-emerald-400">{t('Reverso')}</p><div className="mt-4 text-lg leading-7 text-emerald-700 dark:text-emerald-300"><StudyMarkdown content={selectedCard.back} /></div>{selectedCard.hint && <div className="mt-5 text-sm text-neutral-500">{t('Pista')}: <StudyMarkdownInline content={selectedCard.hint} /></div>}</section>
            {selectedCard.sourceExcerpt && <blockquote className="mt-4 rounded-xl border border-neutral-200 border-l-4 border-l-teal-500 bg-neutral-50 p-4 text-sm leading-6 text-neutral-600 dark:border-neutral-800 dark:bg-neutral-900/30 dark:text-neutral-400"><StudyMarkdown content={selectedCard.sourceExcerpt} /></blockquote>}
