      ...
      <section className="mx-auto max-w-7xl overflow-x-auto rounded-xl border border-neutral-200 bg-white dark:border-neutral-800 dark:bg-neutral-950/35">
        {recordings.length ? <table className="w-full min-w-[1050px] border-collapse text-xs" data-testid="study-recordings-table">
          ...
          <tbody>{recordings.map((recording) => <tr key={recording.id} data-testid={`study-recording-${recording.id}`} data-recording-row="" className="cursor-pointer border-b border-neutral-200/70 hover:bg-neutral-50 dark:border-neutral-800/60 dark:hover:bg-neutral-900/40" onClick={() => void open(recording.id)}>
            ...
            <td className="max-w-[130px] truncate px-3 py-2.5 text-neutral-500">{recordingScopeName(recording, workspace, 'course')}</td><td className="max-w-[140px] truncate px-3 py-2.5 text-neutral-500">{recordingScopeName(recording, workspace, 'subject')}</td><td className="max-w-[130px] truncate px-3 py-2.5 text-neutral-500">{recordingScopeName(recording, workspace, 'topic')}</td><td className="whitespace-nowrap px-3 py-2.5 text-neutral-500">{new Intl.DateTimeFormat(undefined, { dateStyle: 'short' }).format(new Date(recording.createdAt))}</td><td className="px-3 py-2.5 text-neutral-500">{formatStudyTimestamp(recording.durationSeconds)}</td><td className="px-3 py-2.5"><span className="rounded-full border border-neutral-300 px-2 py-1 text-[10px] text-neutral-600 dark:border-neutral-700 dark:text-neutral-400">{t(STATUS_LABELS[recording.processingStatus])}</span></td><td className="px-3 py-2.5 text-neutral-500">{bytesLabel(recording.sizeBytes)}</td>
