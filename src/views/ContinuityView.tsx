  ...
  return (
    ...
    >
      <Icon
        ...
        size={13}
        className={`mt-0.5 shrink-0 ${serious ? 'text-red-400' : item.severity === 'warning' ? 'text-amber-400' : 'text-neutral-600'}`}
      />
      <span className="min-w-0 flex-1">
        <span className="block truncate text-sm text-neutral-100">{findingText(item.headline)}</span>
        {!compact && (
  ...
  return (
    <div className="space-y-5 p-6" data-testid="continuity-sheet">
      <div>
        ...
        {finding.detail && <p className="mt-1 text-xs text-neutral-500">{findingText(finding.detail)}</p>}
      </div>

      <section className={PERSON_DOSSIER_SECTION_CLASS} data-testid="continuity-subjects">
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-neutral-500">{t('Lo que choca')}</h3>
        <ul className="space-y-1">
          {finding.subjects.map((subject) => (
            <li key={`${subject.kind}:${subject.id}:${subject.field ?? ''}`}>
              ...
              >
                <span className="min-w-0 flex-1 truncate text-neutral-200">{subject.title}</span>
                {SECTION_OF_KIND[subject.kind] && <Icon name="chevronRight" size={12} className="text-neutral-600" />}
              </button>
            </li>
          ))}
        </ul>
      ...
      {check && (
        <section className={PERSON_DOSSIER_SECTION_CLASS}>
          <h3 className="mb-1 text-xs font-semibold uppercase tracking-wider text-neutral-500">{t('Por qué lo digo')}</h3>
          <p className="text-xs leading-5 text-neutral-400">{t(check.explains)}</p>
        </section>
      )}
      ...
      {finding.checkId === 'coverage.undatedScenes' && (
        <button className="btn btn-ghost w-full border border-neutral-700 text-xs" onClick={() => void recompute()}>
          {t('Recalcular los días desde la cadena')}
        </button>
      )}

      <section className={PERSON_DOSSIER_SECTION_CLASS} data-testid="continuity-mute">
        <h3 className="mb-1 text-xs font-semibold uppercase tracking-wider text-neutral-500">{t('Está bien así')}</h3>
...
function NothingWrong() {
  ...
  return (
    <div data-testid="continuity-empty">
      <p className="text-sm text-neutral-300">{t('Sin contradicciones.')}</p>
      {summary && (
...
function BlindSpots() {
  ...
  return (
    <section className="mt-3 border-t border-neutral-800 pt-2" data-testid="continuity-blindspots">
      <h4 className="text-[11px] font-semibold uppercase tracking-wider text-neutral-600">
        {t('Lo que todavía no puedo comprobar')}
      </h4>
      <ul className="mt-1 space-y-0.5">
        {gaps.map((gap) => (
          <li key={gap.fingerprint} className="text-[11px] leading-4 text-neutral-500">
            · {findingText(gap.headline)}
            {gap.detail && <span className="block pl-2 text-[10px] text-neutral-600">{findingText(gap.detail)}</span>}
          </li>
        ))}
      </ul>
    </section>
    ...
    >
      <section className="card-modal w-full max-w-2xl p-5" role="dialog" aria-modal="true" data-testid="exceptions-modal">
        <div className="mb-3 flex items-start gap-3">
          <h3 className="min-w-0 flex-1 text-base font-semibold text-neutral-100">{t('Excepciones aceptadas')}</h3>
          <button className="btn btn-ghost h-8 w-8 shrink-0 p-0 text-neutral-400" aria-label={t('Cerrar')} onClick={onClose}>
            <Icon name="x" size={15} />
          </button>
        </div>
        {mutes.length === 0 ? (
          <p className="text-sm text-neutral-500">{t('Todavía no has aceptado ninguna excepción.')}</p>
        ) : (
          <ul className="max-h-96 space-y-1 overflow-y-auto">
            {mutes.map((mute) => (
              <li key={mute.fingerprint} className="flex items-start gap-2 rounded border border-neutral-800 p-2">
                <span className="min-w-0 flex-1">
                  {/* The headline as it READ when it was silenced. A list of fingerprints
                      six months later is unreadable. */}
                  <span className="block truncate text-xs text-neutral-200">{mute.headline ?? mute.fingerprint}</span>
                ...
                <button
                  className="btn btn-ghost shrink-0 border border-neutral-700 px-2 text-[11px]"
                  onClick={async () => {
