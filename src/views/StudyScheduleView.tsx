        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1050px] table-fixed border-collapse" data-testid="study-schedule-table">
            ...
            {(['morning', 'afternoon'] as const).map((section) => <tbody key={section}>{periods(section).map((period, index) => <tr key={period.id} className={section === 'afternoon' && index === 0 ? 'border-t-4 border-t-neutral-700' : ''}>
              <th className="border-b border-r border-neutral-800 bg-neutral-900/75 p-3 text-left align-top"><div className="flex items-start gap-2"><span className={`mt-1 rounded-full px-2 py-0.5 text-[9px] font-semibold uppercase ${section === 'morning' ? 'bg-amber-500/15 text-amber-300' : 'bg-indigo-500/15 text-indigo-300'}`}>{t(SECTION_LABELS[section])}</span><div className="min-w-0 flex-1"><input className="w-full border-0 bg-transparent text-sm font-semibold text-neutral-200 outline-none" value={periodLabel(period)} onChange={(event) => updatePeriod(period.id, { label: event.target.value })} onBlur={() => updatePeriod(period.id, {}, true)} /><div className="mt-2 flex items-center gap-1"><input aria-label={t('Hora de inicio')} className="input h-8 min-w-0 flex-1 px-2 text-xs" type="time" value={period.startTime} onChange={(event) => updatePeriod(period.id, { startTime: event.target.value })} onBlur={() => updatePeriod(period.id, {}, true)} /><span className="text-neutral-600">–</span><input aria-label={t('Hora de fin')} className="input h-8 min-w-0 flex-1 px-2 text-xs" type="time" value={period.endTime} onChange={(event) => updatePeriod(period.id, { endTime: event.target.value })} onBlur={() => updatePeriod(period.id, {}, true)} /></div></div><button className="mt-1 text-neutral-600 hover:text-red-400" title={t('Eliminar franja')} onClick={() => removePeriod(period.id)}><Icon name="trash" size={13} /></button></div></th>
