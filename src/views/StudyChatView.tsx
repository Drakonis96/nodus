...
export function StudyChatView({ settings, onOpenDocument, onOpenMaterial, onOpenRecording, initialPrompt, variant = 'study' }: {
  ...
  // still rejects keeps the partial answer instead of showing a generation error.
  ...
  const resetChat = () => { if (busy) return; setConversation(null); setInput(''); setReasoning(''); setError(''); };
  ...
  const send = async (value = input) => {
    ...
    let streamed = '';
    try {
      const response = await window.nodus.streamStudyAssistant({ conversationId: active.id, messages: base, selection, task: 'answer', level: 'standard', tone: 'clear', language: 'auto', allowExternalKnowledge: false, model }, {
        onDelta: (delta) => { streamed += delta; setConversation((current) => current ? { ...current, messages: current.messages.map((item) => item.id === assistant.id ? { ...item, content: item.content + delta } : item) } : current); },
      ...
      if (stopRequestedRef.current) {
        ...
        // of replacing the whole answer with the cancellation error. A stop with
        // nothing streamed yet drops the empty placeholder turn.
        const finalAssistant = { ...assistant, content: streamed, interrupted: Boolean(streamed.trim()) };
        const next = await window.nodus.updateStudyAssistantConversation(active.id, { messages: streamed.trim() ? [...base, finalAssistant] : base, selection, model });
        if (next) setConversation(next);
      } else {
  ...
  return <div className="flex h-full min-h-0 bg-neutral-50 text-neutral-900 dark:bg-neutral-950 dark:text-neutral-100" data-testid="study-chat-view">
    ...
    <main className="flex min-w-0 flex-1 flex-col">
      ...
      <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto px-5 py-6"><div className="mx-auto flex max-w-3xl flex-col gap-4">
        ...
        {conversation?.messages.map((item) => item.role === 'user' ? <div key={item.id} className="ml-auto max-w-[82%] rounded-2xl rounded-br-sm bg-teal-600 px-4 py-3 text-sm leading-6 text-white">{item.content}</div> : <article key={item.id} className={`mr-auto max-w-[95%] rounded-2xl border bg-white px-4 py-3 shadow-sm dark:bg-neutral-900/45 ${item.error ? 'border-red-300 dark:border-red-900' : 'border-neutral-200 dark:border-neutral-800'}`} data-testid="study-chat-assistant-message"><ChatMarkdown streaming={busy && item.id === conversation.messages[conversation.messages.length - 1]?.id} content={item.content || (busy ? '…' : '')} verify={false} onStudyEvidence={(id) => { const citation = item.citations?.find((entry) => entry.id === id); if (citation) openCitation(citation); }} />{item.citations?.length ? <div className="mt-3 flex flex-wrap gap-1.5 border-t border-neutral-200 pt-3 dark:border-neutral-800">{item.citations.map((citation) => <button key={citation.id} className="rounded-full border border-teal-200 px-2 py-1 text-[10px] text-teal-700 hover:border-teal-500 dark:border-teal-900 dark:text-teal-300" onClick={() => openCitation(citation)}>{citation.id} · {citation.title}</button>)}</div> : null}{item.interrupted ? <ChatAbortedNotice /> : null}</article>)}
