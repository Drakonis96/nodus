...
export function DocumentIndexManager({ vaultId, onClose }: { vaultId: string | null; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/65 p-4 sm:p-8" role="dialog" aria-modal="true" aria-label={t('Índice documental')} onClick={onClose}>
    <div className="card flex max-h-full w-full max-w-4xl flex-col overflow-hidden border border-neutral-700 bg-neutral-950" onClick={(event) => event.stopPropagation()}>
      ...
      <main className="min-h-0 flex-1 overflow-y-auto p-5">
        ...
        {live && <section className="mt-4 rounded-lg border border-indigo-800/60 bg-indigo-950/20 p-4"><div className="flex items-center justify-between gap-3"><div><div className="text-sm font-medium">{live.mode === 'continuous' ? t('Indexación continua') : t('Campaña documental')}</div><div className="text-xs text-neutral-500">{tx('{done} de {total} obras · {status}', { done: live.completedJobs, total: live.totalJobs, status: campaignStatusLabel(live.status) })}</div></div><div className="flex gap-2">{live.status === 'paused' ? <button className="btn btn-primary" disabled={busy} onClick={() => void setStatus('running')}>{t('Reanudar')}</button> : <button className="btn btn-ghost border border-neutral-700" disabled={busy} onClick={() => void setStatus('paused')}>{t('Pausar')}</button>}<button className="btn btn-ghost border border-red-800 text-red-300" disabled={busy} onClick={() => setConfirmCancel(true)}>{t('Cancelar')}</button></div></div><div className="mt-3 h-2 overflow-hidden rounded bg-neutral-800"><div className="h-full bg-indigo-500" style={{ width: `${live.estimatedUnits ? Math.round((live.completedUnits / live.estimatedUnits) * 100) : 0}%` }} /></div></section>}
