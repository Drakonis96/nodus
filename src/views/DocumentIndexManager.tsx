...
export function DocumentIndexManager({ vaultId, onClose }: { vaultId: string | null; onClose: () => void }) {
  ...
  const jobs = useMemo(
    () => progress.jobs
      .filter((item) => !vaultId || item.vaultId === vaultId)
      .sort(compareDocumentIndexJobsForDisplay),
    [progress, vaultId],
  );
  const liveCampaigns = campaigns.filter((item) => ['queued', 'running', 'paused'].includes(item.status));
  const live = liveCampaigns[0] ?? null;
  ...
  const campaignIds = new Set(liveCampaigns.map((campaign) => campaign.campaignId));
  const standalone = jobs.filter((job) => !job.campaignId || !campaignIds.has(job.campaignId));
  // The job payload is deliberately bounded for large vaults. Campaign counters
  // remain exact, so the cards must never report the 120-row transport cap as the queue.
  const active = liveCampaigns.reduce((sum, campaign) => sum + campaign.runningJobs, 0)
    + standalone.filter((job) => job.status === 'running').length;
  const queued = liveCampaigns.reduce((sum, campaign) => sum + campaign.queuedJobs + campaign.pausedJobs, 0)
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/65 p-4 sm:p-8" role="dialog" aria-modal="true" aria-label={t('Índice documental')} onClick={onClose}>
    <div className="card-modal flex max-h-full w-full max-w-4xl flex-col overflow-hidden" onClick={(event) => event.stopPropagation()}>
      ...
      <main className="min-h-0 flex-1 overflow-y-auto p-5">
        ...
        <section className="mt-5"><h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-neutral-500">{t('Actividad reciente')}</h3><div className="space-y-1">{jobs.slice(0, 30).map((job) => <div key={job.jobId} className="flex items-center gap-3 rounded border border-neutral-800 px-3 py-2 text-xs" data-testid={`document-index-manager-job-${job.jobId}`}><span className="min-w-0 flex-1 truncate text-neutral-300">{job.title ?? job.nodusId}</span><span className="text-neutral-500">{jobPhaseDetail(job)}</span><span className="w-10 text-right tabular-nums text-neutral-500">{Math.round(job.progress * 100)}%</span><span className={job.status === 'failed' || job.status === 'unavailable' ? 'text-red-300' : job.status === 'completed' ? 'text-emerald-300' : 'text-indigo-300'}>{jobStatusLabel(job.status)}</span></div>)}{jobs.length === 0 && <p className="py-6 text-center text-xs text-neutral-600">{t('Todavía no hay actividad documental.')}</p>}</div></section>
      </main>
    </div>
...
function CampaignCard({ live, liveError, busy, setStatus, onCancel }: {
  live: DocumentIndexProgress['campaigns'][number];
  liveError: string | null;
  busy: boolean;
  setStatus: (status: 'running' | 'paused' | 'cancelled') => Promise<void>;
  onCancel: () => void;
}) {
  const fraction = live.estimatedUnits ? live.completedUnits / live.estimatedUnits : 0;
  const percent = documentIndexPercentLabel(fraction);
  const visiblePercent = Math.max(fraction * 100, live.runningJobs > 0 ? 0.2 : 0);
  return <section className="mt-4 rounded-lg border border-indigo-800/60 bg-indigo-950/20 p-4">
    <div className="flex items-center justify-between gap-3">
      <div className="min-w-0">
        <div className="text-sm font-medium">{live.mode === 'continuous' ? t('Indexación continua') : t('Campaña documental')}</div>
        <div className="mt-0.5 flex flex-wrap items-center gap-x-2 text-xs text-neutral-500">
          <span>{tx('{done} de {total} obras · {status}', { done: live.completedJobs, total: live.totalJobs, status: campaignStatusLabel(live.status) })}</span>
          <span>· {live.runningJobs} {t('En curso')}</span>
          <span>· {live.queuedJobs + live.pausedJobs} {t('En cola')}</span>
        </div>
      </div>
    ...
    <div className="mt-3 flex items-center gap-3">
      ...
      <span className="w-12 shrink-0 text-right text-xs font-medium tabular-nums text-indigo-300">{percent}</span>
    </div>
  </section>;
}
...
function jobPhaseDetail(job: DocumentIndexProgress['jobs'][number]): string {
  const phase = jobPhaseLabel(job.phase);
