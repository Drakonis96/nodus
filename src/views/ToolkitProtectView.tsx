...
function SecondaryButton({ children, onClick, disabled, icon, className = '' }: { children: React.ReactNode; onClick: () => void; disabled?: boolean; icon?: string; className?: string }) {
  return (
    <button type="button" disabled={disabled} onClick={onClick} className={`inline-flex h-9 items-center justify-center gap-2 rounded-lg border border-neutral-300 bg-white px-3 text-sm font-medium text-neutral-700 hover:bg-neutral-100 disabled:cursor-not-allowed disabled:opacity-50 dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-200 dark:hover:bg-neutral-800 ${className}`}>
    ...
    <div data-testid="protect-redact" className="mx-auto max-w-[1500px] space-y-4">
      ...
      <div className="grid gap-4 xl:grid-cols-[280px_minmax(0,1fr)]">
        <aside className="space-y-4 rounded-xl border border-neutral-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-900/40">
          <div className="grid grid-cols-2 gap-2">{(Object.keys(TOOL_LABELS) as ProtectEditorTool[]).map((value) => <button key={value} type="button" onClick={() => setTool(value)} className={`h-9 rounded-lg border px-2 text-xs font-medium ${tool === value ? 'border-amber-500 bg-amber-50 text-amber-800 dark:bg-amber-950/40 dark:text-amber-200' : 'border-neutral-200 dark:border-neutral-700'}`}>{t(TOOL_LABELS[value])}</button>)}</div>
          {tool === 'brush' && <Range label={t('Grosor de barra')} value={brush} min={8} max={100} step={1} onChange={setBrush} suffix=" px" />}
          ...
          {tool === 'select' && <div className="grid gap-2"><SecondaryButton className="w-full" onClick={() => editor.current?.deleteSelected()} disabled={!selected} icon="trash">{t('Eliminar selección')}</SecondaryButton><SecondaryButton className="w-full" onClick={() => void copyAll()} disabled={!selected || state.pages.length < 2 || pageLoading} icon="copy">{t('Copiar a todas las páginas')}</SecondaryButton></div>}
          {tool === 'crop' && <div className="grid gap-2"><SecondaryButton className="w-full" onClick={() => editor.current?.applyCrop()} disabled={!crop} icon="check">{t('Aplicar recorte')}</SecondaryButton><SecondaryButton className="w-full" onClick={() => editor.current?.clearCrop()} disabled={!crop} icon="x">{t('Borrar recorte')}</SecondaryButton><p className="text-xs text-neutral-500">{t('El recorte mínimo es de 24 × 24 px y vacía el historial de esta página.')}</p></div>}
          <div className="border-t border-neutral-200 pt-4 dark:border-neutral-800"><p className="mb-2 text-xs font-semibold uppercase tracking-wide text-neutral-500">{t('Transformar página')}</p><div className="grid grid-cols-2 gap-2"><SecondaryButton onClick={() => rotate(-1)} icon="rotateCcw">{t('Izquierda')}</SecondaryButton><SecondaryButton onClick={() => rotate(1)} icon="rotateCw">{t('Derecha')}</SecondaryButton></div><Range label={t('Enderezar')} value={straighten} min={-10} max={10} step={0.5} onChange={(value) => editor.current?.setStraightenPreview(value)} suffix="°" /></div>
          ...
          <div className="grid grid-cols-2 gap-2 border-t border-neutral-200 pt-4 dark:border-neutral-800"><SecondaryButton onClick={() => editor.current?.undo()} disabled={!page?.undo.length} icon="undo">{t('Deshacer')}</SecondaryButton><SecondaryButton onClick={() => editor.current?.zoomButton(1.25)} icon="plus">{t('Zoom')}</SecondaryButton><SecondaryButton onClick={() => editor.current?.zoomButton(0.8)} icon="minus">{t('Alejar')}</SecondaryButton><SecondaryButton onClick={() => editor.current?.resetView()} icon="fit">{t('Ajustar')}</SecondaryButton></div>
        </aside>
