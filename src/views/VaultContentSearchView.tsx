...
export function VaultContentSearchView({ options, onOpen, testId }: {
  ...
  return <div className="global-search-workspace theme-workspace-surface flex h-full min-h-0 flex-col p-6" data-testid={testId}>
    <div className="mx-auto w-full max-w-3xl shrink-0">
      <div className="mb-4 flex items-center gap-3"><Icon name="search" size={22} className="global-search-accent" /><h1 className="text-xl font-semibold">{t('Búsqueda global')}</h1></div>
    ...
    <main className="mt-4 min-h-0 flex-1 overflow-y-auto"><div className="mx-auto w-full max-w-3xl space-y-1">
      ...
      {results.map((hit) => {
        ...
        return <button key={`${hit.kind}:${hit.id}`} onClick={() => onOpen(hit)} className="global-search-result flex w-full items-start gap-3 rounded-md px-3 py-2 text-left">
