          ...
          {!profile ? (
            ...
            <section>
              ...
              <div className="space-y-2">{profile.sections.map((section) => <article key={section.sectionId} className="rounded-lg border border-neutral-800 px-3 py-2" style={{ marginLeft: `${Math.min(3, Math.max(0, section.level - 1)) * 12}px` }}>
                <div className="flex items-center gap-2"><span className="text-sm font-medium text-neutral-200">{section.title}</span>{section.role && <span className="rounded bg-neutral-800 px-1.5 py-0.5 text-[10px] text-neutral-500">{section.role}</span>}<span className="ml-auto text-[11px] text-neutral-600">{[section.pageStart, section.pageEnd].filter(Boolean).join('–')}</span></div>
