  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden" data-testid="database-deep-research">
      ...
      <SectionToolbar testId="database-deep-research-toolbar">
        <div className="relative min-w-56 flex-1"><Icon name="search" size={14} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-neutral-500" /><input className="input h-9 w-full pl-8" value={reportSearch} onChange={(event) => { setReportSearch(event.target.value); onSnapshotChange?.({ search: event.target.value }); }} placeholder={t("Buscar entre tus informes…")} aria-label={t("Buscar entre tus informes")} /></div>
