...
export const DEEP_RESEARCH_VERSION_TRANSLATIONS = {
  ...
  ko: table([
    "시스템 버전",
    "v1 · 단순 검색(기본값)",
    "코퍼스에서 이미 추출된 아이디어와 구절을 사용하고 전체 문서 분석을 시작하지 않습니다. 간단한 쿼리에 권장되며 일반적으로 더 적은 수의 토큰을 사용합니다.",
    "v2 · 확장된 분석(더 많은 토큰)",
    "더 많은 토큰을 사용합니다. 학술 저장소에서는 아이디어와 관계에서 시작하여 최대 8개의 관련 전체 문서를 분석할 수 있습니다. 완전한 프로필이 없는 문서를 분석하고, 오래된 프로필을 다시 작성하고, 최신 프로필을 재사용합니다. 첫 번째 실행에는 상당한 비용이 발생할 수 있습니다.",
    "'새 보고서'를 누르고 질문이나 아이디어를 입력하세요. 보고서는 미리 설정된 길이 없이 코퍼스가 지원할 수 있는 기여만 개발합니다.",
    "“새 장치”를 누르고 주제를 입력하세요. Nodus는 사전 설정된 길이 없이 귀하의 재료가 지원할 수 있는 부품을 개발합니다.",
  ]),
  ja: table([
    "システムバージョン",
    "v1 · 単純な取得 (デフォルト)",
