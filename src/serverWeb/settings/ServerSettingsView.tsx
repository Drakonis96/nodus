...
function blankProfile(theme: 'dark' | 'light', preferences: AIPreferences = {}): PortableProfileValues {
  ...
  return {
    schemaVersion: 1,
    appearance: {
      theme, uiLanguage: 'es', promptLanguage: 'es', animationSpeed: 1, interfaceScale: 1,
...
function Section({ title, description, children, testId }: { title: string; description?: string; children: React.ReactNode; testId?: string }) {
  return <section className="ss-card" data-testid={testId}><header className="ss-section-head"><h2>{title}</h2>{description && <p>{description}</p>}</header>{children}</section>;
}

function Row({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) {
  return <div className="ss-row"><div><strong>{label}</strong>{hint && <p>{hint}</p>}</div><div className="ss-control">{children}</div></div>;
}

function Switch({ checked, onChange, label }: { checked: boolean; onChange: (next: boolean) => void; label: string }) {
  return <button type="button" role="switch" aria-checked={checked} aria-label={label} className="ss-switch" data-on={checked} onClick={() => onChange(!checked)}><span /></button>;
}
...
export function ServerSettingsView({ csrfToken, isAdmin, theme, initialTab, onThemeChange }: ServerSettingsViewProps) {
  const requested = initialTab || new URLSearchParams(location.search).get('tab') || 'providers';
  const [tab, setTab] = useState<TabId>(TABS.some((entry) => entry.id === requested) ? requested as TabId : 'providers');
  ...
  useEffect(() => {
    const onPopState = () => {
      const next = new URLSearchParams(location.search).get('tab') || 'providers';
  ...
  const createSpace = async (event: FormEvent) => {
    ...
    await runAdmin(() => api.createAdminSpace(newSpace, adminCsrf), 'Vault creado. Ya puedes generar el código de conexión.');
  ...
  const visibleTabs = normalizedQuery ? TABS.filter((entry) => `${entry.label} ${entry.keywords}`.toLocaleLowerCase().includes(normalizedQuery)) : TABS.filter((entry) => entry.id === tab);
  if (!profile) return <div className="server-settings-native ss-loading">Cargando ajustes…</div>;
  ...
  const serverPanel = <>
    ...
    <Section title="Vaults publicados"><div className="ss-admin-list">{(admin?.spaces || me?.spaces || []).map((space) => {
      ...
      return <article className="ss-admin-item" key={space.id}><div className="ss-admin-item-head"><div><strong>{space.name}</strong><small>{String(space.vaultType || space.vault?.type || 'academic')} · {space.updatedAt ? new Date(space.updatedAt).toLocaleString() : 'Sin publicar'}</small></div>{isAdmin && <button className="ss-button secondary" type="button" onClick={async () => { setBusy('admin'); try { const result = await api.createAdminPairing(space.id, adminCsrf); setPairing(result); setNotice('Código de conexión creado.'); } catch (next) { setError(errorMessage(next)); } finally { setBusy(''); } }}>Conectar Desktop</button>}</div>{isAdmin && <div className="ss-policy-grid">{[['allowLibraryDocuments','Biblioteca'],['allowPassages','Pasajes'],['allowUserContent','Notas y proyectos'],['allowVectors','Índice semántico'],['allowPrimarySources','Fuentes primarias'],['allowTestimonies','Testimonios'],['allowPersonalImports','Anotaciones privadas']].map(([field, label]) => <label key={field}><Switch label={label} checked={policy[field] === true} onChange={(value) => void runAdmin(() => api.updateAdminSpace(space.id, { publicationPolicy: { [field]: value } }, adminCsrf), `Política de ${space.name} actualizada.`)} />{label}</label>)}</div>}</article>;
  ...
  return <div className="server-settings-native" data-theme={theme} data-testid="settings-view"><header className="ss-page-head"><div><h1>Ajustes</h1><p>Busca un ajuste o entra por una sección temática.</p><small>Nodus v{admin?.server.version || me?.server?.version || '—'}</small></div><label className="ss-search"><Icon name="search" size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar en ajustes…" /></label></header><nav className="ss-tabs" aria-label="Secciones de ajustes">{TABS.map((entry) => <button type="button" key={entry.id} data-testid={`settings-tab-${entry.id}`} className={tab === entry.id && !normalizedQuery ? 'active' : ''} onClick={() => selectTab(entry.id)}><Icon name={entry.icon} size={14} />{entry.label}</button>)}</nav>{error && <div className="ss-error" role="alert">{error}</div>}{notice && <div className="ss-notice" role="status">{notice}</div>}<main className="ss-panels">{visibleTabs.length ? visibleTabs.map((entry) => <div key={entry.id}>{normalizedQuery && <h2 className="ss-result-title">{entry.label}</h2>}{panels[entry.id]}</div>) : <div className="ss-empty">No hay ajustes que coincidan con la búsqueda.</div>}</main></div>;
}
