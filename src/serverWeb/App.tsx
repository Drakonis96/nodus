...
const VIEW_COLLECTIONS: Partial<Record<View, CollectionMeta>> = {
  ...
  testimonyParticipants: { collection: 'testimony-participants', label: 'Participantes', icon: 'users' },
...
const SERVER_TOOL_VIEWS = new Set<View>(['browser', 'radar', 'compass', 'toolkit']);
...
function routeFromLocation(): Route {
  ...
  if (parts[0] === 'view' && parts[1]) {
    ...
    return { kind: 'view', view: requested === 'writing' || requested === 'projects' ? 'workspace' : requested };
  }
...
function viewPath(view: ServerView): string {
  const canonical = view === 'writing' || view === 'projects' ? 'workspace' : view;
...
function visibleNav(type: VaultType): NavItem[] {
  ...
  return NAV_ITEMS.filter((item) => !SERVER_TOOL_VIEWS.has(item.id) && (item.id === 'home' || item.id === 'settings' || allowed.has(item.id)));
}
...
function searchDetailView(collection: string, type: VaultType): View | null {
  ...
  if (key.startsWith('world_groups') || key === 'world-groups') return 'factions';
  if (key.startsWith('world_scenes') || key === 'world-scenes') return 'scenes';
  if (key.startsWith('world_articles') || key === 'world-articles') return 'encyclopedia';
  if (key.startsWith('world_threads') || key === 'world-threads') return 'arcs';
  if (key.startsWith('world_rules') || key === 'world-rules') return 'rules';
  if (key.startsWith('world_questions') || key === 'world-questions') return 'questions';
  if (key === 'study_courses' || key === 'study-courses') return 'studyCourses';
  if (key === 'study_materials' || key === 'study-materials') return 'studyLibrary';
  if (key === 'study_questions' || key === 'study-questions') return 'studyQuestions';
  if (key === 'study_ideas' || key === 'study-ideas') return 'studyIdeas';
  if (key === 'teaching_exams' || key === 'teaching-exams') return 'teachingExams';
  if (key === 'teaching_rubrics' || key === 'teaching-rubrics') return 'teachingRubrics';
  if (key === 'archive_items' || key === 'archive-items') return type === 'prosopography' ? 'prosopSources' : 'archive';
  if (['archive_repositories', 'archive-repositories', 'archive_description_units', 'archive-units', 'archive_excerpts', 'archive-excerpts', 'archive_source_analyses', 'source-analyses'].includes(key)) return type === 'prosopography' ? 'prosopSources' : 'archive';
  if (key === 'testimony_interviews' || key === 'testimony-interviews') return 'testimonyInterviews';
  if (['testimony_transcripts', 'testimony-transcripts', 'testimony_codes', 'testimony-codes'].includes(key)) return 'testimonyInterviews';
  if (key === 'testimony_contrasts' || key === 'testimony-contrasts') return 'testimonyContrasts';
...
function ServerHeaderAction({ icon, label, title, onClick, className = '', showLabel = false, dataTestId }: { icon: string; label: string; title?: string; onClick: (event: React.MouseEvent<HTMLButtonElement>) => void; className?: string; showLabel?: boolean; dataTestId?: string }) {
  return <HoverLabelButton icon={icon} label={label} title={title} onClick={onClick} showLabel={showLabel} data-testid={dataTestId} className={`server-header-action btn-ghost h-9 min-h-9 min-w-9 ${className}`} />;
}
...
function ErrorState({ error, onRetry }: { error: unknown; onRetry?: () => void }) {
  ...
  return <div className="m-6 rounded-lg border border-red-800 bg-red-950/30 p-4 text-sm text-red-300" role="alert" data-testid="error-state"><strong>{t(expired ? 'La sesión ha caducado.' : 'No se ha podido cargar esta vista.')}</strong><p className="mt-1 opacity-80">{error instanceof Error ? error.message : String(error)}</p>{expired ? <a href={`/login?next=${encodeURIComponent(location.pathname + location.search)}`} className="btn mt-3">{t('Iniciar sesión')}</a> : onRetry && <button className="btn btn-ghost mt-3" onClick={onRetry}>{t('Reintentar')}</button>}</div>;
}
...
function PublishedWorldHome({ active, summary: _summary, onOpen }: { active: Space; summary: SpaceSummary; onOpen: (view: View) => void }) {
  ...
  return <div className="h-full overflow-y-auto p-6 server-view-padding" data-testid="worldbuilding-overview"><div className="mx-auto max-w-5xl"><HomeIntroCard eyebrow={`${t('Vault de worldbuilding')} · ${t(native ? 'Nativo del servidor' : 'Publicado')}`} title={active.name} description={active.description || t(native ? 'Crea y organiza personajes, lugares y lore directamente en Nodus Server.' : 'Personajes, lugares y lore del mundo publicados para consulta.')} icon="globe" /><div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4" data-testid="overview-metrics"><Metric label={t('Personajes')} value={people.length} onClick={() => onOpen('characters')} /><Metric label={t('Protagonistas')} value={protagonists} onClick={() => onOpen('characters')} /><Metric label={t('Con vida')} value={alive} onClick={() => onOpen('characters')} /><Metric label={t('En la enciclopedia')} value={entries.length} onClick={() => onOpen('encyclopedia')} /></div><section className="mt-6 rounded-xl border border-neutral-200 bg-white dark:border-neutral-800 dark:bg-neutral-900/40"><div className="flex items-center justify-between border-b border-neutral-200 px-4 py-3 dark:border-neutral-800"><h2 className="text-sm font-semibold">{t('Personajes recientes')}</h2><button className="text-xs text-indigo-400" onClick={() => onOpen('characters')}>{t('Ver todos')}</button></div>{recent.length ? <div className="grid gap-2 p-4 sm:grid-cols-2">{recent.map((person) => <button key={String(person.person_id)} className="rounded-lg border border-neutral-200 p-3 text-left hover:border-indigo-300 dark:border-neutral-800" onClick={() => onOpen('characters')}><strong className="block truncate text-sm">{text(person.display_name ?? person.name, 'Personaje')}</strong><span className="mt-1 block line-clamp-2 text-xs text-neutral-500">{text(person.biography ?? person.notes, native ? 'Personaje del servidor' : 'Personaje publicado')}</span></button>)}</div> : <p className="p-6 text-center text-xs text-neutral-500">{t('Todavía no hay personajes.')}{native ? ` ${t('Crea el primero desde Personajes.')}` : ''}</p>}</section></div></div>;
}
...
function PublishedGenealogyHome({ active, summary, onOpen }: { active: Space; summary: SpaceSummary; onOpen: (view: View) => void }) {
  ...
  const tile = (label: string, value: unknown, view: View, icon: string) => <button className="server-record-card flex items-center gap-3" onClick={() => onOpen(view)}><span className="server-record-icon"><Icon name={icon} /></span><span><strong className="block text-lg">{Number(value || 0).toLocaleString()}</strong><small className="text-xs text-neutral-500">{t(label)}</small></span></button>;
  return <div className="h-full overflow-y-auto p-6 server-view-padding" data-testid="genealogy-overview"><div className="mx-auto max-w-5xl"><HomeIntroCard eyebrow={`${t('Vault de genealogía')} · ${t(native ? 'Nativo del servidor' : 'Publicado')}`} title={active.name} description={active.description || t(native ? 'Construye personas, parentescos, acontecimientos y lugares directamente en Server.' : 'Personas, parentescos, acontecimientos y lugares publicados para consulta.')} icon="tree" /><div className="mt-5 grid gap-3 sm:grid-cols-4" data-testid="overview-metrics">{tile('Personas', counts.persons, 'persons', 'users')}{tile('Vínculos de parentesco', counts.relationships, 'tree', 'tree')}{tile('Eventos', counts.events, 'timeline', 'clock')}{tile('Lugares', counts.places, 'map', 'map')}</div><section className="mt-6 grid gap-3 sm:grid-cols-2">{tile(native ? 'Documentos' : 'Documentos publicados', summary.assets, 'archive', 'archive')}<article className="server-record-card flex items-center gap-3"><span className="server-record-icon"><Icon name="lock" /></span><span><strong className="block text-sm">{t('Sugerencias de parentesco')}</strong><small className="text-xs text-neutral-500">{t('Privadas de la cuenta')}</small></span></article></section></div></div>;
}

function Metric({ label, value, onClick }: { label: string; value: number; onClick: () => void }) {
  return <button className="rounded-lg border border-neutral-800 px-3 py-3 text-left hover:border-indigo-700" onClick={onClick}><div className="text-xl font-semibold tabular-nums">{value.toLocaleString()}</div><div className="truncate text-xs text-neutral-500">{label}</div></button>;
}

function Sidebar({ type, activeView, compact, collapsedGroups, sidebarOrder, sidebarHidden, onToggleGroup, onNavigate }: { type: VaultType; activeView: View; compact: boolean; collapsedGroups: Set<string>; sidebarOrder: string[]; sidebarHidden: string[]; onToggleGroup: (id: string) => void; onNavigate: (view: View) => void }) {
  ...
  const specialized = type === 'worldbuilding'
    ? <WorldbuildingSidebar compact={compact} activeView={activeView} sidebarOrder={sidebarOrder} sidebarHidden={[...sidebarHidden, ...SERVER_TOOL_VIEWS]} onNavigate={(view) => onNavigate(view)} />
    : type === 'prosopography'
      ? <ProsopographySidebar compact={compact} activeView={activeView} sidebarOrder={sidebarOrder} sidebarHidden={[...sidebarHidden, ...SERVER_TOOL_VIEWS]} onNavigate={(view) => onNavigate(view)} />
      : type === 'testimonios'
        ? <TestimonySidebar compact={compact} activeView={activeView} sidebarOrder={sidebarOrder} sidebarHidden={[...sidebarHidden, ...SERVER_TOOL_VIEWS]} onNavigate={(view) => onNavigate(view)} />
        : type === 'primary_sources'
          ? <PrimarySourcesSidebar compact={compact} activeView={activeView} sidebarOrder={sidebarOrder} sidebarHidden={[...sidebarHidden, ...SERVER_TOOL_VIEWS]} onNavigate={(view) => onNavigate(view)} />
          : type === 'estudio'
            ? <StudySidebar compact={compact} activeView={activeView} sidebarOrder={sidebarOrder} sidebarHidden={[...sidebarHidden, ...SERVER_TOOL_VIEWS]} onNavigate={(view) => onNavigate(view)} />
            : type === 'docencia'
              ? <TeachingSidebar compact={compact} activeView={activeView} sidebarOrder={sidebarOrder} sidebarHidden={[...sidebarHidden, ...SERVER_TOOL_VIEWS]} onNavigate={(view) => onNavigate(view)} onOpenRoadmap={() => onNavigate('settings')} />
              : null;
  if (specialized) {
    const remaining = type === 'estudio' ? groups.filter((group) => group.id !== 'explore' && group.id !== 'tools') : [];
...
const VAULT_HOME_METRICS: Record<string, Array<[string, string]>> = {
  genealogy: [['persons', 'Personas'], ['places', 'Lugares'], ['events', 'Eventos'], ['relationships', 'Relaciones']],
  prosopography: [],
  primary_sources: [['archive_items', 'Fuentes'], ['archive_description_units', 'Unidades'], ['archive_excerpts', 'Extractos'], ['archive_source_analyses', 'Análisis']],
  testimonios: [['testimony_interviews', 'Entrevistas'], ['testimony_transcripts', 'Transcripciones'], ['testimony_codes', 'Códigos'], ['testimony_contrasts', 'Contrastes']],
  databases: [['db_databases', 'Bases de datos'], ['db_rows', 'Registros'], ['db_views', 'Vistas'], ['db_cells', 'Valores']],
  estudio: [['study_courses', 'Cursos'], ['study_materials', 'Materiales'], ['study_calendar_events', 'Eventos'], ['study_questions', 'Preguntas']],
  docencia: [['teaching_exams', 'Exámenes'], ['teaching_rubrics', 'Rúbricas'], ['study_courses', 'Cursos'], ['study_materials', 'Materiales']],
  worldbuilding: [['persons', 'Personas'], ['places', 'Lugares'], ['world_groups', 'Grupos'], ['world_scenes', 'Escenas']],
};
...
function Home({ active, summary, onOpen, onRefresh }: { active: Space; summary: SpaceSummary; onOpen: (view: View) => void; onRefresh?: () => Promise<void> }) {
  ...
  const metrics = configured.filter(([key]) => counts[key] !== undefined).map(([key, label]) => [t(label), counts[key]] as const);
  return <div className="h-full overflow-y-auto p-6 server-view-padding" data-testid="overview-view"><div className="mx-auto max-w-5xl">
    <div className="relative"><HomeIntroCard eyebrow={`${t(vaultTypeLabel(type))} · ${t(native ? 'Nativo del servidor' : 'Publicado')}`} title={active.name} description={active.description || t(native ? 'Trabaja directamente en esta bóveda sin depender de Nodus Desktop.' : 'Consulta el conocimiento publicado con el mismo espacio de trabajo de Nodus Desktop.')} icon={vaultTypeIcon(type)} />{onRefresh && <button className="btn btn-ghost absolute right-4 top-4 gap-1.5 text-xs" onClick={() => void onRefresh()} data-testid="overview-refresh"><Icon name="sync" size={13} />{t('Actualizar')}</button>}</div>
    <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4" data-testid="overview-metrics">{metrics.map(([label, value]) => <div key={label} className="rounded-lg border border-neutral-800 px-3 py-2"><div className="truncate text-xs text-neutral-500">{label}</div><div className="text-lg font-semibold tabular-nums">{Number(value).toLocaleString()}</div></div>)}{metrics.length === 0 && <div className="rounded-lg border border-neutral-800 px-3 py-2"><div className="text-xs text-neutral-500">{t(type === 'prosopography' ? 'Datos privados' : 'Recursos')}</div><div className="text-lg font-semibold">{type === 'prosopography' ? '—' : (summary.assets || 0)}</div></div>}</div>
    <section className="mt-6"><div className="mb-3 flex items-center justify-between"><div><div className="text-xs font-semibold text-neutral-300">{t('Explorar la bóveda')}</div><p className="mt-1 text-xs text-neutral-600">{t('Las mismas secciones y jerarquía que en Desktop.')}</p></div><span className="rounded-full border border-teal-800/60 bg-teal-950/30 px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-teal-300">{t(native ? 'Editable' : 'Solo lectura')}</span></div><div className="server-record-grid">{cards.map((item) => <button key={item.id} className="server-record-card flex items-center gap-3" onClick={() => onOpen(item.id)} data-testid={`overview-card-${item.id}`}><span className="server-record-icon"><Icon name={item.icon} /></span><span className="min-w-0 flex-1"><strong className="block truncate text-sm text-neutral-200">{t(item.label)}</strong><small className="block truncate text-xs text-neutral-600">{t(native ? 'Abrir sección' : 'Abrir sección publicada')}</small></span><Icon name="chevronRight" size={14} className="text-neutral-700" /></button>)}</div></section>
  </div></div>;
}
...
const COLLECTION_TABLE_COLUMNS: Partial<Record<View, ServerTableColumn[]>> = {
  argument: [{ key: 'label', label: 'Idea' }, { key: 'type', label: 'Tipo', width: '8rem' }, { key: 'statement', label: 'Enunciado' }, { key: 'confidence', label: 'Confianza', width: '7rem' }],
  research: [{ key: 'label', label: 'Tema' }, { key: 'description', label: 'Descripción' }, { key: 'idea_count', label: 'Ideas', width: '6rem' }, { key: 'updated_at', label: 'Actualizado', width: '10rem' }],
  hypothesis: [{ key: 'title', label: 'Hipótesis' }, { key: 'description', label: 'Descripción' }, { key: 'status', label: 'Estado', width: '8rem' }, { key: 'confidence', label: 'Confianza', width: '7rem' }],
  persons: [{ key: 'display_name', label: 'Nombre' }, { key: 'summary', label: 'Resumen' }, { key: 'birth_date', label: 'Nacimiento', width: '9rem' }, { key: 'occupation', label: 'Ocupación', width: '11rem' }],
  characters: [{ key: 'display_name', label: 'Personaje' }, { key: 'summary', label: 'Resumen' }, { key: 'role', label: 'Rol', width: '9rem' }, { key: 'status', label: 'Estado', width: '8rem' }],
  places: [{ key: 'name', label: 'Lugar' }, { key: 'description', label: 'Descripción' }, { key: 'type', label: 'Tipo', width: '9rem' }, { key: 'coordinates', label: 'Coordenadas', width: '10rem' }],
  timeline: [{ key: 'title', label: 'Evento' }, { key: 'description', label: 'Descripción' }, { key: 'start_date', label: 'Inicio', width: '9rem' }, { key: 'end_date', label: 'Fin', width: '9rem' }],
  relations: [{ key: 'label', label: 'Relación' }, { key: 'from_person', label: 'Origen' }, { key: 'to_person', label: 'Destino' }, { key: 'type', label: 'Tipo', width: '9rem' }],
  factions: [{ key: 'name', label: 'Facción' }, { key: 'description', label: 'Descripción' }, { key: 'type', label: 'Tipo', width: '9rem' }, { key: 'status', label: 'Estado', width: '8rem' }],
  cultures: [{ key: 'name', label: 'Cultura' }, { key: 'description', label: 'Descripción' }, { key: 'region', label: 'Región', width: '10rem' }, { key: 'status', label: 'Estado', width: '8rem' }],
  scenes: [{ key: 'title', label: 'Escena' }, { key: 'summary', label: 'Resumen' }, { key: 'status', label: 'Estado', width: '8rem' }, { key: 'updated_at', label: 'Actualizada', width: '10rem' }],
  encyclopedia: [{ key: 'title', label: 'Artículo' }, { key: 'summary', label: 'Resumen' }, { key: 'category', label: 'Categoría', width: '10rem' }, { key: 'updated_at', label: 'Actualizado', width: '10rem' }],
  questions: [{ key: 'title', label: 'Pregunta' }, { key: 'description', label: 'Contexto' }, { key: 'status', label: 'Estado', width: '8rem' }, { key: 'priority', label: 'Prioridad', width: '8rem' }],
};
...
function inferredColumns(view: View, items: JsonRecord[]): ServerTableColumn[] {
  ...
  const preferred = ['title', 'name', 'label', 'display_name', 'description', 'summary', 'type', 'status', 'updated_at'];
  const keys = [...new Set(items.flatMap((item) => Object.keys(item)))]
    .filter((key) => items.some((item) => item[key] != null && typeof item[key] !== 'object'));
  const ordered = [...preferred.filter((key) => keys.includes(key)), ...keys.filter((key) => !preferred.includes(key))].slice(0, 4);
  return ordered.map((key, index) => ({ key, label: key.replace(/_/g, ' '), width: index > 1 ? '10rem' : undefined }));
}

function CollectionDetailPanel({ spaceId, collection, id, icon }: { spaceId: string; collection: string; id: string; icon: string }) {
  const [data, setData] = useState<JsonRecord>(); const [error, setError] = useState<unknown>();
  useEffect(() => { setData(undefined); setError(undefined); void api.detail(spaceId, collection, id).then(setData).catch(setError); }, [collection, id, spaceId]);
  if (error) return <ErrorState error={error} />;
  if (!data) return <Loading />;
  const nested = Object.values(data).find((value) => value && typeof value === 'object' && !Array.isArray(value)) as JsonRecord | undefined;
  const item = nested || data;
  return <div className="h-full overflow-auto p-5"><div className="mx-auto max-w-5xl"><header className="flex items-start gap-3 border-b border-neutral-200 pb-5 dark:border-neutral-800"><span className="grid h-10 w-10 place-items-center rounded-xl bg-indigo-100 text-indigo-600 dark:bg-indigo-500/15 dark:text-indigo-300"><Icon name={icon} size={18} /></span><div className="min-w-0"><h2 className="text-xl font-semibold">{titleFor(item)}</h2><p className="mt-1 text-xs text-neutral-500">{t('Registro publicado · solo lectura')}</p></div></header><dl className="server-desktop-record mt-5">{Object.entries(item).filter(([, value]) => value !== null && value !== undefined).map(([key, value]) => <div key={key}><dt>{key.replace(/_/g, ' ')}</dt><dd>{typeof value === 'object' ? <pre>{JSON.stringify(value, null, 2)}</pre> : text(value)}</dd></div>)}</dl></div></div>;
}
...
function CollectionView({ spaceId, view }: { spaceId: string; view: View }) {
  ...
  const [query, setQuery] = useState(''); const [openTabs, setOpenTabs] = useState<Array<{ id: string; label: string }>>([]); const [activeId, setActiveId] = useState<string | null>(null);
  ...
  useEffect(() => { void load(); }, [load]); const items = pageItems(page); const filtered = items.filter((item) => !query.trim() || Object.values(item).some((value) => typeof value !== 'object' && text(value, '').toLowerCase().includes(query.trim().toLowerCase()))); const columns = inferredColumns(view, items);
  const openItem = (item: JsonRecord, index: number) => { const id = recordId(item, String(index)); const label = titleFor(item); setOpenTabs((tabs) => tabs.some((tab) => tab.id === id) ? tabs : [...tabs, { id, label }]); setActiveId(id); };
  const closeTab = (id: string) => { setOpenTabs((tabs) => tabs.filter((tab) => tab.id !== id)); if (activeId === id) setActiveId(null); };
  ...
  return <div className="flex h-full min-h-0 flex-col bg-white text-neutral-900 dark:bg-neutral-950 dark:text-neutral-100" data-testid="collection-workspace"><header className="shrink-0 border-b border-neutral-200 px-5 pt-4 dark:border-neutral-800"><div className="mb-3 flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-indigo-100 text-indigo-600 dark:bg-indigo-500/15 dark:text-indigo-300"><Icon name={meta.icon} size={18} /></span><div><h1 className="text-base font-semibold">{collectionLabel}</h1><p className="text-[11px] text-neutral-500">{page?.total ?? items.length} {t('registros publicados')}</p></div></div><div className="flex min-w-0 items-end gap-1 overflow-x-auto"><button className={`flex h-9 shrink-0 items-center gap-2 rounded-t-lg border border-b-0 px-3 text-xs ${!activeId ? 'border-neutral-300 bg-white dark:border-neutral-700 dark:bg-neutral-900' : 'border-transparent text-neutral-500'}`} onClick={() => setActiveId(null)}><Icon name="table" size={13} /> {collectionLabel}</button>{openTabs.map((tab) => <div key={tab.id} className={`flex h-9 shrink-0 items-center rounded-t-lg border border-b-0 ${activeId === tab.id ? 'border-neutral-300 bg-white dark:border-neutral-700 dark:bg-neutral-900' : 'border-transparent text-neutral-500'}`}><button className="flex h-full max-w-64 items-center gap-2 px-3 text-xs" onClick={() => setActiveId(tab.id)}><Icon name={meta.icon} size={13} /><span className="truncate">{tab.label}</span></button><button className="mr-1 grid h-6 w-6 place-items-center rounded hover:bg-neutral-200 dark:hover:bg-neutral-800" onClick={() => closeTab(tab.id)} aria-label={`${t('Cerrar')} ${tab.label}`}><Icon name="x" size={11} /></button></div>)}</div></header>{activeId ? <div className="min-h-0 flex-1"><CollectionDetailPanel spaceId={spaceId} collection={meta.collection} id={activeId} icon={meta.icon} /></div> : <><div className="shrink-0 border-b border-neutral-200 p-3 dark:border-neutral-800"><div className="relative"><Icon name="search" size={15} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-neutral-500" /><input className="input input-with-leading-icon w-full" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`${t('Buscar en')} ${collectionLabel.toLowerCase()}…`} /></div></div><div className="min-h-0 flex-1 overflow-auto" data-testid="record-list"><div className="min-w-[980px]"><div className="grid h-10 items-center border-b border-neutral-200 px-4 text-[10px] font-semibold uppercase tracking-wider text-neutral-500 dark:border-neutral-800 dark:text-neutral-600" style={{ gridTemplateColumns: template }}>{columns.map((column) => <span key={column.key} className="truncate">{t(column.label)}</span>)}<span /></div>{error ? <ErrorState error={error} onRetry={() => void load()} /> : loading ? <Loading /> : filtered.length === 0 ? <EmptyState title={`${t('No hay')} ${collectionLabel.toLowerCase()} ${t('publicados')}`} /> : filtered.map((item, index) => <button key={recordId(item, String(index))} className="grid min-h-[62px] w-full items-center border-b border-neutral-100 px-4 py-2.5 text-left text-xs hover:bg-neutral-50 dark:border-neutral-900 dark:hover:bg-neutral-900/55" style={{ gridTemplateColumns: template }} onClick={() => openItem(item, index)}>{columns.map((column, columnIndex) => <span key={column.key} className={`${columnIndex === 0 ? 'font-medium text-neutral-900 dark:text-neutral-200' : 'text-neutral-500'} line-clamp-2 min-w-0 pr-4`}>{text(item[column.key], columnIndex === 0 ? titleFor(item) : '—')}</span>)}<Icon name="chevronRight" size={14} className="text-neutral-400" /></button>)}</div></div><footer className="flex h-10 shrink-0 items-center border-t border-neutral-200 px-3 text-xs text-neutral-500 dark:border-neutral-800">{filtered.length} / {page?.total ?? items.length}</footer></>}</div>;
}

function DetailView({ spaceId, route }: { spaceId: string; route: Extract<Route, { kind: 'detail' }> }) {
...
export default function App() {
  ...
  const profileThemeRef = useRef<PortableProfileValues['appearance']['theme']>('system');
  ...
  const refreshSpaces = useCallback(async () => {
    ...
    setMe(value); setActiveId((current) => {
  ...
  useEffect(() => {
    ...
    const onSystemTheme = () => {
      if (profileThemeRef.current === 'system') setTheme(resolveTheme('system'));
    };
  ...
  const spaces = me?.spaces || []; const active = useMemo(() => spaces.find((space) => space.id === activeId) || spaces[0], [spaces, activeId]); const type = normalizeVaultType(active?.vaultType || active?.vault?.type); const activeView = route.view;
  const primarySourcesLoader = useMemo(() => active?.id && type === 'primary_sources' ? {
    getPrimarySourceMapWorkspace: () => api.primarySources.map(active.id) as unknown as Promise<PrimarySourceMapWorkspace>,
    getPrimarySourceTimelineWorkspace: () => api.primarySources.timeline(active.id) as unknown as Promise<PrimarySourceTimelineWorkspace>,
    getPrimarySourceRelationsWorkspace: () => api.primarySources.relations(active.id) as unknown as Promise<PrimarySourceRelationsWorkspace>,
    listPrimarySourcePersons: (query: string, filter: PrimarySourcePersonFilter) => api.primarySources.persons(active.id, { q: query, filter }) .then((response) => (response.people ?? []) as PrimarySourcePersonSummary[]),
    getPrimarySourcePersonDossier: (id: string) => api.primarySources.person(active.id, id).then((response) => response as unknown as PrimarySourcePersonDossier),
    searchPrimarySourceCorpus: (request: PrimarySourceSearchRequest) => api.primarySources.search(active.id, { q: request.query, ...request.filters }) as unknown as Promise<PrimarySourceSearchResponse>,
  } : undefined, [active?.id, type]);
  useEffect(() => {
    if (!active?.id) return;
  ...
  const openView = (view: View) => { if (SERVER_TOOL_VIEWS.has(view)) return; navigate(viewPath(view)); setDrawer(false); };
  const resize = (event: ReactPointerEvent) => { const start = event.clientX; const initial = sidebarWidth; const move = (next: PointerEvent) => setSidebarWidth(Math.max(SERVER_SIDEBAR_MIN_WIDTH, Math.min(SERVER_SIDEBAR_MAX_WIDTH, initial + next.clientX - start))); const up = () => { document.body.classList.remove('is-resizing-sidebar'); removeEventListener('pointermove', move); removeEventListener('pointerup', up); removeEventListener('pointercancel', up); }; document.body.classList.add('is-resizing-sidebar'); addEventListener('pointermove', move); addEventListener('pointerup', up); addEventListener('pointercancel', up); };
  const resizeWithKeyboard = (event: React.KeyboardEvent<HTMLButtonElement>) => { const requested = event.key === 'Home' ? SERVER_SIDEBAR_MIN_WIDTH : event.key === 'End' ? SERVER_SIDEBAR_MAX_WIDTH : event.key === 'ArrowLeft' ? sidebarWidth - 8 : event.key === 'ArrowRight' ? sidebarWidth + 8 : null; if (requested === null) return; event.preventDefault(); setSidebarWidth(Math.max(SERVER_SIDEBAR_MIN_WIDTH, Math.min(SERVER_SIDEBAR_MAX_WIDTH, requested))); };
  useEffect(() => { localStorage.setItem('nodus-server-sidebar-width', String(sidebarWidth)); }, [sidebarWidth]);
  useEffect(() => { localStorage.setItem(SERVER_NAV_COLLAPSED_STORAGE_KEY, navCollapsed ? '1' : '0'); }, [navCollapsed]);
  useEffect(() => { localStorage.setItem(SERVER_COLLAPSED_GROUPS_STORAGE_KEY, JSON.stringify([...collapsedGroups])); }, [collapsedGroups]);
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      setVaultBadgePlacement((previous) => {
        const next = placeHeaderBadge({ headerWidth: headerEl.clientWidth, logoWidth: headerLogoEl.offsetWidth, actionsWidth: headerActionsEl.offsetWidth, badgeWidth: vaultBadgeEl.offsetWidth });
  ...
  if (error && !me) return <div className="h-full"><ErrorState error={error} /></div>; if (!me) return <Loading />; if (spaces.length === 0) return <EmptyState title={t('No tienes bóvedas asignadas')} detail={t('Solicita acceso a un administrador de Nodus Server.')} />; if (error && !summary) return <ErrorState error={error} />;
  ...
  const content = (() => {
    ...
    if (route.kind === 'detail' && ['ideas', 'works', 'authors'].includes(route.collection)) {
      ...
      return <AcademicDetailExplorer key={`${kind}:${route.id}`} spaceId={active.id} origin={t(route.view === 'search' ? 'Buscar' : 'Inicio')} initialTarget={{ kind, id: route.id, label: route.id } as AcademicTarget} onOrigin={() => history.back()} />;
    }
    ...
    if (route.view === 'search') return <SearchServerView spaceId={active.id} vaultType={type} onNavigate={(collection, id) => { const owner = searchDetailView(collection, type); navigate(`/detail/${owner || 'search'}/${encodeURIComponent(collection)}/${encodeURIComponent(id)}`); }} />;
    if (route.view === 'prosopSearch' || route.view === 'studySearch' || route.view === 'dbSearch') {
      return <SearchServerView key={`${active.id}:${route.view}`} spaceId={active.id} vaultType={type} onNavigate={(collection, id) => { const owner = searchDetailView(collection, type); navigate(`/detail/${owner || 'search'}/${encodeURIComponent(collection)}/${encodeURIComponent(id)}`); }} />;
    }
  ...
  return <div className={`server-desktop-surface h-full flex flex-col ${type}`} data-theme={theme} style={{ '--vault-accent': VAULT_TYPE_COLORS[type], '--server-sidebar-width': `${sidebarWidth}px` } as React.CSSProperties} data-testid="app-shell" data-surface="server">
