...
function Metric({ label, value, onClick }: { label: string; value: number; onClick: () => void }) {
  return <button className="rounded-lg border border-neutral-800 px-3 py-3 text-left hover:border-indigo-700" onClick={onClick}><div className="text-xl font-semibold tabular-nums">{value.toLocaleString('es')}</div><div className="truncate text-xs text-neutral-500">{label}</div></button>;
}
...
export default function App() {
  ...
  if (error && !me) return <div className="h-full"><ErrorState error={error} /></div>; if (!me) return <Loading />; if (spaces.length === 0) return <EmptyState title="No tienes bóvedas asignadas" detail="Solicita acceso a un administrador de Nodus Server." />; if (error && !summary) return <ErrorState error={error} />;
