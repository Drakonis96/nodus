...
function fieldLabel(column: string): string {
  return t(FIELD_LABELS[column] || column.replace(/_id$/, '').replaceAll('_', ' ').replace(/^./, (letter) => letter.toLocaleUpperCase('es')));
}
...
function newRecordText(singular: string): string {
  return `${t(FEMININE_RECORDS.has(singular) ? 'Nueva' : 'Nuevo')} ${t(singular)}`;
}

function firstRecordText(singular: string): string {
  return `${t(FEMININE_RECORDS.has(singular) ? 'la primera' : 'el primer')} ${t(singular)}`;
}
...
export function NativeContentAuthoring({ spaceId, surface, revision, csrfToken, canWrite, onChanged }: {
  ...
  if (loading) return <div className="grid h-full place-items-center text-sm text-neutral-500" data-testid="native-content-loading">{t('Cargando contenido de la bóveda…')}</div>;
  if (!contract || !table) return <div className="grid h-full place-items-center p-8 text-center" data-testid="native-content-unavailable"><div><h2 className="text-base font-semibold">{t('Esta vista aún no admite contenido nativo')}</h2><p className="mt-2 max-w-lg text-sm text-neutral-500">{t('La bóveda sigue siendo válida, pero esta colección derivada no expone un contrato de escritura seguro.')}</p>{error && <p className="mt-3 text-xs text-red-400">{error}</p>}</div></div>;
  const labels = SURFACE_LABELS[surface] || { title: fieldLabel(table), singular: 'registro' };
  const recordCount = `${rows.length} ${t(rows.length === 1 ? 'registro' : 'registros')}`;
...
export function NativeRecordEditor({ spaceId, table, contract, revision, csrfToken, row, labels, onCancel, onSaved }: {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (contract.key.some((column) => !String(key[column]).trim())) throw new Error(t('Completa la clave del registro.'));
      ...
      onSaved();
    } catch (cause) { setError(cause instanceof Error ? cause.message : t('No se pudo guardar.')); }
