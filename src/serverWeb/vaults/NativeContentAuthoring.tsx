...
function fieldLabel(column: string): string {
  return t(FIELD_LABELS[column] || column.replace(/_id$/, '').replaceAll('_', ' ').replace(/^./, (letter) => letter.toLocaleUpperCase(getActiveLang())));
}
...
export function NativeContentAuthoring({ spaceId, surface, revision, csrfToken, canWrite, onChanged }: {
  ...
  useEffect(() => {
    ...
    }).catch((cause) => { if (mounted) { setContract(undefined); setRows([]); setError(errorText(cause)); } }).finally(() => { if (mounted) setLoading(false); });
