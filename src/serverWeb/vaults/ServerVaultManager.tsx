...
type Action = 'duplicate' | 'reset' | 'delete';
...
function Modal({ title, children, onClose }: { title: string; children: React.ReactNode; onClose: () => void }) {
  return <div className="fixed inset-0 z-[80] grid place-items-center bg-black/70 p-4" role="dialog" aria-modal="true" data-testid="server-vault-modal">
...
export function ServerVaultManager({ spaces, active, isAdmin, csrfToken, onSelect, onChanged, onClose }: {
  ...
  return <>
    <div ref={panelRef} className="server-vault-popover" role="dialog" aria-label="Gestionar vaults" data-testid="vault-manager" tabIndex={-1}>
      ...
      <div className="max-h-[58vh] space-y-1.5 overflow-y-auto p-3">{shown.map((space) => { const managed = space as ManagedSpace; const type = spaceType(managed); const selected = active.id === space.id; const editable = isAdmin && isNative(managed); return <article className="flex min-w-0 items-center gap-2 rounded-lg border border-neutral-800 px-2 py-2" key={space.id} data-testid={`vault-row-${space.id}`}><span className="grid h-8 w-8 shrink-0 place-items-center rounded-md text-white" style={{ backgroundColor: VAULT_TYPE_COLOR[type] }}><Icon name={vaultTypeIcon(type)} size={15} /></span><button type="button" className="min-w-0 flex-1 text-left" onClick={() => onSelect(space.id)} data-testid={`vault-option-${space.id}`}><strong className="block truncate text-xs text-neutral-200">{space.name}</strong><small className="block truncate text-[10px] text-neutral-600">{vaultTypeLabel(type)} · {managed.storageKind === 'desktop_published' ? 'Publicado desde Desktop' : selected ? 'Activo' : space.role || 'Lectura'}</small></button><div className="flex shrink-0 items-center gap-0.5"><button type="button" className="rounded p-1.5 text-neutral-400 hover:bg-neutral-800 disabled:opacity-40" disabled={selected || busy} onClick={() => onSelect(space.id)} title={selected ? 'Vault activo' : 'Activar'} aria-label={selected ? 'Vault activo' : `Activar ${space.name}`}><Icon name={selected ? 'check' : 'play'} size={14} /></button>{isAdmin && <><button type="button" className="rounded p-1.5 text-neutral-400 hover:bg-neutral-800 disabled:opacity-40" disabled={!editable || busy} onClick={() => { setRenameTarget(managed); setRenameValue(space.name); }} title={editable ? 'Renombrar' : 'Solo lectura'} aria-label={`Renombrar ${space.name}`}><Icon name="edit" size={14} /></button><button type="button" className="rounded p-1.5 text-neutral-400 hover:bg-neutral-800 disabled:opacity-40" disabled={!editable || busy} onClick={() => { setDuplicateTarget(managed); setDuplicateValue(`${space.name} copia`); }} title="Duplicar" aria-label={`Duplicar ${space.name}`}><Icon name="copy" size={14} /></button><button type="button" className="rounded p-1.5 text-neutral-400 hover:bg-neutral-800 disabled:opacity-40" disabled={!editable || busy} onClick={() => void exportSpace(managed)} title="Exportar" aria-label={`Exportar ${space.name}`}><Icon name="download" size={14} /></button><button type="button" className="rounded p-1.5 text-neutral-400 hover:bg-neutral-800 disabled:opacity-40" disabled={!editable || busy} onClick={() => setImportTarget(managed)} title="Importar" aria-label={`Importar en ${space.name}`}><Icon name="upload" size={14} /></button><button type="button" className="rounded p-1.5 text-red-400 hover:bg-red-950/40 disabled:opacity-40" disabled={!editable || busy} onClick={() => setConfirmAction({ action: 'delete', space: managed })} title="Eliminar" aria-label={`Eliminar ${space.name}`}><Icon name="trash" size={14} /></button></>}</div></article>; })}{shown.length === 0 && <p className="p-5 text-center text-xs text-neutral-600">No hay coincidencias.</p>}{isAdmin && <div className="flex justify-end border-t border-neutral-800 pt-2"><button type="button" className="btn btn-ghost gap-1 px-2 py-1 text-xs" disabled={busy || !active || !isNative(active as ManagedSpace)} onClick={() => setConfirmAction({ action: 'reset', space: active as ManagedSpace })} data-testid="vault-reset"><Icon name="refresh" size={13} />Reinicializar activo</button></div>}{notice && <p className="rounded-md border border-teal-800/60 px-2 py-1 text-xs text-teal-300" role="status">{notice}</p>}{error && <p className="rounded-md border border-red-900/60 px-2 py-1 text-xs text-red-300" role="alert">{error}</p>}</div>
    </div>
    ...
    {confirmAction && <Modal title={confirmAction.action === 'delete' ? 'Eliminar vault' : 'Reinicializar vault'} onClose={() => setConfirmAction(null)}><p className="text-sm leading-6 text-neutral-300">{confirmAction.action === 'delete' ? `Se eliminará definitivamente el vault nativo «${confirmAction.space.name}» y su base de datos del servidor.` : `Se vaciará el contenido de «${confirmAction.space.name}» conservando su identidad y configuración.`}</p><div className="mt-5 flex justify-end gap-2"><button type="button" className="btn btn-ghost" onClick={() => setConfirmAction(null)}>Cancelar</button><button type="button" className="btn border border-red-700 text-red-300" onClick={() => void destructive()} disabled={busy}>{confirmAction.action === 'delete' ? 'Eliminar' : 'Reinicializar'}</button></div></Modal>}
