 ...
 *
 *  `plate: false` drops the near-black rounded square behind the mark, for a light surface
 *  where it reads as a black hole rather than as a logo. The pale stops go with it: they
 *  were chosen to carry against that plate, and on white they barely show, so the plateless
 *  variant keeps the mark at roughly the accent's own weight. */
export function marketplaceLogoSvg(accent = '#6366f1', { plate = true }: { plate?: boolean } = {}): string {
  const color = /^#[0-9a-f]{6}$/i.test(accent) ? accent : '#6366f1';
  const light = shade(color, plate ? .55 : -.1), dark = shade(color, -.4), g = geometry;
  ...
  ${plate ? `<rect x="4" y="4" width="120" height="120" rx="32" fill="${shade(color, -.85)}"/>` : ''}
