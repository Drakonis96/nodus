...
export function deriveTreeKinship(input: TreeKinshipInput): Map<string, TreeKinshipContext> {
  ...
  const ancestorContext = (focusId: string, id: string, info: AncestorPath): TreeKinshipContext => {
    ...
    if (info.distance > 4) {
      const branchEs = branch === 'paternal' ? ' paterno/a' : branch === 'maternal' ? ' materno/a' : '';
      const branchEn = branch === 'paternal' ? ' paternal' : branch === 'maternal' ? ' maternal' : '';
      context.labelEs = `Ascendiente${branchEs} de ${info.distance}.ª generación`;
      context.labelEn = `${ordinalEn(info.distance)}-generation${branchEn} ancestor`;
    }
    return context;
  };
  ...
  const affinityFromSpouse = (context: TreeKinshipContext): TreeKinshipContext => {
    ...
    const baseEs = treeKinshipLabel(context, 'es');
    const baseEn = treeKinshipLabel(context, 'en');
    return { role: 'relative_by_marriage', branch: 'neutral', tone: 0, depth: context.depth + 1, labelEs: `${baseEs} de su cónyuge`, labelEn: `${baseEn} of their spouse` };
  };
  for (const spouse of spousesOf.get(input.focusId) ?? []) {
  ...
  const spouseOfBlood = (known: TreeKinshipContext, otherId: string): TreeKinshipContext => {
    ...
    const spouseEs = sex === 'male' ? 'Esposo' : sex === 'female' ? 'Esposa' : 'Cónyuge';
    const spouseEn = sex === 'male' ? 'Husband' : sex === 'female' ? 'Wife' : 'Spouse';
    return {
      role: 'relative_by_marriage', branch: known.branch, tone: known.tone, depth: known.depth + 1,
      labelEs: `${spouseEs} de su ${treeKinshipLabel(known, 'es').toLocaleLowerCase('es')}`,
      labelEn: `${spouseEn} of their ${treeKinshipLabel(known, 'en').toLocaleLowerCase('en')}`,
    };
  };
  for (const { a, b } of input.spouseEdges) {
  ...
  interface ConnectionStep { es: string; en: string }
  ...
  const stepFor = (sex: string | undefined, maleEs: string, femaleEs: string, unknownEs: string, maleEn: string, femaleEn: string, unknownEn: string): ConnectionStep => ({
    es: sex === 'male' ? maleEs : sex === 'female' ? femaleEs : unknownEs,
    en: sex === 'male' ? maleEn : sex === 'female' ? femaleEn : unknownEn,
  });
  for (const { parent, child } of input.parentEdges) {
    addConnection(child, parent, stepFor(sexOf.get(parent), 'padre', 'madre', 'progenitor/a', 'father', 'mother', 'parent'));
    addConnection(parent, child, stepFor(sexOf.get(child), 'hijo', 'hija', 'hijo/a', 'son', 'daughter', 'child'));
  }
  for (const { a, b } of input.spouseEdges) {
    addConnection(a, b, stepFor(sexOf.get(b), 'esposo', 'esposa', 'cónyuge', 'husband', 'wife', 'spouse'));
    addConnection(b, a, stepFor(sexOf.get(a), 'esposo', 'esposa', 'cónyuge', 'husband', 'wife', 'spouse'));
  }
  for (const [a, siblings] of siblingsOf) for (const b of siblings) {
    addConnection(a, b, stepFor(sexOf.get(b), 'hermano', 'hermana', 'hermano/a', 'brother', 'sister', 'sibling'));
  }
  ...
  for (const personId of allIds) {
    ...
    if (path) {
      result.set(personId, {
        ...
        labelEs: `Conexión familiar: ${path.map((step) => step.es).join(' → ')}`,
        labelEn: `Family connection: ${path.map((step) => step.en).join(' → ')}`,
      });
