...
export const NODI_WINDOW_METHODS = [
  ...
  'copyChatImage',
  // Capability packages, on the same terms as the v1 plugins listed above: this window
  // already installs, approves, rolls back and removes those, so withholding the v2
  // equivalents would be a difference in vocabulary rather than in privilege.
  //
  // The read half is not optional. A result can appear in a Nodi conversation exactly as it
  // does in the Assistant, and the chat renderer asks which fences each provider claims
  // before it draws any message at all — so the absence of `listCapabilities` here did not
  // degrade one card, it took the whole chat panel down with a TypeError.
  'listCapabilities',
  'onCapabilityRegistryChanged',
  'renderCapabilityArtifact',
  'renderLegacyCapabilityResult',
  'readCapabilityModel',
  'readCapabilityMedia',
  'fetchCapabilityTile',
  ...
  'rollbackPlugin',
  'removePlugin',
  'configurePluginSecret',
  'restorePluginSkillAuthorVersion',
