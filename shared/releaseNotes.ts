...
const RELEASE_3_1_0_HIGHLIGHTS: RawReleaseHighlight[] = [
  {
    scope: 'general',
    es: 'Una bóveda puede vivir en un servidor y replicarse en tu equipo. Al crear una bóveda ahora eliges entre local, como siempre, o conectada: introduces la dirección de un Nodus Server y tus credenciales, y te traes una copia completa del espacio al que tengas acceso. No es un visor remoto ni una ventana a otra máquina: es una base de datos real en tu disco, así que el grafo, los debates, el mapa de argumentos, los informes de Deep Research y las inmersiones funcionan exactamente igual, y siguen funcionando en un tren sin cobertura. Se actualiza sola en segundo plano. Y si quien administra el servidor te retira el acceso, la bóveda se queda entera en tu equipo con un aviso claro: deja de sincronizarse, no se borra nada, y puedes conservarla como bóveda local.',
    en: 'A vault can live on a server and be replicated onto your machine. Creating a vault now offers a choice between local, as always, or connected: you enter a Nodus Server address and your credentials, and pull down a complete copy of a space you have access to. It is not a remote viewer or a window onto another machine — it is a real database on your disk, so the graph, the debates, the argument map, the Deep Research reports and the immersions all work exactly as they do locally, and keep working on a train with no signal. It refreshes itself in the background. And if whoever runs the server withdraws your access, the vault stays whole on your machine with a plain notice: it stops syncing, nothing is deleted, and you can keep it as a local vault.',
    fr: 'Un coffre peut vivre sur un serveur et être répliqué sur votre machine. À la création, vous choisissez désormais entre local, comme toujours, et connecté : vous saisissez l’adresse d’un Nodus Server et vos identifiants, et vous récupérez une copie complète d’un espace auquel vous avez accès. Ce n’est pas une visionneuse distante ni une fenêtre sur une autre machine : c’est une vraie base de données sur votre disque, donc le graphe, les débats, la carte d’arguments, les rapports Deep Research et les immersions fonctionnent exactement comme en local, et continuent de fonctionner dans un train sans réseau. Elle se met à jour toute seule en arrière-plan. Et si l’administrateur du serveur vous retire l’accès, le coffre reste entier sur votre machine avec un avertissement clair : la synchronisation s’arrête, rien n’est supprimé, et vous pouvez le conserver comme coffre local.',
    de: 'Ein Tresor kann auf einem Server liegen und auf Ihren Rechner repliziert werden. Beim Anlegen wählen Sie jetzt zwischen lokal, wie bisher, und verbunden: Sie geben die Adresse eines Nodus Server und Ihre Zugangsdaten ein und holen sich eine vollständige Kopie eines Bereichs, auf den Sie Zugriff haben. Kein Fernbetrachter und kein Fenster auf eine andere Maschine — es ist eine echte Datenbank auf Ihrer Platte, sodass Graph, Debatten, Argumentkarte, Deep-Research-Berichte und Immersionen genau wie lokal funktionieren, auch im Zug ohne Empfang. Sie aktualisiert sich im Hintergrund von selbst. Und wenn die Serververwaltung Ihren Zugriff entzieht, bleibt der Tresor vollständig auf Ihrem Rechner, mit einem klaren Hinweis: Die Synchronisierung endet, nichts wird gelöscht, und Sie können ihn als lokalen Tresor behalten.',
    pt: 'Um cofre pode viver num servidor e ser replicado no seu computador. Ao criar um cofre escolhe agora entre local, como sempre, ou ligado: introduz o endereço de um Nodus Server e as suas credenciais e traz uma cópia completa de um espaço a que tenha acesso. Não é um visualizador remoto nem uma janela para outra máquina: é uma base de dados real no seu disco, por isso o grafo, os debates, o mapa de argumentos, os relatórios de Deep Research e as imersões funcionam exatamente como localmente, e continuam a funcionar num comboio sem rede. Atualiza-se sozinho em segundo plano. E se quem administra o servidor lhe retirar o acesso, o cofre fica inteiro no seu computador com um aviso claro: deixa de sincronizar, nada é apagado, e pode mantê-lo como cofre local.',
    'pt-BR': 'Um cofre pode viver em um servidor e ser replicado no seu computador. Ao criar um cofre você agora escolhe entre local, como sempre, ou conectado: informa o endereço de um Nodus Server e suas credenciais e baixa uma cópia completa de um espaço a que tenha acesso. Não é um visualizador remoto nem uma janela para outra máquina: é um banco de dados real no seu disco, então o grafo, os debates, o mapa de argumentos, os relatórios de Deep Research e as imersões funcionam exatamente como localmente, e continuam funcionando em um trem sem sinal. Ele se atualiza sozinho em segundo plano. E se quem administra o servidor retirar seu acesso, o cofre permanece inteiro no seu computador com um aviso claro: para de sincronizar, nada é apagado, e você pode mantê-lo como cofre local.',
  },
  {
    scope: 'general',
    es: 'Cada cuenta tiene un nivel de acceso por espacio, y ese nivel decide dónde acaba tu trabajo. Con lectura consultas el corpus y todo lo que escribas o generes —notas, informes, inmersiones— se queda en tu equipo y no sale nunca; la propia base de datos de tu réplica no tiene forma de enviarlo, no es solo que la interfaz no lo ofrezca. Con escritura, tus notas e informes viajan al vault principal la próxima vez que su propietario se conecte, y hasta entonces la pantalla te dice cuántos cambios esperan. Quien administra el servidor asigna varios espacios de una vez y un nivel distinto en cada uno, y puede cambiarlo sin revocar y volver a conceder. Ahora se comparten también las bóvedas de genealogía, docencia, estudio y bases de datos, no solo las académicas.',
    en: 'Every account has an access level per space, and that level decides where your work ends up. With read access you consult the corpus and anything you write or generate — notes, reports, immersions — stays on your machine and never leaves; your replica\'s database has no way to send it, not merely an interface that does not offer to. With write access your notes and reports travel to the main vault the next time its owner connects, and until then the screen tells you how many changes are waiting. Whoever runs the server assigns several spaces at once with a different level in each, and can change a level without revoking and re-granting. Genealogy, teaching, study and database vaults can now be shared too, not only academic ones.',
    fr: 'Chaque compte a un niveau d’accès par espace, et ce niveau décide où finit votre travail. En lecture, vous consultez le corpus et tout ce que vous écrivez ou générez — notes, rapports, immersions — reste sur votre machine et n’en sort jamais ; la base de données de votre réplique n’a aucun moyen de l’envoyer, ce n’est pas seulement une interface qui ne le propose pas. En écriture, vos notes et rapports rejoignent le coffre principal à la prochaine connexion de son propriétaire, et d’ici là l’écran indique combien de modifications attendent. L’administrateur du serveur attribue plusieurs espaces d’un coup avec un niveau différent dans chacun, et peut changer un niveau sans révoquer puis réattribuer. Les coffres de généalogie, d’enseignement, d’étude et de bases de données se partagent désormais aussi, pas seulement les coffres académiques.',
    de: 'Jedes Konto hat je Bereich eine Zugriffsstufe, und diese Stufe entscheidet, wo Ihre Arbeit landet. Mit Lesezugriff sehen Sie den Korpus, und alles, was Sie schreiben oder erzeugen — Notizen, Berichte, Immersionen — bleibt auf Ihrem Rechner und verlässt ihn nie; die Datenbank Ihres Replikats hat keine Möglichkeit, es zu senden, nicht bloß eine Oberfläche, die es nicht anbietet. Mit Schreibzugriff gelangen Ihre Notizen und Berichte in den Haupttresor, sobald dessen Eigentümer sich das nächste Mal verbindet; bis dahin zeigt der Bildschirm, wie viele Änderungen warten. Die Serververwaltung vergibt mehrere Bereiche auf einmal mit je eigener Stufe und kann eine Stufe ändern, ohne zu widerrufen und neu zu vergeben. Auch Genealogie-, Unterrichts-, Studien- und Datenbanktresore lassen sich jetzt teilen, nicht nur akademische.',
    pt: 'Cada conta tem um nível de acesso por espaço, e esse nível decide onde acaba o seu trabalho. Com leitura consulta o corpus e tudo o que escrever ou gerar — notas, relatórios, imersões — fica no seu computador e nunca sai; a própria base de dados da sua réplica não tem forma de o enviar, não é apenas uma interface que não o oferece. Com escrita, as suas notas e relatórios seguem para o cofre principal na próxima vez que o proprietário se ligar, e até lá o ecrã diz quantas alterações aguardam. Quem administra o servidor atribui vários espaços de uma vez com um nível diferente em cada um, e pode alterá-lo sem revogar e voltar a conceder. Passam a partilhar-se também os cofres de genealogia, ensino, estudo e bases de dados, não apenas os académicos.',
    'pt-BR': 'Cada conta tem um nível de acesso por espaço, e esse nível decide onde termina o seu trabalho. Com leitura você consulta o corpus e tudo o que escrever ou gerar — notas, relatórios, imersões — fica no seu computador e nunca sai; o próprio banco de dados da sua réplica não tem como enviá-lo, não é apenas uma interface que não oferece. Com escrita, suas notas e relatórios seguem para o cofre principal na próxima vez que o proprietário se conectar, e até lá a tela informa quantas alterações aguardam. Quem administra o servidor atribui vários espaços de uma vez com um nível diferente em cada um, e pode alterá-lo sem revogar e conceder de novo. Passam a ser compartilhados também os cofres de genealogia, ensino, estudo e bancos de dados, não apenas os acadêmicos.',
  },
  {
    scope: 'general',
    es: 'Corrección sobre lo que se publica, y un interruptor nuevo. La política de privacidad afirmaba que los embeddings nunca se suben, y desde que existe la búsqueda semántica compartida eso había dejado de ser cierto: los vectores de las ideas sí viajan, para que desde una réplica o un móvil se pueda buscar por significado y no solo por texto literal. El documento ya lo dice, dice que cambió, y hay un interruptor —«Incluir vectores semánticos»— que lo corta de verdad. Los vectores de pasajes siguen atados al interruptor de pasajes. De paso se han cerrado dos filtraciones que nadie había notado: una bóveda de estudio o docencia compartida enviaba las grabaciones de clase, los intentos y las ejecuciones de calificación, y ya no; y los documentos —PDF, audio, adjuntos de bases de datos— siguen sin salir nunca del equipo, mientras que las ilustraciones de los informes y los retratos de personas ahora llegan completos a la réplica.',
    en: 'A correction about what gets published, and a new switch. The privacy policy stated that embeddings are never uploaded, and since shared semantic search arrived that had stopped being true: idea vectors do travel, so that a replica or a phone can search by meaning rather than only by literal text. The document now says so, says that it changed, and there is a switch — “Include semantic vectors” — that really stops it. Passage vectors stay tied to the passages switch. Two leaks nobody had noticed were closed on the way: a shared study or teaching vault was sending class recordings, attempt records and grading runs, and no longer does; and documents — PDFs, audio, database attachments — still never leave your machine, while report illustrations and people’s portraits now arrive complete in a replica.',
    fr: 'Une correction sur ce qui est publié, et un nouvel interrupteur. La politique de confidentialité affirmait que les embeddings ne sont jamais envoyés, et depuis l’arrivée de la recherche sémantique partagée ce n’était plus vrai : les vecteurs des idées circulent bien, pour qu’une réplique ou un téléphone puisse chercher par le sens et pas seulement par le texte littéral. Le document le dit désormais, indique que cela a changé, et un interrupteur — « Inclure les vecteurs sémantiques » — l’arrête réellement. Les vecteurs issus des passages restent liés à l’interrupteur des passages. Deux fuites que personne n’avait remarquées ont été colmatées au passage : un coffre d’étude ou d’enseignement partagé envoyait les enregistrements de cours, les tentatives et les campagnes de notation, ce n’est plus le cas ; et les documents — PDF, audio, pièces jointes de bases de données — ne quittent toujours jamais votre machine, tandis que les illustrations des rapports et les portraits des personnes arrivent maintenant complets dans une réplique.',
    de: 'Eine Korrektur dazu, was veröffentlicht wird, und ein neuer Schalter. Die Datenschutzerklärung behauptete, Embeddings würden nie hochgeladen; seit es die geteilte semantische Suche gibt, stimmte das nicht mehr: Die Vektoren der Ideen reisen sehr wohl mit, damit ein Replikat oder ein Telefon nach Bedeutung suchen kann und nicht nur nach wörtlichem Text. Das Dokument sagt es jetzt, sagt auch, dass es sich geändert hat, und ein Schalter — „Semantische Vektoren einbeziehen“ — stoppt es wirklich. Vektoren aus Passagen bleiben an den Passagen-Schalter gebunden. Zwei unbemerkte Lecks wurden nebenbei geschlossen: Ein geteilter Studien- oder Unterrichtstresor schickte Unterrichtsaufnahmen, Versuche und Bewertungsläufe mit — nicht mehr; und Dokumente — PDFs, Audio, Datenbankanhänge — verlassen Ihren Rechner weiterhin nie, während Berichtsillustrationen und Porträts von Personen nun vollständig im Replikat ankommen.',
    pt: 'Uma correção sobre o que se publica, e um novo interruptor. A política de privacidade afirmava que os embeddings nunca são enviados, e desde que existe a pesquisa semântica partilhada isso deixara de ser verdade: os vetores das ideias viajam, para que a partir de uma réplica ou de um telemóvel se possa pesquisar por significado e não apenas por texto literal. O documento já o diz, diz que mudou, e há um interruptor — «Incluir vetores semânticos» — que o corta mesmo. Os vetores dos passagens continuam ligados ao interruptor das passagens. Pelo caminho fecharam-se duas fugas que ninguém notara: um cofre de estudo ou ensino partilhado enviava as gravações de aula, as tentativas e as execuções de avaliação, e já não; e os documentos — PDF, áudio, anexos de bases de dados — continuam a nunca sair do computador, enquanto as ilustrações dos relatórios e os retratos das pessoas chegam agora completos à réplica.',
    'pt-BR': 'Uma correção sobre o que é publicado, e um novo interruptor. A política de privacidade afirmava que os embeddings nunca são enviados, e desde que existe a busca semântica compartilhada isso deixara de ser verdade: os vetores das ideias viajam, para que a partir de uma réplica ou de um celular se possa buscar por significado e não apenas por texto literal. O documento já diz isso, diz que mudou, e há um interruptor — «Incluir vetores semânticos» — que corta de fato. Os vetores dos trechos continuam presos ao interruptor dos trechos. No caminho foram fechados dois vazamentos que ninguém havia notado: um cofre de estudo ou ensino compartilhado enviava as gravações de aula, as tentativas e as execuções de avaliação, e não envia mais; e os documentos — PDF, áudio, anexos de bancos de dados — continuam nunca saindo do computador, enquanto as ilustrações dos relatórios e os retratos das pessoas agora chegam completos à réplica.',
  },
];
