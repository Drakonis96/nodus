...
// that arrived in this cycle — Testimony, Worldbuilding, Primary Sources and
// Prosopography — are why the major moves.
const RELEASE_3_0_0_HIGHLIGHTS: RawReleaseHighlight[] = [
  ...
  {
    scope: 'testimonios',
    es: 'El original nunca se corrige. El archivo entra tal y como se recibió, con su huella SHA-256 y marcado como inmutable, y la transcripción automática tampoco se toca: corregir, revisar, aprobar, anonimizar o traducir crea una versión nueva que recuerda de cuál procede. Los fragmentos citados se reanclan solos cuando pueden y quedan marcados cuando no, para que una cita no se mueva nunca en silencio. Y no hay recorte de silencios: en historia oral una pausa puede ser parte de lo que se cuenta.',
    en: 'The master is never corrected. The file enters exactly as received, with its SHA-256 checksum and marked immutable, and the machine transcript is untouched too: correcting, reviewing, approving, anonymising or translating creates a new version that remembers where it came from. Quoted fragments re-anchor themselves when they can and are flagged when they cannot, so a quotation never moves in silence. And there is no silence trimming: in oral history a pause can be part of what is being told.',
    fr: 'L’original ne se corrige jamais. Le fichier entre tel qu’il a été reçu, avec son empreinte SHA-256 et marqué immuable, et la transcription automatique n’est pas touchée non plus : corriger, relire, valider, anonymiser ou traduire crée une nouvelle version qui se souvient d’où elle vient. Les fragments cités se réancrent seuls quand ils le peuvent et sont signalés quand ils ne le peuvent pas, pour qu’une citation ne se déplace jamais en silence. Et aucun silence n’est coupé : en histoire orale, une pause peut faire partie de ce qui est raconté.',
    de: 'Das Original wird nie korrigiert. Die Datei kommt genau so an, wie sie empfangen wurde, mit SHA-256-Prüfsumme und als unveränderlich markiert; auch die automatische Transkription bleibt unberührt: Korrigieren, Prüfen, Freigeben, Anonymisieren oder Übersetzen erzeugt eine neue Fassung, die ihre Herkunft kennt. Zitierte Ausschnitte verankern sich neu, wo es sicher geht, und werden sonst markiert, damit sich ein Zitat nie stillschweigend verschiebt. Und Pausen werden nicht weggeschnitten: In der Oral History kann eine Pause Teil des Erzählten sein.',
    pt: 'O original nunca se corrige. O ficheiro entra tal como foi recebido, com a sua impressão SHA-256 e marcado como imutável, e a transcrição automática também não se toca: corrigir, rever, aprovar, anonimizar ou traduzir cria uma versão nova que se lembra de onde vem. Os excertos citados reancoram-se sozinhos quando podem e ficam marcados quando não podem, para que uma citação nunca se mova em silêncio. E não há corte de silêncios: em história oral uma pausa pode fazer parte do que se conta.',
    'pt-BR': 'O original nunca se corrige. O arquivo entra exatamente como foi recebido, com seu hash SHA-256 e marcado como imutável, e a transcrição automática também não se toca: corrigir, revisar, aprovar, anonimizar ou traduzir cria uma versão nova que lembra de onde veio. Os trechos citados se reancoram sozinhos quando dá e ficam marcados quando não dá, para que uma citação nunca se mova em silêncio. E não há corte de silêncios: em história oral uma pausa pode ser parte do que se conta.',
  },
  {
    scope: 'testimonios',
    es: 'El acuerdo con cada narrador se versiona y tiene efecto real. Estado del acuerdo, nivel de acceso y estado del trabajo son tres dimensiones independientes, cada cambio queda como una versión fechada, y esas condiciones bloquean de verdad las exportaciones, los paquetes de consulta y lo que la IA puede ver. Una entrevista restringida, embargada o retirada no sale por ningún canal, y el nombre real de quien pidió seudónimo no aparece nunca en una cita, un derivado ni un prompt.',
    en: 'The agreement with each narrator is versioned and has real effect. Agreement status, access level and workflow status are three independent dimensions, every change is kept as a dated version, and those conditions genuinely block exports, access packages and what the AI can see. A restricted, embargoed or withdrawn interview leaves through no channel, and the real name of anyone who asked for a pseudonym never appears in a quotation, a derivative or a prompt.',
    fr: 'L’accord avec chaque narrateur est versionné et produit un effet réel. État de l’accord, niveau d’accès et état du travail sont trois dimensions indépendantes, chaque changement est conservé comme version datée, et ces conditions bloquent vraiment les exports, les paquets de consultation et ce que l’IA peut voir. Un entretien restreint, sous embargo ou retiré ne sort par aucun canal, et le vrai nom de qui a demandé un pseudonyme n’apparaît jamais dans une citation, un dérivé ni une requête.',
    de: 'Die Vereinbarung mit jeder erzählenden Person wird versioniert und wirkt wirklich. Stand der Vereinbarung, Zugangsstufe und Arbeitsstand sind drei unabhängige Dimensionen, jede Änderung bleibt als datierte Fassung erhalten, und diese Bedingungen blockieren tatsächlich Exporte, Nutzungspakete und das, was die KI sehen darf. Ein eingeschränktes, gesperrtes oder zurückgezogenes Interview verlässt keinen Kanal, und der echte Name von jemandem, der ein Pseudonym erbeten hat, erscheint nie in einem Zitat, einer Ableitung oder einem Prompt.',
    pt: 'O acordo com cada narrador é versionado e tem efeito real. Estado do acordo, nível de acesso e estado do trabalho são três dimensões independentes, cada alteração fica como uma versão datada, e essas condições bloqueiam mesmo as exportações, os pacotes de consulta e o que a IA pode ver. Uma entrevista restrita, embargada ou retirada não sai por nenhum canal, e o nome real de quem pediu pseudónimo nunca aparece numa citação, num derivado nem num prompt.',
    'pt-BR': 'O acordo com cada narrador é versionado e tem efeito real. Estado do acordo, nível de acesso e estado do trabalho são três dimensões independentes, cada mudança fica como uma versão datada, e essas condições bloqueiam mesmo as exportações, os pacotes de consulta e o que a IA pode ver. Uma entrevista restrita, embargada ou retirada não sai por nenhum canal, e o nome real de quem pediu pseudônimo nunca aparece em uma citação, um derivado nem um prompt.',
  },
  {
    scope: 'testimonios',
    es: 'Codificar, contrastar y citar sin salir del audio. Seleccionas un pasaje de la transcripción y le pones códigos de un catálogo compartido por toda la bóveda; desde un fragmento creas una nota que ya trae la cita, el hablante, el minuto y un enlace de vuelta. Contrastes pone varias entrevistas una al lado de otra por código o por tema, marca lo compartido, lo distinto y qué entrevistas no dijeron nada, sin convertir esa ausencia en una conclusión y sin usar IA en ningún paso.',
    en: 'Coding, comparing and quoting without leaving the audio. You select a passage of the transcript and put codes on it from a catalogue shared by the whole vault; from a fragment you create a note that already carries the quotation, the speaker, the timecode and a link back. Comparisons lays several interviews side by side by code or theme, marking what they share, what differs and which interviews said nothing — without turning that absence into a conclusion and without AI at any step.',
    fr: 'Coder, comparer et citer sans quitter l’audio. Vous sélectionnez un passage de la transcription et vous lui attribuez des codes d’un catalogue partagé par tout le coffre ; depuis un fragment vous créez une note qui contient déjà la citation, le locuteur, la minute et un lien de retour. Les Comparaisons placent plusieurs entretiens côte à côte par code ou par thème, en signalant ce qu’ils partagent, ce qui diffère et quels entretiens n’ont rien dit — sans transformer cette absence en conclusion et sans IA à aucune étape.',
    de: 'Codieren, vergleichen und zitieren, ohne das Audio zu verlassen. Sie wählen eine Stelle der Transkription und vergeben Codes aus einem Katalog, den der ganze Tresor teilt; aus einem Ausschnitt entsteht eine Notiz, die Zitat, Sprecher, Zeitmarke und Rücklink bereits enthält. Vergleiche legen mehrere Interviews nach Code oder Thema nebeneinander und zeigen Gemeinsames, Abweichendes und welche Interviews nichts gesagt haben – ohne dieses Schweigen zu einer Schlussfolgerung zu machen und ohne KI in irgendeinem Schritt.',
    pt: 'Codificar, contrastar e citar sem sair do áudio. Seleciona uma passagem da transcrição e põe-lhe códigos de um catálogo partilhado por todo o cofre; a partir de um excerto cria uma nota que já traz a citação, o falante, o minuto e uma ligação de volta. Contrastes põe várias entrevistas lado a lado por código ou por tema, marca o que partilham, o que difere e que entrevistas não disseram nada, sem converter essa ausência numa conclusão e sem usar IA em nenhum passo.',
    'pt-BR': 'Codificar, contrastar e citar sem sair do áudio. Você seleciona uma passagem da transcrição e coloca códigos de um catálogo compartilhado por todo o cofre; a partir de um trecho você cria uma nota que já traz a citação, o falante, o minuto e um link de volta. Contrastes coloca várias entrevistas lado a lado por código ou por tema, marca o que compartilham, o que difere e quais entrevistas não disseram nada, sem transformar essa ausência em conclusão e sem usar IA em nenhum passo.',
  },
  {
    scope: 'testimonios',
    es: 'Transcripción local y exportación archivística. Whisper corre en tu equipo y el audio no sale de aquí; las voces se separan solas y ponerles nombre sigue siendo manual y en lote, porque Nodus separa voces pero no reconoce personas ni va a fingir que sí. Al terminar puedes exportar tres paquetes distintos —preservación, consulta y revisión—, cada uno con su manifiesto, sus sumas de comprobación SHA-256 y la lista explícita de qué quedó fuera y por qué. La demo «Memoria del valle» enseña el recorrido completo con audio sintético y ninguna voz real.',
    en: 'Local transcription and archival export. Whisper runs on your machine and the audio does not leave it; voices are separated automatically and naming them stays manual and in bulk, because Nodus separates voices but does not recognise people and will not pretend otherwise. When you finish you can export three different packages — preservation, access and review — each with its manifest, its SHA-256 checksums and an explicit list of what was left out and why. The “Valley Memory” demo walks the whole path with synthetic audio and no real voice.',
    fr: 'Transcription locale et export archivistique. Whisper tourne sur votre ordinateur et l’audio n’en sort pas ; l’attribution des locuteurs est manuelle et par lots, car Nodus ne reconnaît pas les voix et ne fera pas semblant. À la fin, vous pouvez exporter trois paquets distincts — conservation, consultation et relecture —, chacun avec son manifeste, ses sommes de contrôle SHA-256 et la liste explicite de ce qui a été écarté et pourquoi. La démo « Mémoire de la vallée » parcourt tout le chemin avec un audio synthétique et aucune voix réelle.',
    de: 'Lokale Transkription und archivischer Export. Whisper läuft auf Ihrem Rechner, und das Audio verlässt ihn nicht; die Sprecherzuordnung ist manuell und im Stapel, denn Nodus erkennt keine Stimmen und wird das auch nicht vorgeben. Am Ende lassen sich drei verschiedene Pakete exportieren – Erhaltung, Nutzung und Prüfung –, jedes mit Manifest, SHA-256-Prüfsummen und einer ausdrücklichen Liste dessen, was warum ausgeschlossen wurde. Die Demo „Erinnerung des Tals“ zeigt den ganzen Weg mit synthetischem Audio und ohne eine einzige echte Stimme.',
    pt: 'Transcrição local e exportação arquivística. O Whisper corre no seu computador e o áudio não sai daqui; as vozes separam-se sozinhas e dar-lhes nome continua manual e em lote, porque o Nodus separa vozes mas não reconhece pessoas nem vai fingir que sim. No fim pode exportar três pacotes distintos — preservação, consulta e revisão —, cada um com o seu manifesto, as suas somas de verificação SHA-256 e a lista explícita do que ficou de fora e porquê. A demo «Memória do vale» mostra o percurso completo com áudio sintético e nenhuma voz real.',
    'pt-BR': 'Transcrição local e exportação arquivística. O Whisper roda no seu computador e o áudio não sai daqui; a atribuição de falantes é manual e em lote, porque o Nodus não reconhece vozes nem vai fingir que reconhece. No fim você pode exportar três pacotes distintos — preservação, consulta e revisão —, cada um com seu manifesto, suas somas de verificação SHA-256 e a lista explícita do que ficou de fora e por quê. A demo “Memória do vale” mostra o percurso completo com áudio sintético e nenhuma voz real.',
  },
  {
    scope: 'testimonios',
    es: 'La IA entra en la entrevista con tres operaciones y una puerta. Detectar hablantes separa las voces de la grabación en este equipo —el audio no sale— y propone «Voz 1» y «Voz 2» sin decidir quién es nadie; proponer análisis sugiere códigos y pasajes citables, y toda cita que no aparezca literal en la transcripción se descarta y se cuenta; corregir con IA puntúa y acentúa, y el tramo que cambie palabras se rechaza y se queda como estaba. Nada se guarda solo, y ninguna de las tres toca una entrevista cuyo acuerdo no lo autorice.',
    en: 'AI enters the interview through three operations and one gate. Detecting speakers separates the voices on this computer — the audio never leaves — and proposes “Voice 1” and “Voice 2” without deciding who anyone is; proposing an analysis suggests codes and citable passages, and any quotation that is not verbatim in the transcript is discarded and counted; correcting with AI punctuates and accents, and any segment that changes words is rejected and left as it was. Nothing is saved on its own, and none of the three touches an interview whose agreement does not allow it.',
    fr: 'L’IA entre dans l’entretien par trois opérations et une porte. La détection des locuteurs sépare les voix sur cet ordinateur — l’audio ne sort pas — et propose « Voix 1 » et « Voix 2 » sans décider qui est qui ; la proposition d’analyse suggère des codes et des passages citables, et toute citation qui n’apparaît pas littéralement dans la transcription est écartée et comptée ; la correction par IA ponctue et accentue, et tout segment qui change des mots est rejeté et laissé tel quel. Rien n’est enregistré tout seul, et aucune des trois ne touche un entretien dont l’accord ne l’autorise pas.',
    de: 'Die KI betritt das Interview über drei Operationen und ein Tor. Die Sprechererkennung trennt die Stimmen auf diesem Rechner – das Audio verlässt ihn nicht – und schlägt „Stimme 1“ und „Stimme 2“ vor, ohne zu entscheiden, wer jemand ist; der Analysevorschlag nennt Codes und zitierfähige Passagen, und jedes Zitat, das nicht wörtlich in der Transkription steht, wird verworfen und gezählt; die KI-Korrektur setzt Satzzeichen und Akzente, und jeder Abschnitt, der Wörter ändert, wird abgelehnt und bleibt wie er war. Nichts wird von selbst gespeichert, und keine der drei rührt ein Interview an, dessen Vereinbarung es nicht erlaubt.',
    pt: 'A IA entra na entrevista com três operações e uma porta. Detetar falantes separa as vozes da gravação neste computador — o áudio não sai — e propõe «Voz 1» e «Voz 2» sem decidir quem é quem; propor análise sugere códigos e passagens citáveis, e toda a citação que não apareça literal na transcrição é descartada e contada; corrigir com IA pontua e acentua, e o trecho que mude palavras é rejeitado e fica como estava. Nada se guarda sozinho, e nenhuma das três toca uma entrevista cujo acordo não o autorize.',
    'pt-BR': 'A IA entra na entrevista com três operações e uma porta. Detectar falantes separa as vozes da gravação neste computador — o áudio não sai — e propõe “Voz 1” e “Voz 2” sem decidir quem é quem; propor análise sugere códigos e passagens citáveis, e toda citação que não apareça literal na transcrição é descartada e contada; corrigir com IA pontua e acentua, e o trecho que mudar palavras é rejeitado e fica como estava. Nada é salvo sozinho, e nenhuma das três toca uma entrevista cujo acordo não autorize.',
  },
  {
    scope: 'testimonios',
    es: 'Búsqueda por significado, gobernada por el consentimiento. El índice semántico se construye sólo con las entrevistas cuyo acuerdo permite tratamiento por IA, y la consulta vuelve a comprobar el permiso: un embargo firmado hoy tapa lo que se indexó el mes pasado. La demo llega además con voces sintéticas de verdad —generadas a partir de su propio guion— para que se pueda transcribir, detectar hablantes y buscar desde el primer minuto sin haber grabado a nadie.',
    en: 'Meaning-based search, governed by consent. The semantic index is built only from interviews whose agreement allows AI processing, and each query re-checks the permission: an embargo signed today covers what was indexed last month. The demo now also ships real synthetic voices — generated from its own script — so you can transcribe, detect speakers and search from the first minute without having recorded anyone.',
    fr: 'Recherche par sens, gouvernée par le consentement. L’index sémantique n’est construit qu’avec les entretiens dont l’accord autorise le traitement par IA, et chaque requête revérifie la permission : un embargo signé aujourd’hui masque ce qui a été indexé le mois dernier. La démo est en outre livrée avec de vraies voix synthétiques — générées à partir de son propre script — pour transcrire, détecter les locuteurs et chercher dès la première minute sans avoir enregistré personne.',
    de: 'Bedeutungssuche, geregelt durch die Einwilligung. Der semantische Index entsteht nur aus Interviews, deren Vereinbarung KI-Verarbeitung erlaubt, und jede Abfrage prüft die Erlaubnis erneut: Eine heute unterschriebene Sperre verdeckt, was letzten Monat indexiert wurde. Die Demo bringt zudem echte synthetische Stimmen mit – aus ihrem eigenen Skript erzeugt –, damit man von der ersten Minute an transkribieren, Sprecher erkennen und suchen kann, ohne jemanden aufgenommen zu haben.',
    pt: 'Pesquisa por significado, governada pelo consentimento. O índice semântico constrói-se só com as entrevistas cujo acordo permite tratamento por IA, e a consulta volta a verificar a permissão: um embargo assinado hoje tapa o que se indexou no mês passado. A demonstração traz ainda vozes sintéticas a sério — geradas a partir do seu próprio guião — para se poder transcrever, detetar falantes e procurar desde o primeiro minuto sem ter gravado ninguém.',
    'pt-BR': 'Busca por significado, governada pelo consentimento. O índice semântico é construído só com as entrevistas cujo acordo permite tratamento por IA, e a consulta verifica a permissão de novo: um embargo assinado hoje tapa o que foi indexado no mês passado. A demonstração traz ainda vozes sintéticas de verdade — geradas a partir do próprio roteiro — para dar para transcrever, detectar falantes e buscar desde o primeiro minuto sem ter gravado ninguém.',
  },
  {
    scope: 'worldbuilding',
  ...
  {
    scope: 'worldbuilding',
    es: 'Mapas: sube tu propia imagen o genérala en diez estilos cartográficos, con mapas anidados —una ciudad es una chincheta en el continente y tiene mapa propio— y una escala calibrada. Pon chinchetas, contornos y rutas, mide distancias y tiempos de viaje, y Nodus te avisa de los viajes para los que la historia no deja tiempo. Los nombres los dibuja Nodus, no el modelo, así que siguen siendo legibles, buscables y cambian cuando renombras el lugar.',
    en: 'Maps: upload your own image or generate one in ten cartographic styles, with nested maps — a city is a pin on the continent and has a map of its own — and a calibrated scale. Drop pins, outlines and routes, measure distances and travel times, and Nodus tells you which journeys the story leaves no time for. Nodus draws the labels, not the model, so names stay legible, searchable and follow a renamed place.',
    fr: 'Cartes : importez votre propre image ou générez-la dans dix styles cartographiques, avec des cartes imbriquées —une ville est une épingle sur le continent et possède sa propre carte— et une échelle calibrée. Posez des épingles, des contours et des itinéraires, mesurez distances et temps de trajet, et Nodus vous signale les voyages pour lesquels l’histoire ne laisse pas le temps. Les noms sont dessinés par Nodus, pas par le modèle : ils restent donc lisibles, cherchables et suivent un lieu renommé.',
    de: 'Karten: Lade dein eigenes Bild hoch oder erzeuge es in zehn kartografischen Stilen, mit verschachtelten Karten – eine Stadt ist eine Nadel auf dem Kontinent und hat ihre eigene Karte – und kalibriertem Maßstab. Setze Nadeln, Umrisse und Routen, miss Entfernungen und Reisezeiten, und Nodus weist dich auf Reisen hin, für die die Geschichte keine Zeit lässt. Die Beschriftungen zeichnet Nodus, nicht das Modell, also bleiben Namen lesbar, durchsuchbar und folgen einem umbenannten Ort.',
    pt: 'Mapas: carregue a sua própria imagem ou gere-a em dez estilos cartográficos, com mapas aninhados —uma cidade é um alfinete no continente e tem mapa próprio— e uma escala calibrada. Ponha alfinetes, contornos e rotas, meça distâncias e tempos de viagem, e o Nodus avisa-o das viagens para as quais a história não deixa tempo. Os nomes são desenhados pelo Nodus, não pelo modelo, pelo que continuam legíveis, pesquisáveis e mudam quando renomeia o lugar.',
    'pt-BR': 'Mapas: envie sua própria imagem ou gere-a em dez estilos cartográficos, com mapas aninhados —uma cidade é um alfinete no continente e tem mapa próprio— e uma escala calibrada. Coloque alfinetes, contornos e rotas, meça distâncias e tempos de viagem, e o Nodus avisa das viagens para as quais a história não deixa tempo. Os nomes são desenhados pelo Nodus, não pelo modelo, então seguem legíveis, buscáveis e mudam quando você renomeia o lugar.',
  },
  {
    scope: 'worldbuilding',
    es: 'Enciclopedia A–Z con enlaces internos: artículos propios para lo que no es una entidad —magia, religiones, lenguas, criaturas, artefactos— y todas las demás entidades del mundo proyectadas como entradas, en un único índice. Escribes [[Nombre]] y el enlace queda hecho; los enlaces rojos se reparan solos en cuanto creas la entrada, y la biblia del mundo se exporta sin una sola dirección muerta.',
    en: 'An A–Z encyclopedia with internal links: native articles for the lore that is not an entity — magic, religions, languages, creatures, artifacts — and every other entity in the world projected as an entry, in one index. You write [[Name]] and the link is made; red links repair themselves as soon as you create the entry, and the world bible exports without a single dead address.',
    fr: 'Encyclopédie A–Z avec liens internes : des articles propres pour ce qui n’est pas une entité —magie, religions, langues, créatures, artefacts— et toutes les autres entités du monde projetées comme entrées, dans un index unique. Vous écrivez [[Nom]] et le lien est fait ; les liens rouges se réparent d’eux-mêmes dès que vous créez l’entrée, et la bible du monde s’exporte sans une seule adresse morte.',
    de: 'A–Z-Enzyklopädie mit internen Links: eigene Artikel für das, was keine Entität ist – Magie, Religionen, Sprachen, Kreaturen, Artefakte – und alle übrigen Entitäten der Welt als Einträge projiziert, in einem einzigen Index. Du schreibst [[Name]] und der Link steht; rote Links reparieren sich selbst, sobald du den Eintrag erstellst, und die Weltbibel exportiert ohne eine einzige tote Adresse.',
    pt: 'Enciclopédia A–Z com ligações internas: artigos próprios para o que não é uma entidade —magia, religiões, línguas, criaturas, artefactos— e todas as outras entidades do mundo projetadas como entradas, num único índice. Escreve [[Nome]] e a ligação fica feita; as ligações vermelhas reparam-se sozinhas assim que cria a entrada, e a bíblia do mundo exporta-se sem um único endereço morto.',
    'pt-BR': 'Enciclopédia A–Z com links internos: artigos próprios para o que não é uma entidade —magia, religiões, línguas, criaturas, artefatos— e todas as outras entidades do mundo projetadas como entradas, em um único índice. Você escreve [[Nome]] e o link fica feito; os links vermelhos se reparam sozinhos assim que você cria a entrada, e a bíblia do mundo é exportada sem um único endereço morto.',
  },
  {
    scope: 'worldbuilding',
    es: 'Analizar: Reglas, Conflictos, Arcos, Continuidad y Preguntas abiertas no son cinco secciones que alimentar, son cinco lecturas de lo que ya escribiste, y aparecen en la ficha de la escena que estás escribiendo. Y en el chat del mundo Nodus calcula los hechos —quién sabía qué, dónde estaba, qué contradice esto— y el modelo solo escribe con ellos: lo que el material no contiene se dice que falta en lugar de inventarse.',
    en: 'Analyse: Rules, Conflicts, Arcs, Continuity and Open questions are not five sections to feed, they are five readings of what you already wrote, and they appear on the sheet of the scene you are writing. And in the world chat Nodus computes the facts — who knew what, where they were, what this contradicts — and the model only writes with them: what the material does not contain is reported missing instead of invented.',
    fr: 'Analyser : Règles, Conflits, Arcs, Continuité et Questions ouvertes ne sont pas cinq sections à alimenter, ce sont cinq lectures de ce que vous avez déjà écrit, et elles apparaissent sur la fiche de la scène que vous écrivez. Et dans le chat du monde, Nodus calcule les faits —qui savait quoi, où il était, ce que cela contredit— et le modèle n’écrit qu’avec eux : ce que le matériau ne contient pas est déclaré manquant plutôt qu’inventé.',
    de: 'Analysieren: Regeln, Konflikte, Bögen, Kontinuität und offene Fragen sind keine fünf Abschnitte, die man füttern muss, sondern fünf Lesarten dessen, was du schon geschrieben hast – und sie erscheinen auf dem Blatt der Szene, an der du schreibst. Und im Welt-Chat berechnet Nodus die Fakten – wer wusste was, wo war er, was widerspricht dem – und das Modell schreibt nur mit ihnen: Was das Material nicht enthält, wird als fehlend benannt statt erfunden.',
    pt: 'Analisar: Regras, Conflitos, Arcos, Continuidade e Perguntas abertas não são cinco secções para alimentar, são cinco leituras do que já escreveu, e aparecem na ficha da cena que está a escrever. E no chat do mundo o Nodus calcula os factos —quem sabia o quê, onde estava, o que é que isto contradiz— e o modelo só escreve com eles: o que o material não contém é dito em falta em vez de inventado.',
    'pt-BR': 'Analisar: Regras, Conflitos, Arcos, Continuidade e Perguntas abertas não são cinco seções para alimentar, são cinco leituras do que você já escreveu, e aparecem na ficha da cena que você está escrevendo. E no chat do mundo o Nodus calcula os fatos —quem sabia o quê, onde estava, o que isso contradiz— e o modelo só escreve com eles: o que o material não contém é dito em falta em vez de inventado.',
  },
  {
    scope: 'worldbuilding',
    es: 'Manuscrito: la escena tiene por fin su texto, y se escribe con el mundo al lado —las beats que debe cumplir, las leyes en juego, las decisiones que la bloquean y las alertas de continuidad, ya calculadas, en el margen. Estantería de libros y capítulos, instantáneas antes de reescribir, modo máquina de escribir que mantiene la línea a la altura de los ojos, y una lectura con IA que solo informa de qué beats están en la página, sin juzgar la prosa ni reescribirla.',
    en: 'Manuscript: the scene finally has its text, and you write it with the world beside you — the beats it has to hit, the laws in play, the decisions blocking it and the continuity warnings, already computed, in the margin. A shelf of books and chapters, snapshots before a rewrite, a typewriter mode that keeps the line you are writing at eye level, and an AI reading that only reports which beats are on the page, without judging the prose or rewriting it.',
    fr: 'Manuscrit : la scène a enfin son texte, et vous l’écrivez avec le monde à côté —les beats qu’elle doit atteindre, les lois en jeu, les décisions qui la bloquent et les alertes de continuité, déjà calculées, dans la marge. Une étagère de livres et de chapitres, des instantanés avant une réécriture, un mode machine à écrire qui garde la ligne à hauteur des yeux, et une lecture par IA qui indique seulement quels beats sont sur la page, sans juger la prose ni la réécrire.',
    de: 'Manuskript: Die Szene hat endlich ihren Text, und du schreibst ihn mit der Welt daneben – die Beats, die sie treffen muss, die geltenden Gesetze, die Entscheidungen, die sie blockieren, und die Kontinuitätswarnungen, alle schon berechnet, am Rand. Ein Regal aus Büchern und Kapiteln, Momentaufnahmen vor dem Umschreiben, ein Schreibmaschinenmodus, der die Zeile auf Augenhöhe hält, und eine KI-Lesung, die nur berichtet, welche Beats auf der Seite stehen, ohne die Prosa zu bewerten oder umzuschreiben.',
    pt: 'Manuscrito: a cena tem finalmente o seu texto, e escreve-se com o mundo ao lado —as beats que deve cumprir, as leis em jogo, as decisões que a bloqueiam e os avisos de continuidade, já calculados, na margem. Estante de livros e capítulos, instantâneos antes de reescrever, modo máquina de escrever que mantém a linha à altura dos olhos, e uma leitura com IA que só informa que beats estão na página, sem julgar a prosa nem a reescrever.',
    'pt-BR': 'Manuscrito: a cena tem finalmente o seu texto, e você o escreve com o mundo ao lado —as beats que ela deve cumprir, as leis em jogo, as decisões que a bloqueiam e os avisos de continuidade, já calculados, na margem. Estante de livros e capítulos, instantâneos antes de reescrever, modo máquina de escrever que mantém a linha na altura dos olhos, e uma leitura com IA que só informa quais beats estão na página, sem julgar a prosa nem reescrevê-la.',
  },
  {
    scope: 'worldbuilding',
    es: 'La IA del mundo, siempre bajo un botón: propone una biografía a partir de la ficha que ya rellenaste, entrevista al personaje en su propia voz, y genera retratos con una semilla visual para que el segundo se parezca al primero. Galerías navegables, portadas para facciones y culturas, originales descargables a resolución completa, y un mundo de demostración ilustrado para verlo todo funcionando desde el primer minuto.',
    en: 'The world’s AI, always under a button: it proposes a biography from the sheet you already filled in, interviews the character in their own voice, and generates portraits with a visual seed so the second one still looks like the first. Navigable galleries, covers for factions and cultures, originals downloadable at full resolution, and an illustrated demo world so you can see it all working from the first minute.',
    fr: 'L’IA du monde, toujours sous un bouton : elle propose une biographie à partir de la fiche que vous avez déjà remplie, interroge le personnage dans sa propre voix et génère des portraits avec une graine visuelle pour que le second ressemble encore au premier. Des galeries navigables, des couvertures pour les factions et les cultures, des originaux téléchargeables en pleine résolution, et un monde de démonstration illustré pour voir tout cela fonctionner dès la première minute.',
    de: 'Die KI der Welt, immer unter einer Schaltfläche: Sie schlägt eine Biografie aus dem Steckbrief vor, den du schon gefüllt hast, befragt die Figur in ihrer eigenen Stimme und erzeugt Porträts mit einem visuellen Startwert, damit das zweite dem ersten noch gleicht. Durchblätterbare Galerien, Titelbilder für Fraktionen und Kulturen, Originale in voller Auflösung herunterladbar und eine illustrierte Demo-Welt, damit du alles von der ersten Minute an in Betrieb siehst.',
    pt: 'A IA do mundo, sempre sob um botão: propõe uma biografia a partir da ficha que já preencheu, entrevista a personagem na sua própria voz e gera retratos com uma semente visual para que o segundo se pareça com o primeiro. Galerias navegáveis, capas para fações e culturas, originais transferíveis em resolução completa, e um mundo de demonstração ilustrado para ver tudo a funcionar desde o primeiro minuto.',
    'pt-BR': 'A IA do mundo, sempre sob um botão: propõe uma biografia a partir da ficha que você já preencheu, entrevista o personagem na própria voz dele e gera retratos com uma semente visual para que o segundo se pareça com o primeiro. Galerias navegáveis, capas para facções e culturas, originais baixáveis em resolução completa, e um mundo de demonstração ilustrado para ver tudo funcionando desde o primeiro minuto.',
  },
  {
  ...
  {
    scope: 'primary_sources',
    es: 'En el archivo, la IA está sujeta a política y a evidencia: cada operación queda registrada con su proveedor y su modelo, los embeddings prefieren el texto ya revisado, y todo resultado automático se queda como propuesta hasta que una persona lo acepta. Las políticas de privacidad de la bóveda se aplican de verdad, y las citas y exportaciones llevan su rastro de auditoría.',
    en: 'In the archive, AI is bound by policy and by evidence: every operation is recorded with its provider and model, embeddings prefer text that has already been reviewed, and every automatic result stays a proposal until a person accepts it. The vault’s privacy policies are genuinely enforced, and citations and exports carry their audit trail.',
    fr: 'Dans l’archive, l’IA est soumise à la politique et à la preuve : chaque opération est enregistrée avec son fournisseur et son modèle, les embeddings préfèrent le texte déjà relu, et tout résultat automatique reste une proposition jusqu’à ce qu’une personne l’accepte. Les politiques de confidentialité du coffre s’appliquent vraiment, et les citations et exports portent leur piste d’audit.',
    de: 'Im Archiv ist die KI an Richtlinie und Evidenz gebunden: Jede Operation wird mit Anbieter und Modell protokolliert, Embeddings bevorzugen bereits geprüften Text, und jedes automatische Ergebnis bleibt ein Vorschlag, bis ein Mensch es annimmt. Die Datenschutzrichtlinien des Tresors greifen wirklich, und Zitate und Exporte tragen ihre Prüfspur.',
    pt: 'No arquivo, a IA está sujeita a política e a evidência: cada operação fica registada com o seu fornecedor e o seu modelo, os embeddings preferem o texto já revisto, e todo o resultado automático fica como proposta até que uma pessoa o aceite. As políticas de privacidade do cofre aplicam-se a sério, e as citações e exportações levam o seu rasto de auditoria.',
    'pt-BR': 'No arquivo, a IA está sujeita a política e a evidência: cada operação fica registrada com seu provedor e seu modelo, os embeddings preferem o texto já revisado, e todo resultado automático fica como proposta até que uma pessoa o aceite. As políticas de privacidade do cofre se aplicam de verdade, e as citações e exportações levam sua trilha de auditoria.',
  },
  {
    scope: 'prosopography',
  ...
  {
    scope: 'prosopography',
    es: 'Las entrevistas de prosopografía se diarizan de verdad: la separación de hablantes conserva intacta la transcripción literal, se alinea con la cronología, admite indicar qué hablantes esperas, acepta archivos grandes y garantiza que el fichero remoto se borra al terminar.',
    en: 'Prosopography interviews get real diarization: speaker separation keeps the literal transcript intact, aligns with the timeline, lets you say which speakers you expect, accepts large files and guarantees the remote file is deleted when it finishes.',
    fr: 'Les entretiens de prosopographie sont réellement diarisés : la séparation des locuteurs conserve intacte la transcription littérale, s’aligne sur la chronologie, permet d’indiquer quels locuteurs vous attendez, accepte les gros fichiers et garantit que le fichier distant est supprimé à la fin.',
    de: 'Prosopografie-Interviews werden wirklich diarisiert: Die Sprechertrennung lässt die wörtliche Transkription unberührt, richtet sich an der Zeitleiste aus, erlaubt die Angabe der erwarteten Sprecher, nimmt große Dateien an und garantiert, dass die entfernte Datei am Ende gelöscht wird.',
    pt: 'As entrevistas de prosopografia são diarizadas a sério: a separação de falantes mantém intacta a transcrição literal, alinha-se com a cronologia, permite indicar que falantes espera, aceita ficheiros grandes e garante que o ficheiro remoto é apagado ao terminar.',
    'pt-BR': 'As entrevistas de prosopografia são diarizadas de verdade: a separação de falantes mantém intacta a transcrição literal, alinha-se com a cronologia, permite indicar quais falantes você espera, aceita arquivos grandes e garante que o arquivo remoto é apagado ao terminar.',
  },
  {
    scope: 'docencia',
