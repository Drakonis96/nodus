...
const RAW = [
  ...
  '   Give each species as its systematic IUPAC name ONLY — no SMILES, no formula, no backticks, except the one structure fallback below. List',
  ...
  '   - If, and only if, you cannot give a systematic name a reference service will resolve — an exotic',
  '     fused cage or a named literature intermediate — write that species as its name followed by its',
  '     isomeric SMILES in backticks, for example: «the photodimer — `C1=CC2(C=C1)OCCO2`». The application',
  '     takes the SMILES as the structure and checks it with RDKit. Give the name alone for every species a',
  '     systematic name will resolve; never use the fallback for a species you can name.',
  ...
  '   - Do NOT write any SMILES (except the single structure fallback above), molecular formula, reaction',
  '     string or «reactants>agents>products» line anywhere. The application derives every structure and',
  '     every balanced equation from your names, so an equation is never your job.',
